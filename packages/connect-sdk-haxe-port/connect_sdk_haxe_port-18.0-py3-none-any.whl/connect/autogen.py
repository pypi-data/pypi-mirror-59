...
class connect_Blob(connect_Base):
    _hx_class_name = "connect.Blob"
    __slots__ = ("bytes",)
    _hx_fields = ["bytes"]
    ...
    def _toArray(self):
        return connect_Blob._bytesToArray(self._getBytes())

    @staticmethod
    def load(path):
        return connect_Blob(sys_io_File.getBytes(path))

    @staticmethod
    def _fromBytes(_hx_bytes):
        return connect_Blob(_hx_bytes)
    ...
    def _hx_empty_init(_hx_o):
        _hx_o.bytes = None
connect_Blob._hx_class = connect_Blob
_hx_classes["connect.Blob"] = connect_Blob
...
class connect_Config(connect_Base):
    ...
    def __init__(self,apiUrl,apiKey,products,data):
        ...
        self.products = (products.copy() if ((products is not None)) else connect_Collection())
...
class connect_DateTime:
    _hx_class_name = "connect.DateTime"
    ...
    def now():
        date = Date.now()
        return connect_DateTime(date.dateUTC.year,(date.dateUTC.month - 1),date.dateUTC.day,date.dateUTC.hour,date.dateUTC.minute,date.dateUTC.second)
    ...
    def fromString(s):
        ...
        return connect_DateTime(year,month,day,hour,minute,second)
    ...
    def _hx_empty_init(_hx_o):
        ...
        _hx_o.minutes = None
        _hx_o.seconds = None
connect_DateTime._hx_class = connect_DateTime
_hx_classes["connect.DateTime"] = connect_DateTime
...
class connect_Diff:
    ...
    def __init__(self,first,second):
        ...
        self.a = None
        _gthis = self
        connect_Diff.checkStructs(first,second)
        ...
        commonFields = _g4
        def _hx_local_3(f3):
            return (not connect_Diff.areEqual(Reflect.field(first,f3),Reflect.field(second,f3)))
        ...
        def _hx_local_6(f6):
            ...
            if (connect_Util.isStruct(a) and connect_Util.isStruct(b)):
                _this2 = _gthis.c
                value2 = connect_Diff(a,b)
                _this2.h[f6] = value2
            elif (connect_Util.isArray(a) and connect_Util.isArray(b)):
                _this3 = _gthis.c
                value3 = connect_Diff.compareArrays(a,b)
                _this3.h[f6] = value3
            else:
                _gthis.c.h[f6] = [a, b]
        ...
        def _hx_local_2(k8):
            ...
            if Std._hx_is(change,list):
                if (Reflect.field(change,"length") == 2):
                    ...
                    value1 = connect_Diff.applyArray(original,change)
        ...
        def _hx_local_0(k2):
            ...
            if Std._hx_is(change,list):
                if (Reflect.field(change,"length") == 2):
                    ...
                    value = connect_Diff.swapArray(change)
                    changes.h[k2] = value
            else:
                value1 = Reflect.field(change,"swap")()
                changes.h[k2] = value1
        Lambda.iter(changedKeys,_hx_local_0)
        diff = Type.createEmptyInstance(connect_Diff)
    ...
    def applyArray(obj,arr):
        ...
        def _hx_local_0(change):
            ...
            tmp = ((change[2] if 2 < len(change) else None) if ((len(change) == 3)) else (connect_Diff.applyArray(originalArray,(change[1] if 1 < len(change) else None)) if (Std._hx_is((change[1] if 1 < len(change) else None),list)) else Reflect.field((change[1] if 1 < len(change) else None),"apply")(originalObject)))
    ...
    def swapArray(arr):
        ...
        def _hx_local_0(change):
            ...
            if (len(change) == 3):
                ...
                return [i, connect_Diff.swapArray((change[1] if 1 < len(change) else None))]
            else:
        ...
        def _hx_local_2(f2):
            ...
            if Std._hx_is(change,list):
                if (Reflect.field(change,"length") == 2):
                    ...
                    value2 = connect_Diff.parseArray(change)
                    changes.h[f2] = value2
            else:
                value3 = connect_Diff.parse(change)
                changes.h[f2] = value3
    ...
    def parseArray(arr):
        ...
        def _hx_local_0(el):
            ...
            if (len(el) == 3):
                ...
                return [i, connect_Diff.parseArray((el[1] if 1 < len(el) else None))]
            else:
                return [i, connect_Diff.parse((el[1] if 1 < len(el) else None))]
        changes = list(map(_hx_local_0,(arr[2] if 2 < len(arr) else None)))
        return [additions, deletions, changes]
    ...
    def compareArrays(first,second):
        ...
        while x.hasNext():
            ...
            x2 = (None if (connect_Diff.areEqual(a,b)) else ([i1, connect_Diff(a,b)] if ((connect_Util.isStruct(a) and connect_Util.isStruct(b))) else ([i1, connect_Diff.compareArrays(a,b)] if ((connect_Util.isArray(a) and connect_Util.isArray(b))) else [i1, a, b])))
            _g.append(x2)
        changeList = _g
        _g1 = []
        _g11 = 0
    ...
    def isSupported(v):
        ...
        if (not ((isSimple or connect_Util.isArray(v)))):
            return connect_Util.isStruct(v)
        else:
            return True

    @staticmethod
    def checkStructs(first,second):
        if ((not connect_Util.isStruct(first)) or (not connect_Util.isStruct(second))):
    ...
    def _hx_empty_init(_hx_o):
        ...
        _hx_o.d = None
        _hx_o.c = None
connect_Diff._hx_class = connect_Diff
_hx_classes["connect.Diff"] = connect_Diff
...
class connect_Env(connect_Base):
    ...
    def loadConfig(filename):
        if (connect_Env.config is None):
            ...
            products = (configProducts if (Std._hx_is(configProducts,connect_Collection)) else (connect_Collection._fromArray([configProducts]) if (Std._hx_is(configProducts,str)) else None))
...
class connect_Flow(connect_Base):
    ...
    def approveByTemplate(self,id):
        ...
        if (request is not None):
            connect_StepStorage.removeStepData(request.id,self.getStepParam())
            if (request.asset is not None):
                request.update()
            request.approveByTemplate(id)
            self.abort("")
        elif (tcr is not None):
            connect_StepStorage.removeStepData(tcr.id,self.getStepParam())
    ...
    def _run(self,_hx_list):
        _gthis = self
        connect_Env.getLogger().openSection(((("Running " + HxOverrides.stringOrNull(self.getClassName())) + " on ") + Std.string(connect_DateTime.now())))
        def _hx_local_5():
            def _hx_local_3(m):
                def _hx_local_2():
                    def _hx_local_1():
                ...
                return _hx_local_2()
            return connect_Collection._fromArray(list(filter(_hx_local_3,_hx_list.toArray()))) if (self.filterFunc is not None) else _hx_list
    ...
    def loadStepDataIfStored(self):
        stepData = connect_StepStorage.load(self.model.id,self.getStepParam())
        self.data = stepData.data
        if (stepData.storage != connect_StorageType.FailedStorage):
    ...
    def processStep(self,step,index,lastRequestStr,lastDataStr):
        requestStr = connect_Util.beautifyObject(self.model.toObject(),(connect_Env.getLogger().getLevel() != connect_logger_Logger.LEVEL_DEBUG))
    ...
    def prepareRequestAndOpenLogSection(self,model):
        ...
        if (self.getAssetRequest() is not None):
            connect_Env.getLogger().setFilename((("" + HxOverrides.stringOrNull(self.getAssetRequest().asset.id)) + ".md"))
        elif (self.getTierConfigRequest() is not None):
            connect_Env.getLogger().setFilename((("" + HxOverrides.stringOrNull(self.getTierConfigRequest().configuration.id)) + ".md"))
        elif (self.getUsageFile() is not None):
            connect_Env.getLogger().setFilename((("" + HxOverrides.stringOrNull(self.getUsageFile().id)) + ".md"))
        connect_Env.getLogger().openSection(((("Processing request \"" + HxOverrides.stringOrNull(self.model.id)) + "\" on ") + Std.string(connect_DateTime.now())))
        if ((self.getAssetRequest() is not None) and self.getAssetRequest().needsMigration()):
    ...
    def processAbortAndCloseLogSection(self,index,requestStr,dataStr):
        if self.abortRequested:
            if (self.abortMessage is None):
                ...
                saveResult = connect_StepStorage.save(self.model,connect_StepData(index,self.data,connect_StorageType.ConnectStorage),param,Reflect.field(self.model,"update"))
    ...
    def logStepData(self,level,request,data,lastRequest,lastData):
        ...
        while output.hasNext():
            output1 = output.next()
            _hx_list = connect_Collection().push(connect_Flow.getFormattedRequest(request,lastRequest,output1.formatter)).push(connect_Flow.getFormattedData(data,lastData,self.data,output1.formatter))
            connect_Env.getLogger()._writeToOutput(level,output1.formatter.formatList(_hx_list),output1)
    ...
    def getAssetRequestChanges(self):
        if (self.getAssetRequest() is not None):
            ...
            diff = connect_Util.createObjectDiff(self.model.toObject(),originalModel)
    ...
    def getFormattedRequest(request,lastRequest,fmt):
        if (request != lastRequest):
            if (connect_Env.getLogger().getLevel() == connect_logger_Logger.LEVEL_DEBUG):
                lastRequestObj = (python_lib_Json.loads(lastRequest,**python__KwArgs_KwArgs_Impl_.fromT(_hx_AnonObject({'object_hook': python_Lib.dictToAnon}))) if (connect_Util.isJsonObject(lastRequest)) else None)
                requestObj = (python_lib_Json.loads(request,**python__KwArgs_KwArgs_Impl_.fromT(_hx_AnonObject({'object_hook': python_Lib.dictToAnon}))) if ((connect_Util.isJsonObject(request) and ((lastRequestObj is not None)))) else None)
                diff = (connect_Util.createObjectDiff(requestObj,lastRequestObj) if (((lastRequestObj is not None) and ((requestObj is not None)))) else None)
                requestStr = (connect_Util.beautifyObject(diff,False) if ((diff is not None)) else request)
    ...
    def getDataTable(data,fmt):
        ...
        while key.hasNext():
            ...
            _g.append(key1)
        dataKeys = _g
        dataCol = connect_Collection().push(connect_Collection().push("Key").push("Value"))
        def _hx_local_0(key2):
            tmp = connect_Collection().push(key2).push(data.get(key2))
            dataCol.push(tmp)
...
class connect_Processor(connect_Base):
    ...
    def run(self,modelClass,filters):
        ...
        connect_Env.getLogger().openSection(("Running Processor on " + Std.string(connect_DateTime.now())))
        try:
            connect_Env.getLogger().openSection(("Listing requests on " + Std.string(connect_DateTime.now())))
...
class connect_StorageType(Enum):
    __slots__ = ()
    _hx_class_name = "connect.StorageType"
    _hx_constructs = ["ConnectStorage", "LocalStorage", "FailedStorage"]
connect_StorageType.ConnectStorage = connect_StorageType("ConnectStorage", 0, ())
connect_StorageType.LocalStorage = connect_StorageType("LocalStorage", 1, ())
connect_StorageType.FailedStorage = connect_StorageType("FailedStorage", 2, ())
connect_StorageType._hx_class = connect_StorageType
_hx_classes["connect.StorageType"] = connect_StorageType
...
class connect_StepStorage:
    ...
    def load(requestId,param):
        fileData = connect_StepStorage.loadRequestFromFile(requestId)
        stepData = (fileData if ((fileData is not None)) else connect_StepStorage.loadRequestFromParam(requestId,param))
        if (stepData is not None):
            return stepData
        else:
            return connect_StepData(0,_hx_AnonObject({}),connect_StorageType.FailedStorage)

    @staticmethod
    def save(request,stepData,param,updateFunc):
        connect_StepStorage.removeStepDataFromFile(request.id)
        paramObj = connect_StepStorage.objWithRequestData(connect_StepStorage.loadAllFromParam(param),request.id,stepData)
        fileObj = connect_StepStorage.objWithRequestData(connect_StepStorage.loadAllFromFile(),request.id,stepData)
        if connect_StepStorage.saveInConnect(request,connect_StepStorage.encodeData(paramObj),param,updateFunc):
            return connect_StorageType.ConnectStorage
        elif connect_StepStorage.saveInFile(connect_StepStorage.encodeData(fileObj)):
            return connect_StorageType.LocalStorage
        else:
            return connect_StorageType.FailedStorage

    @staticmethod
    def removeStepData(requestId,param):
        connect_StepStorage.removeStepDataFromFile(requestId)
        paramObj = connect_StepStorage.loadAllFromParam(param)
        if ((paramObj is not None) and python_Boot.hasField(paramObj,requestId)):
            param.value = connect_StepStorage.encodeData(connect_StepStorage.objWithoutRequestData(paramObj,requestId))
            return True
        return False

    @staticmethod
    def removeStepDataFromFile(requestId):
        fileObj = connect_StepStorage.loadAllFromFile()
        if ((fileObj is not None) and python_Boot.hasField(fileObj,requestId)):
            fileData = connect_StepStorage.objWithoutRequestData(fileObj,requestId)
            connect_StepStorage.saveInFile(connect_StepStorage.encodeData(fileData))
    ...
    def loadRequestFromParam(requestId,param):
        dataObject = connect_StepStorage.loadAllFromParam(param)
        if (dataObject is not None):
            return connect_StepStorage.getRequestField(dataObject,requestId,connect_StorageType.ConnectStorage)
        return None

    @staticmethod
    def loadRequestFromFile(requestId):
        dataObject = connect_StepStorage.loadAllFromFile()
        if (dataObject is not None):
            return connect_StepStorage.getRequestField(dataObject,requestId,connect_StorageType.LocalStorage)
        return None

    @staticmethod
    def loadAllFromParam(param):
        if (((param is not None) and ((param.value is not None))) and ((param.value != ""))):
            return connect_StepStorage.decodeData(param.value)
        return None

    @staticmethod
    def loadAllFromFile():
        dataFilename = connect_StepStorage.getDataFilename()
        if (sys_FileSystem.exists(dataFilename) and (not sys_FileSystem.isDirectory(dataFilename))):
            return connect_StepStorage.decodeData(sys_io_File.getContent(dataFilename))
        return None
    ...
    def getRequestField(object,requestId,storage):
        if python_Boot.hasField(object,requestId):
            ...
            return connect_StepData(stepData.current_step,stepData.data,storage)
        return None
...
class connect_Util:
    ...
    def beautify(text,compact):
        try:
            return connect_Util.beautifyObject(python_lib_Json.loads(text,**python__KwArgs_KwArgs_Impl_.fromT(_hx_AnonObject({'object_hook': python_Lib.dictToAnon}))),compact)
        except Exception as _hx_e:
            _hx_e1 = _hx_e.val if isinstance(_hx_e, _HxException) else _hx_e
            ex = _hx_e1
            return text
    ...
    @staticmethod
    def isJson(text):
        if (not connect_Util.isJsonObject(text)):
            return connect_Util.isJsonArray(text)
        else:
            return True
    ...
    def createObjectDiff(object,previous):
        return connect_Util.addIdsToObject(connect_Diff(previous,object).apply(_hx_AnonObject({'id': Reflect.field(object,"id")})),previous)
    ...
    def addIdsToObject(object,original):
        ...
        def _hx_local_0(field):
            ...
            if python_Boot.hasField(original,field):
                ...
                if (connect_Util.isStruct(value1) and connect_Util.isStruct(originalValue)):
                    value2 = connect_Util.addIdsToObject(value1,originalValue)
                ...
                elif (connect_Util.isArray(value1) and connect_Util.isArray(originalValue)):
                    value3 = connect_Util.compactArray(value1,originalValue)
    ...
    def compactArray(array,second):
        ...
        while (_g < _g1):
            ...
            if (connect_Util.isStruct(value) and connect_Util.isStruct(secondValue)):
                x = connect_Util.addIdsToObject(value,secondValue)
                out.append(x)
            elif (connect_Util.isArray(value) and connect_Util.isArray(secondValue)):
                x1 = connect_Util.compactArray(value,secondValue)
                out.append(x1)
...
class connect_api_impl_ApiClientImpl(connect_Base):
    ...
    def logRequest(level,method,url,headers,body,response):
        ...
        while output.hasNext():
            ...
            requestList = connect_Collection()
            ...
            connect_Env.getLogger()._writeToOutput(level,fmt.formatBlock((("" + ("null" if firstMessage is None else firstMessage)) + HxOverrides.stringOrNull(fmt.formatList(requestList)))),output1)
    ...
    def getHeadersTable(headers,fmt):
        ...
        while key.hasNext():
            ...
            _g.append(key1)
        headerKeys = _g
        headersCol = connect_Collection().push(connect_Collection().push("Name").push("Value"))
        def _hx_local_0(key2):
            tmp = connect_Collection().push(key2).push(fixedHeaders.get(key2))
            headersCol.push(tmp)
    ...
    def getFormattedData(data,title,fmt):
        if connect_Util.isJson(data):
            ...
            block = fmt.formatCodeBlock(connect_Util.beautify(data,compact),"json")
...
class connect_logger_Logger(connect_Base):
    ...
    __slots__ = ("path", "level", "outputs", "sections")
    _hx_fields = ["path", "level", "outputs", "sections"]
    _hx_methods = ["getPath", "getLevel", "setFilename", "getFilename", "getOutputs", "openSection", "closeSection", "writeBlock", "writeCodeBlock", "writeList", "writeTable", "write", "log", "debug", "info", "notice", "warning", "error", "critical", "alert", "emergency", "_writeToOutput", "writeSections"]
    ...
    def __init__(self,config):
        ...
        try:
            ...
            tmp = None
        self.level = tmp
        self.outputs = config.outputs_.copy()
        self.sections = []
    ...
    def setFilename(self,filename):
        ...
        def _hx_local_0(o,last):
            if last:
                ...
                return False
        setFilenameResult = Lambda.fold(self.outputs,_hx_local_0,True)
    ...
    def getFilename(self):
        firstWriter = (self.outputs.get(0).writer if ((self.outputs.length() > 0)) else None)
    ...
    def writeBlock(self,level,block):
        output = self.outputs.iterator()
        while output.hasNext():
            output1 = output.next()
            self._writeToOutput(level,output1.formatter.formatBlock(block),output1)

    def writeCodeBlock(self,level,code,language):
        output = self.outputs.iterator()
        while output.hasNext():
            output1 = output.next()
            self._writeToOutput(level,output1.formatter.formatCodeBlock(code,language),output1)

    def writeList(self,level,_hx_list):
        output = self.outputs.iterator()
        while output.hasNext():
            output1 = output.next()
            self._writeToOutput(level,output1.formatter.formatList(_hx_list),output1)

    def writeTable(self,level,table):
        output = self.outputs.iterator()
        while output.hasNext():
            output1 = output.next()
            self._writeToOutput(level,output1.formatter.formatTable(table),output1)

    def write(self,level,message):
        output = self.outputs.iterator()
        while output.hasNext():
            output1 = output.next()
            self._writeToOutput(level,message,output1)
    ...
    def _writeToOutput(self,level,message,output):
        if (self.level >= level):
            self.writeSections()
...
class connect_logger_LoggerConfig(connect_Base):
    ...
    __slots__ = ("path_", "level_", "outputs_")
    _hx_fields = ["path_", "level_", "outputs_"]
    _hx_methods = ["path", "level", "outputs"]
    _hx_statics = []
    _hx_interfaces = []
    _hx_super = connect_Base
    ...
    def __init__(self):
        ...
        self.outputs_ = connect_Collection().push(connect_logger_LoggerOutput(connect_logger_MarkdownLoggerFormatter(),connect_logger_FileLoggerWriter()))
...
class connect_logger_LoggerOutput:
    _hx_class_name = "connect.logger.LoggerOutput"
    ...
    def _hx_empty_init(_hx_o):
        ...
        _hx_o.writer = None
connect_logger_LoggerOutput._hx_class = connect_logger_LoggerOutput
_hx_classes["connect.logger.LoggerOutput"] = connect_logger_LoggerOutput
                ...
                if (tmp == 4):
                    ...
                    if (_g2.params[0] == list):
                        ...
                        if (fieldClassName2 == "DateTime"):
                            Reflect.setProperty(model,field,connect_DateTime.fromString(val))
                        else:
                            try:
...
class connect_models_TierAccount(connect_models_IdModel):
    ...
    __slots__ = ("externalId", "name", "environment", "scope", "contactInfo", "marketplace", "hub", "externalUid")
    _hx_fields = ["externalId", "name", "environment", "scope", "contactInfo", "marketplace", "hub", "externalUid"]
    _hx_methods = []
    _hx_statics = ["list", "get"]
    _hx_interfaces = []
    _hx_super = connect_models_IdModel
...
class connect_models_UsageFile(connect_models_IdModel):
    ...
    def uploadRecords(self,records):
        ...
        data = connect_Blob._fromBytes(sheet)
    ...
    def zipSheet(name,sheet):
        ...
        entries.add(connect_models_UsageFile.zipEntry("docProps/core.xml",StringTools.replace(connect_models_UsageFile.CORE,"%DATE%",connect_DateTime.now().toString())))
...
class connect_native_PythonRequest:
    ...
    def request(method,url,headers,body,fileArg,fileName,fileContent,timeout):
        ...
        return connect_api_Response(resp.status_code,resp.text,connect_Blob._fromBytes(contentBytes))
...
class connect_native_PythonZlib:
    ...
    def compress(data,level):
        import zlib
        pythonBytes = bytes(connect_Blob._bytesToArray(data))

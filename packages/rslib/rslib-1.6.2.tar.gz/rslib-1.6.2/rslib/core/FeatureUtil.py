...
class FeatureUtil():
    ...
    def feature_extraction(self, data, padding=False, sparse=True):
        ...
        pos_label = [[x for x in map(int, sample.split('@')[feature_id['pos_label']].split(',')) if x != 0] for sample in data]
        neg_label = [[x for x in map(int, sample.split('@')[feature_id['neg_label']].split(',')) if x != 0] for sample in data]
        mask = [[x for x in set(pos_label[i] + neg_label[i]) if x != 0] for i in range(len(data))]

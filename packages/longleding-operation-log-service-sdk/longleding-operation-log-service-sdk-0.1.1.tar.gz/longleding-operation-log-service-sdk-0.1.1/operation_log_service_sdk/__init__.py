...
from .grpc_client import OperationLog, OperationLogServiceGRPCClient

__all__ = ["init_service", "log", "get_operation_logs"]
_log_service_grpc_client: OperationLogServiceGRPCClient
_source: str = ""


def init_service(endpoint: str, source: str) -> None:
    global _log_service_grpc_client
    global _source
    assert type(endpoint) == str, "endpoint must be a str"
    assert type(source) == str, "source must be a str"
    _log_service_grpc_client = OperationLogServiceGRPCClient(endpoint=endpoint)
    _source = source
...
def log(time: datetime = datetime.now(),
        ...
        content: str = "") -> None:
    global _log_service_grpc_client
    assert _log_service_grpc_client is not None, "log service sdk must be init first"
    ...
    _log_service_grpc_client.log(log_message)
...
def get_operation_logs(start_time: datetime,
    ...
    return _log_service_grpc_client.get_logs(start_time, end_time, offset, limit, reverse, topic_in, source_in, tag_contains)

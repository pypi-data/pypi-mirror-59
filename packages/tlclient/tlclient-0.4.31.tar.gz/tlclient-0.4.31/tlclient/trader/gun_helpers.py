...
class GunInspector(object):
    ...
    def inspect_fists(self, **kwargs):
        ...
        for fi in fist_infos:
            start_time = datetime.datetime.fromtimestamp(fi['start_nano'] / 1e9).strftime('%Y%m%d-%S:%M:%S') if fi['start_nano'] else '-'
            end_time = datetime.datetime.fromtimestamp(fi['end_nano'] / 1e9).strftime('%Y%m%d-%S:%M:%S') if fi['end_nano'] else '-'

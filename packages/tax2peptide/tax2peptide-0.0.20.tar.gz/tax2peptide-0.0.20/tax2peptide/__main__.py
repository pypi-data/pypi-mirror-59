...
def main():
    ...
    if options.path:
        ...
        if not database_folder.exists():
            try:
                ...
                logger.info("New folder %s (option --path) created. All needed database files will be downloaded and "
                            "stored in this direction." % database_folder, exc_info=True)
                prot_gz_b = prot_b = pdb_b = taxdump_b = db_gz_b = db_b = False
                skip_check = True
            except OSError:
                logger.exception("Folder of option --path (%s) does not exist and can not be created."
                                 % database_folder, exc_info=True)
                exit(1)
    ...
    if not skip_check:
        taxdump_b, prot_gz_b, prot_b, pdb_b, db_gz_b, db_b = Output.check_files_exist([
            ...
            database_folder/'prot.accession2taxid', database_folder / url_pdbaccession2taxID.split('/')[-1], path_to_db,

...
class MsSqlTools(object):
    ...
    def get_conn(self):
        return pymssql.connect(server=self.host, user=self.user,password=self.password,database=self.db,charset=self.charset)
    ...
    def query_table(self, table_name, where_filed=['1'], where_values=['1'],top=-1):
        try:
            ...
            with conn.cursor() as cursor:
                ...
                if top>0:
                    sql = f"SELECT top {top} * FROM {table_name} WHERE {where_place_holder}"
                else:

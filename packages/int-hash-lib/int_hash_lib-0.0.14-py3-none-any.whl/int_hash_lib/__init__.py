...
def normalize_broken_format(data):
    if data in ('[]', '{}', '()'):
        return
    return data
    ...
    for k, v in data.items():
        ...
        if hs:
            ...
            if isinstance(v, (datetime.datetime, datetime.date)):
                ...
                v = ','.join([str(d) for d in v if normalize_broken_format(str(d))])
            item[k] = v
    ...
    with StringIO() as f:
        for idx, row in enumerate(rd):
            ...
            for k, v in row_dict.items():
                if k in split_comma:
                    row_dict[k] = [r.strip() for r in v.split(',') if normalize_broken_format(r)]

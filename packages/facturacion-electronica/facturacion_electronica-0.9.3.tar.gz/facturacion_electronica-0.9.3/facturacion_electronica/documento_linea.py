...
class LineaDetalle(object):
    ...
    def MontoItemOtrMnda(self, val):
        if type(val) is str:
            val = val.replace(',', '.')
        self._monto_item_otr_mnda = round(float(val))
    ...
    def RecargoMonto(self, val):
        if type(val) is str:
            val = val.replace(',', '.')
        self._recargo_monto = int(round(float(val), 0))
    ...
    def _compute_price(self):
        ...
        if self.RecargoPct > 0 and not self.RecargoMonto:
            self.RecargoMonto = (total * ((self.RecargoPct or 0.0) / 100.0))
        if self.RecargoMonto > 0:
            total += self.RecargoMonto

...
def by_attr(name):
    """
    Returns a function that gets an item by the specified attribute name.
    ...
    """
    return lambda it: getattr(it, name)


def by_attrs(*names):
    """
    Returns a function that gets a tuple of items by the specified attribute names.
    ...
    (
        Slinkie(items)
            .sort(by_attrs('name', 'number'))
    )
    """
    return lambda it: tuple(getattr(it, name) for name in names)

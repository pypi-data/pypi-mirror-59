...
class BlockGenerateDecompositions(BlockNode):
    ...
    def __init__(self, child, num_decomps=None, inference_type=InferenceType.MARGINAL,
                 name="BlockRandomDecompositions", fixed_permutations=None):
        super().__init__(inference_type=inference_type, name=name, num_decomps=num_decomps)
        self.set_values(child)
        if num_decomps is None and fixed_permutations is None:
            raise ValueError("Must provide either fixed_permutations or num_decomps")
        self._num_decomps = num_decomps or fixed_permutations
        self._permutations = None
        if fixed_permutations is not None:
            if num_decomps is not None and fixed_permutations.shape[0] != num_decomps:
                raise ValueError("Number of permutations should match the number of decomps")
        self._fixed_permutations = fixed_permutations

    def generate_permutations(self, factors):
        if self._fixed_permutations is not None:
            self.set_fixed_permutations(factors, self._fixed_permutations)
        else:
            self.generate_random_permutations(factors)

    def generate_random_permutations(self, factors):
        if not factors:
    ...
    def set_fixed_permutations(self, factors, perms):
        if not factors:
            raise StructureError("{}: factors needs to be a non-empty sequence.")
        num_input_scopes = self._get_num_input_scopes()
        factor_cumprod = np.cumprod(factors)

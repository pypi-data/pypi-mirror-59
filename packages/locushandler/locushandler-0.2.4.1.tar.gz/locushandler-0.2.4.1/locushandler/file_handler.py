...
def map_to_locus(file_path_or_df, file_code_column, barcode_field, granularity='4x6',
    ...
    if isinstance(file_path_or_df, pd.DataFrame):
        ...
        try:
            df_data = pd.read_csv(
                file_path_or_df, encoding=encoding_data, header=header)
    ...
    df_data[file_code_column] = df_data[file_code_column].astype(
        int).astype(str)
    ...
    if merge:
        df_data = merge_locus(
            df_data, column_of_interest=barcode_field, merge_all=True)
    return df_data
...
def get_mapping(barcode_field, classification_system=None, classification_year=None,
    ...
    cols = [f'{barcode_field}_{element}' for element in
            fh.gran_to_fields(granularity,
                              type_column,
                              show_dr,
                              show_io)]
...
def merge_locus(df, column_of_interest, merge_all=False, merge_act=False, merge_obj=False,
    ...
    def trans_int(x): return int(x) if x != '' and x != 'V' else x
    def join_nopoint(x): return ''.join(x)
    def join_point(x): return '.'.join(x)
    def parenthese(x): return '(' + x + ')' if x != '' else x
    ...
    if merge_dr and dr:
        ...
        if 'dr_stage' in dr:
            df[f'{column_of_interest}_dr_stage'] = df[f'{column_of_interest}_dr_stage'].apply(
                trans_int)
    ...
    if merge_io and io:
        if 'io_stage' in io:
            df[f'{column_of_interest}_io_stage'] = df[f'{column_of_interest}_io_stage'].apply(
                trans_int)

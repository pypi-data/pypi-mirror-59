...
def get_basic_group(
    ...
    url = "https://api.mnemonic.no/users/v2/group/{shortNameOrID}".format(shortNameOrID=shortNameOrID, domain=domain)
    headers = {
    ...
    log.debug(
        ...
        ' "%s"' % ("https://api.mnemonic.no/users/v2/group/{shortNameOrID}?" + "&".join(["%s=%s" % (key, str(value)) for key,value in body.items()]))
...
def list_basic_groups(
    ...
    url = "https://api.mnemonic.no/users/v2/group".format(limit=limit, keywordMatch=keywordMatch, offset=offset, domain=domain, subject=subject, customer=customer, includeDeleted=includeDeleted, excludeFlag=excludeFlag, includeFlag=includeFlag, keyword=keyword, keywordField=keywordField, sortBy=sortBy)
    headers = {
        'User-Agent': 'ArgusToolbelt/',
    }
    ...
    log.debug(
        ...
        ' "%s"' % ("https://api.mnemonic.no/users/v2/group?" + "&".join(["%s=%s" % (key, str(value)) for key,value in body.items()]))
...
def search_basic_groups(
    ...
    url = "https://api.mnemonic.no/users/v2/group/search".format()
    headers = {
        'User-Agent': 'ArgusToolbelt/',
    }
    ...
    log.debug(
        ...
        "https://api.mnemonic.no/users/v2/group/search"

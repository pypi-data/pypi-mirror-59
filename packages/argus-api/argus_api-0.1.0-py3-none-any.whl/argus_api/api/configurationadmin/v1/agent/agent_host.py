...
def get_host_by_agent_user(
    ...
    url = "https://api.mnemonic.no/configurationadmin/v1/agent/{user}/host".format(user=user, domain=domain)
    headers = {
    ...
    log.debug(
        ...
        ' "%s"' % ("https://api.mnemonic.no/configurationadmin/v1/agent/{user}/host?" + "&".join(["%s=%s" % (key, str(value)) for key,value in body.items()]))

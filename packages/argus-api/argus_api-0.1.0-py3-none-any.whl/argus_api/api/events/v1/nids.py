...
def find_n_i_d_s_events(
    ...
    url = "https://api.mnemonic.no/events/v1/nids/search".format()
    headers = {
    ...
    log.debug(
        ...
        "https://api.mnemonic.no/events/v1/nids/search"
...
def list_n_i_d_s_events(
    ...
    url = "https://api.mnemonic.no/events/v1/nids".format(limit=limit, customerID=customerID, signature=signature, ip=ip, startTimestamp=startTimestamp, endTimestamp=endTimestamp, offset=offset)
    headers = {
        'User-Agent': 'ArgusToolbelt/',
    }
    ...
    log.debug(
        ...
        ' "%s"' % ("https://api.mnemonic.no/events/v1/nids?" + "&".join(["%s=%s" % (key, str(value)) for key,value in body.items()]))

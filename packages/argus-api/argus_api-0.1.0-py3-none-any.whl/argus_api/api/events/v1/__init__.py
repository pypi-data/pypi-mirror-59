...
def get_event_by_path(
    ...
    url = "https://api.mnemonic.no/events/v1/{type}/{timestamp}/{customerID}/{eventID}".format(type=type, timestamp=timestamp, customerID=customerID, eventID=eventID)
    headers = {
    ...
    log.debug(
        ...
        ' "%s"' % ("https://api.mnemonic.no/events/v1/{type}/{timestamp}/{customerID}/{eventID}?" + "&".join(["%s=%s" % (key, str(value)) for key,value in body.items()]))

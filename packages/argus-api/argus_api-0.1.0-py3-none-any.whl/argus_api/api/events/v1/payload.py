...
def get_payload(
    ...
    url = "https://api.mnemonic.no/events/v1/{type}/{timestamp}/{customerID}/{eventID}/payload".format(type=type, timestamp=timestamp, customerID=customerID, eventID=eventID)
    headers = {
    ...
    log.debug(
        ...
        ' "%s"' % ("https://api.mnemonic.no/events/v1/{type}/{timestamp}/{customerID}/{eventID}/payload?" + "&".join(["%s=%s" % (key, str(value)) for key,value in body.items()]))

...
def search_records_simplified(
    ...
    url = "https://api.mnemonic.no/pdns/v3/{query}".format(aggregate=aggregate, includeAnonymous=includeAnonymous, limit=limit, query=query, rrClass=rrClass, rrType=rrType, customerID=customerID, tlp=tlp, offset=offset)
    headers = {
    ...
    log.debug(
        ...
        ' "%s"' % ("https://api.mnemonic.no/pdns/v3/{query}?" + "&".join(["%s=%s" % (key, str(value)) for key,value in body.items()]))

...
def get_customer_root_folder(
    ...
    url = "https://api.mnemonic.no/documents/v1/customer/{customer}".format(customer=customer)
    headers = {
    ...
    log.debug(
        ...
        ' "%s"' % ("https://api.mnemonic.no/documents/v1/customer/{customer}?" + "&".join(["%s=%s" % (key, str(value)) for key,value in body.items()]))

    ...
    for path, properties in connman.GetContexts():
        ...
        if "Method" in settings4:
            ...
            if connect and s["Method"] == "static":
                ...
                subprocess.check_output(cmd)
            if result and s["Method"] == "static":
                with open("/etc/resolv.conf", 'a') as handle:
                    handle.write("\n")
                    handle.write("# DNS servers set by ofonoctl\n")
                    for ns in s["DomainNameServers"]:
                        handle.write("nameserver {}\n".format(ns))

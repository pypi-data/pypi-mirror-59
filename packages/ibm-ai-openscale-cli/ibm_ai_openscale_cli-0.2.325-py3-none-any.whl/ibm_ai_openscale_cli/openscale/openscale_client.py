...
class OpenScaleClient(OpenScaleReset):
    ...
    def _reliable_create_datamart(self):
        ...
        logger.log_timer('data_mart.setup in {:.3f} seconds'.format(elapsed))
    ...
    def bind_mlinstance(self, credentials):
        ...
        logger.log_timer('data_mart.bindings.add in {:.3f} seconds'.format(elapsed))
    ...
    def use_existing_subscription(self, asset_details_dict):
        ...
        elapsed = time.time() - start
        logger.log_timer('data_mart.subscriptions.get_details(ALL) in {:.3f} seconds'.format(elapsed))
        ...
        logger.log_timer('data_mart.subscriptions.get(ONE) in {:.3f} seconds'.format(elapsed))
    ...
    def subscribe_to_model_deployment(self, asset_details_dict):
        ...
        logger.log_timer('data_mart.subscriptions.add in {:.3f} seconds'.format(elapsed))
    ...
    def configure_subscription(self):
        ...
        elapsed = time.time() - start
        logger.log_timer('subscription.payload_logging.enable in {:.3f} seconds'.format(elapsed))
        ...
        elapsed = time.time() - start
        logger.log_timer('subscription.performance_monitoring.enable in {:.3f} seconds'.format(elapsed))
    ...
    def configure_fairness(self, params):
        if params:
            ...
            elapsed = time.time() - start
            logger.log_timer('subscription.fairness_monitoring.enable in {:.3f} seconds'.format(elapsed))
    ...
    def configure_quality(self, params):
        if params:
            ...
            elapsed = time.time() - start
            logger.log_timer('subscription.quality_monitoring.enable in {:.3f} seconds'.format(elapsed))
    ...
    def configure_explainability(self):
        ...
        elapsed = time.time() - start
        logger.log_timer('subscription.explainability.enable in {:.3f} seconds'.format(elapsed))
    ...
    def configure_drift(self, params):
        if params:
            ...
            elapsed = time.time() - start
            logger.log_timer('subscription.drift_monitoring.enable in {:.3f} seconds'.format(elapsed))
    ...
    def _reliable_define_bkpi_application(self, params):
        ...
        elapsed = time.time() - start
        logger.log_timer('define business application in {:.3f} seconds'.format(elapsed))
    ...
    def _reliable_store_explanation(self, record):
        ...
        logger.log_timer('explanation stored in {:.3f} seconds'.format(elapsed))
    ...
    def _reliable_upload_feedback(self, feedback_data):
        ...
        elapsed = time.time() - start
        logger.log_timer('subscription.feedback_logging.store in {:.3f} seconds'.format(elapsed))
    ...
    def _reliable_single_score(self, engine_client, deployment_url, score_input, score_num, totalelapsed, firststart, lastend, tokenizer=None):
        ...
        logger.log_timer('WML deployments.score in {:.3f} seconds, total requests: {}, duration: {:.3f} seconds, elapsed: {:.3f} seconds'.format(elapsed, score_num+1, duration, totalelapsed))
    ...
    def generate_sample_scoring(self, engine_client, numscores, values_per_score, to_init_payload_logging=False):
        ...
        if self._args.ml_engine_type is MLEngineType.WML:
            ...
            for _ in range(numscores):
                ...
                elapsed = time.time() - start
                logger.log_timer('AzureML score in {:.3f} seconds'.format(elapsed))
            ...
            for _ in range(numscores):
                ...
                elapsed = time.time() - start
                logger.log_timer('SPSS score in {:.3f} seconds'.format(elapsed))
                records_list.append(record)
        elif self._args.ml_engine_type is MLEngineType.CUSTOM:
            engine_client.setup_scoring_metadata(self._subscription)
            self._model.expected_payload_row_count += numscores
            for _ in range(numscores):
                ...
                elapsed = time.time() - start
                logger.log_timer('Custom score in {:.3f} seconds'.format(elapsed))
                records_list.append(record)
        elif self._args.ml_engine_type is MLEngineType.SAGEMAKER:
            records_list = []
            engine_client.setup_scoring_metadata(self._subscription)
            ...
            for _ in range(numscores):
                ...
                elapsed = time.time() - start
                logger.log_timer('Sagemaker score in {:.3f} seconds'.format(elapsed))
                records_list.append(record)
        if records_list:
            start = time.time()
            self._subscription.payload_logging.store(records=records_list)
            elapsed = time.time() - start
            logger.log_timer('subscription.payload_logging.store in {:.3f} seconds'.format(elapsed))
    ...
    def _reliable_trigger_fairness_check(self, background_mode):
        if 'fairness_configuration' not in self._model.configuration_data:
            ...
            try:
                ...
                elapsed = time.time() - start
                logger.log_timer('subscription.fairness_monitoring.run in {:.3f} seconds'.format(elapsed))
    ...
    def _reliable_trigger_quality_check(self, background_mode):
        if 'quality_configuration' not in self._model.configuration_data:
            ...
            try:
                ...
                elapsed = time.time() - start
                logger.log_timer('subscription.quality_monitoring.run in {:.3f} seconds'.format(elapsed))
    ...
    def _reliable_trigger_drift_check(self, background_mode):
        if 'drift_configuration' not in self._model.configuration_data:
            ...
            try:
                ...
                elapsed = time.time() - start
                logger.log_timer('subscription.drift_monitoring.run in {:.3f} seconds'.format(elapsed))
    ...
    def _reliable_trigger_bkpi_correlation_monitor(self):
        ...
        elapsed = time.time() - start
        logger.log_timer('List all monitors in {:.3f} seconds'.format(elapsed))
        ...
        elapsed = time.time() - start
        logger.log_timer('post bkpi_monitoring.run in {:.3f} seconds'.format(elapsed))
    ...
    def _reliable_get_correlation_results(self, correlations_instance_id, correlation_run_id):
        ...
        elapsed = time.time() - start
        logger.log_timer('get bkpi_monitoring.run in {:.3f} seconds'.format(elapsed))
        return (response)
    ...
    def _reliable_post_metrics(self, metric_type, records):
        ...
        elapsed = time.time() - start
        logger.log_timer('post data_mart {} metrics in {:.3f} seconds'.format(metric_type, elapsed))
    ...
    def _reliable_post_debiased_payloads(self, records):
        ...
        elapsed = time.time() - start
        logger.log_timer('post data_mart debiased payloads history in {:.3f} seconds'.format(elapsed))
        words = ['error', 'exception']
        if any(word in str(response.json()) for word in words):
    ...
    def _reliable_post_manual_labeling(self, records):
        ...
        elapsed = time.time() - start
        logger.log_timer('post data_mart manual_labeling history in {:.3f} seconds'.format(elapsed))
        words = ['error', 'exception']
        if any(word in str(response.json()) for word in words):
    ...
    def _reliable_store_bkpi_data_set(self, records, ds_type):
        ...
        elapsed = time.time() - start
        logger.log_timer('Get business data sets in {:.3f} seconds'.format(elapsed))
        if response.status_code != 200:
        ...
        elapsed = time.time() - start
        logger.log_timer('Post {} history in {:.3f} seconds'.format(context, elapsed))
        if response.status_code != 202:
    ...
    def _reliable_store_bkpi_metrics(self, records):
        ...
        elapsed = time.time() - start
        logger.log_timer('Get business application in {:.3f} seconds'.format(elapsed))
        if response.status_code != 200:
        ...
        elapsed = time.time() - start
        logger.log_timer('Post business metrics history in {:.3f} seconds'.format(elapsed))
        if response.status_code != 202:
    ...
    def _reliable_store_bkpi_drift_metrics(self, records):
        ...
        elapsed = time.time() - start
        logger.log_timer('Post business drift metrics history in {:.3f} seconds'.format(elapsed))
        if response.status_code != 202:
    ...
    def _reliable_store_quality_monitor_metrics(self, records):
        ...
        elapsed = time.time() - start
        logger.log_timer('post data_mart quality_monitor metrics in {:.3f} seconds'.format(elapsed))
    ...
    def _reliable_generate_drift_metrics(self, start_time, end_time, windows=8): # eight 3-hour drift windows in 24 hours
        ...
        elapsed = time.time() - start
        logger.log_timer('generate drift metrics in {:.3f} seconds'.format(elapsed))
    ...
    def _reliable_post_drift_metrics(self, url, json, context):
        ...
        elapsed = time.time() - start
        logger.log_timer('posted {} in {:.3f} seconds'.format(context, elapsed))
        words = ['error', 'exception']
    ...
    def _reliable_update_bkpi_subscription(self):
        ...
        subscription_details = self._subscription.get_details()
        elapsed = time.time() - start
        logger.log_timer('get subscription details in {:.3f} seconds'.format(elapsed))
        ...
        elapsed = time.time() - start
        logger.log_timer('updated subscription with bkpi transaction_id in {:.3f} seconds'.format(elapsed))
    ...
    def generate_explain_requests(self):
        ...
        try:
            ...
            logger.log_timer('Found {} available score(s) for explain, in {:.3f} seconds'.format(len(scoring_ids), elapsed))

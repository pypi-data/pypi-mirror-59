...
function main(project) {
    ...
    const $datable_object = $(`div.project-${project} #ant_datatable`);
    if ($datable_object.length) {
        ...
        const columns_left = [
            ...
            // actions
            {
                ...
                "render": function (data, type, full, meta) {
                    ...
                    // }
                    // if ($('body').hasClass(`template-${project}-sample-view`) &&
                    ...
                    //     // todo: fix me.
                    //     if ("data['photo'][0]") {
                    //         action.push(`<a class="_Result" href="./${project}-sample-photo-download?uuid=` +
        ...
        $('#dataBtnGroup a[data-target],' +
            ...
            .on('click confirm.toolbar', null, function (event) {
                ...
                switch (target_data) {
                    // sample
                    case 'sample-view':
                        view = `${project}-sample-view`;
                        break;
                    case 'sample-import':
                        view = `${project}-sample-import`;
                        break;
                    case 'sample-single-add':
                        view = `${project}-single-sample-addform`;
                        break;
                    case 'sample-add':
                        view = `${project}-sample-addform`;
                        break;
                    case 'sample-edit':
                        if (uuid_length === 0) {
                            return 0;
                        }
                        view = `${project}-sample-editform`;
                        break;

                    // extraction
                    case 'extraction-view':
                        view = `${project}-extraction-view`;
                        break;
                    case 'extraction-import':
                        view = `${project}-extraction-import`;
                        break;
                    case 'extraction-add':
                        view = `${project}-extraction-addform`;
                        break;
                    case 'extraction-edit':
                        if (uuid_length === 0) {
                            return 0;
                        }
                        view = `${project}-extraction-editform`;
                        break;

                    // pcr
                    case 'pcr-view':
                        view = `${project}-pcr-view`;
                        break;
                    case 'pcr-import':
                        view = `${project}-pcr-import`;
                        break;
                    case 'pcr-add':
                        view = `${project}-pcr-addform`;
                        break;
                    case 'pcr-edit':
                        if (uuid_length === 0) {
                            return 0;
                        }
                        view = `${project}-pcr-editform`;
                        break;

                    // qc
                    case 'qc-view':
                        view = `${project}-qc-view`;
                        break;
                    case 'qc-import':
                        view = `${project}-qc-import`;
                        break;
                    case 'qc-add':
                        view = `${project}-qc-addform`;
                        break;
                    case 'qc-edit':
                        if (uuid_length === 0) {
                            return 0;
                        }
                        view = `${project}-qc-editform`;
                        break;

                    // detection
                    case 'detection-view':
                        view = `${project}-detection-view`;
                        break;
                    case 'detection-import':
                        view = `${project}-detection-import`;
                        break;
                    case 'detection-add':
                        view = `${project}-detection-addform`;
                        break;
                    case 'detection-edit':
                        if (uuid_length === 0) {
                            return 0;
                        }
                        view = `${project}-detection-editform`;
                        break;

                    // result
                    case 'result-view':
                        view = `${project}-result-view`;
                        break;
                    case 'result-import':
                        view = `${project}-result-import`;
                        break;
                    case 'result-add':
                        view = `${project}-result-addform`;
                        break;
                    case 'result-edit':
                        if (uuid_length === 0) {
                            return 0;
                        }
                        view = `${project}-result-editform`;
                        break;
                    ...
                    case 'change-steps':
                        ...
                        view = `${project}-change-steps-form`;
                        break;
                    case 'state-transition':
                        if (uuid_length === 0) {
                            return 0;
                        }
                        view = `${project}-state-transition-form`;
                        break;
                    // Mark failed redo
                    case 'failed-redo':
                        if (uuid_length === 0) {
                    ...
                    case 'export-dilution':
                        ...
                        view = `${project}-export-dilution`;
                        break;
                    case 'backup':
                        view = `${project}-backup-view`;
                        break;
                    case 'recovery':
                        view = `${project}-recovery-view`;
                        break;
                    default:
                        view = '.';
                        break;
        ...
        if ($upload_dialog.length) {
            ...
            $button_start
                .on('click', function () {
                    const view = `./${project}-sample-photo-upload`;

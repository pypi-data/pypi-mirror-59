...
def import_auth():
 try:
  ...
  raise vIiBa('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=vIiBr)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=vIiBP)
def get_params(args,names=[],mandatory=vIiBr):
 result=()
 for name in names:
  value=vIiBE
  if name in args:
  ...
  if not value and mandatory:
   raise vIiBa('Please provide %s=... parameter'%name)
  result+=(value)
 if vIiBG(result)==1:
  return result[0]
 return result

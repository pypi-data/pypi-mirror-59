...
def import_auth():
 try:
  ...
  raise XegtY('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=XegtC)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=XegtS)
def get_params(args,names=[],mandatory=XegtC):
 result=()
 for name in names:
  value=Xegts
  if name in args:
  ...
  if not value and mandatory:
   raise XegtY('Please provide %s=... parameter'%name)
  result+=(value)
 if XegtM(result)==1:
  return result[0]
 return result

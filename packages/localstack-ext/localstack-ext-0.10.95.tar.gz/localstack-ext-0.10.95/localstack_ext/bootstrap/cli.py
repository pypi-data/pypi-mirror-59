...
def import_auth():
 try:
  ...
  raise iTuXJ('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=iTuXM)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=iTuXw)
def get_params(args,names=[],mandatory=iTuXM):
 result=()
 for name in names:
  value=iTuXE
  if name in args:
  ...
  if not value and mandatory:
   raise iTuXJ('Please provide %s=... parameter'%name)
  result+=(value)
 if iTuXr(result)==1:
  return result[0]
 return result

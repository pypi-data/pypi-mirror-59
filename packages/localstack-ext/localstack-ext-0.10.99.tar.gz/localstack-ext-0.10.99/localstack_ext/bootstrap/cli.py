...
def import_auth():
 try:
  ...
  raise SvVwk('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=SvVwQ)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=SvVwt)
def get_params(args,names=[],mandatory=SvVwQ):
 result=()
 for name in names:
  value=SvVwp
  if name in args:
  ...
  if not value and mandatory:
   raise SvVwk('Please provide %s=... parameter'%name)
  result+=(value)
 if SvVwE(result)==1:
  return result[0]
 return result

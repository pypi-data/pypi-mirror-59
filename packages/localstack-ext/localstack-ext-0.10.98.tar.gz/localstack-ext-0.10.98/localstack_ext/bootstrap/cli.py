...
def import_auth():
 try:
  ...
  raise BPiHI('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=BPiHM)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=BPiHJ)
def get_params(args,names=[],mandatory=BPiHM):
 result=()
 for name in names:
  value=BPiHx
  if name in args:
  ...
  if not value and mandatory:
   raise BPiHI('Please provide %s=... parameter'%name)
  result+=(value)
 if BPiHt(result)==1:
  return result[0]
 return result

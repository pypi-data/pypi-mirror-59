...
def import_auth():
 try:
  ...
  raise NWqVX('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=NWqVS)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=NWqVj)
def get_params(args,names=[],mandatory=NWqVS):
 result=()
 for name in names:
  value=NWqVc
  if name in args:
  ...
  if not value and mandatory:
   raise NWqVX('Please provide %s=... parameter'%name)
  result+=(value)
 if NWqVR(result)==1:
  return result[0]
 return result

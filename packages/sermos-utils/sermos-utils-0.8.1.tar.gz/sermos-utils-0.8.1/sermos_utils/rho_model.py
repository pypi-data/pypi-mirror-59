...
from sermos_utils.constants import DEFAULT_STORE_MODEL_URL, S3_MODEL_BUCKET, DEFAULT_GET_MODEL_URL
...
def get_stored_model_json(model_name: str,
    ...
    # }
    # todo: the api currently only takes exact model versions instead of patterns
    logger.debug("Requesting storage info from Admin API...")
    model_key = model_name + '_' + version_pattern
    request_url = urljoin(get_model_endpoint, quote(model_key, safe=''))
    r = requests.get(url=request_url, headers=headers)
    # r = requests.get(
    #     url=get_model_endpoint, headers=headers, params=query_params)
    response_data = r.json()
    client = get_s3_client_from_api_response(response_data=response_data)
...
def rho_model_loader(model_name: str,
                     ...
                     save_to_local_disk: bool = True) -> Type[RhoModel]:
    ...
    if not model:
        if not deploy_key:
            deploy_key = os.environ['SERMOS_DEPLOY_KEY']
        stored_bytes = get_stored_model_json(model_name=model_name,
                                             version_pattern=version_pattern,

    ...
    for astr in astrs:
        if not astr.startswith('@'):
            ...
            with open(astr[1:], 'r') as filep:
                data = yaml.load(filep, Loader=yaml.Loader)
            ...
            for key, vals in data.items():
                if isinstance(vals, list):
                    for val in vals:
                        arg_list += [
                            '--{}'.format(key), val if isinstance(val, str) else yaml.dump(val)
                        ]
                else:
...
def make_config_parser(sys_args: List[str]) -> argparse.Namespace:
    ...
    for (full_target_name, desc), config_group in APPEND_GROUP.items():
        if full_target_name not in CONFIG_GROUP:
            raise RuntimeError('Target config to be append is not registered: %s' %
                               full_target_name)

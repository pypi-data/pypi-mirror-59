...
class BargainRewardList(ListView):
    model = BargainReward
    template_name = 'bee_django_crm/gift/reward/list.html'
    context_object_name = 'reward_list'
    paginate_by = 20
    queryset = None


class BargainRewardDetail(DetailView):
    model = BargainReward
    ...
    #
    # def get_context_data(self, **kwargs):
    #     context = super(BargainRewardDetail, self).get_context_data(**kwargs)
    #     record_list = CampaignRecord.objects.get(id=self.kwargs["pk"])
        #         bind_user_record_list = record_list.objects(is_mk=True,bind_user__isnull=False)
        #         bind_preuser_record_list = record_list.objects(is_mk=False,bind_preuser__isnull=False)
        #         context["record_count"] = record_list.count()
        #         context["bind_user_record_count"] = bind_user_record_list.count()
        #         context["bind_preuser_record_count"] = bind_preuser_record_list.count()
        # return context
...
class CampaignRecordList(ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super(CampaignRecordList, self).get_context_data(**kwargs)
        q=self.get_queryset()
        result_dict = q.aggregate(result_sum=Sum('result'))
        result_sum = 0
        if result_dict.has_key("result_sum"):
            if result_dict["result_sum"] > 0:
                result_sum = result_dict["result_sum"]
        context["result_sum"] = result_sum
        return context

class CustomRewardDetail(DetailView):
    model = BargainReward
    template_name = 'bee_django_crm/gift/reward/custom_detail.html'
    context_object_name = 'reward'

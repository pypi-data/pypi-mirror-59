...
class ARIMA(_ARIMABase):#pylint: disable=too-many-instance-attributes
    ...
    def fit(self, data, endog=None, exog=None):#pylint: disable=too-many-arguments,too-many-branches
        ...
        endog : str, optional
            ...
            The endogenous variable, i.e. time series. Valid only for ARIMAX; endog cannot be the first column's name (ID column).

            Defaults to the second column.

        exog : list of str, optional

            An optional array of exogenous variables. Valid only for ARIMAX; exog cannot be the ID column's name and the name of endog column.
        ...
        if exog is not None and non_exog is None:
            data_ = data[[ID] + [endog] + exog]
        if exog is not None and non_exog is not None:
            data_ = data[[ID] + [endog] + exog + non_exog]
        if exog is None and non_exog is not None:
            data_ = data[[ID] + [endog] + non_exog]

...
class DISC:
    """
        ...
        transfer_learning : bool, optional, default: False
    ...
    def __init__(self, gene_name, norm_max, depth="16_8_1", repeats=3, dimension_number=512, compress_dimensions=50,
                 transfer_learning=False, noise_intensity=0.1, dropout_rate=0.5, output_scale_factor=2,
                 z_score_library_size_factor=1000000, en_de_act_fn=tf.nn.tanh, output_activation_function=tf.nn.sigmoid,
                 log_fn=print):
        self.log_fn = log_fn
    ...
    def _expression_predictor(self):
        ...
        if self.transfer_learning:
            self.predictor_weights_list = [tf.stop_gradient(x) for x in self.predictor_weights_list]
            self.predictor_bias_list = [tf.stop_gradient(x) for x in self.predictor_bias_list]

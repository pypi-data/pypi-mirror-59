"""Pyscdl allows you to download music from Soundcloud
Usage:
    pyscdl -l <track_url> [-a | -f | -C | -t | -p][-c][-o <offset>]\
...
def get_config():
    ...
    config.read(os.path.join(os.path.expanduser('~'), '.config/pyscdl/pyscdl.cfg'))
    try:
        token = config['scdl']['auth_token']
        path = config['scdl']['path']
    except:
        logger.error('Are you sure pyscdl.cfg is in $HOME/.config/pyscdl/ ?')
        logger.error('Are both "auth_token" and "path" defined there?')
        sys.exit()
    if os.path.exists(path):
        os.chdir(path)
    else:
        logger.error('Invalid path in pyscdl.cfg...')
        sys.exit()
...
def get_filename(track, original_filename=None):
    ...
    if arguments['--addtimestamp']:
        # created_at sample: 2020-01-07T11:11:37Z
        ts = datetime \

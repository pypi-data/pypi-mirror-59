...
class Flasher(threading.Thread):
    ...
    def run(self):
        ...
        GLib.idle_add(self.callback, ['Pivoting to temporary rootfs', 0.0])
    ...
    def pivot_temp_root(self):
        ...
        subprocess.run(['mount', '-t', 'tmpfs', 'none', '/tmp/tmproot'])
        for name in ['proc', 'sys', 'dev', 'run', 'usr', 'var', 'tmp', 'oldroot']:
            os.mkdir('/var/tmproot/{}'.format(name))
        for name in ['/bin', '/etc', '/sbin', '/lib']:
            subprocess.run(['cp', '-ax', name, '/tmp/tmproot/'])
        for name in ['bin', 'sbin', 'lib']:
            subprocess.run(['cp', '-ax', '/usr/{}'.format(name), '/tmp/tmproot/usr/'])
        for name in ['empty', 'lib', 'local', 'locks', 'opt', 'run', 'spool', 'tmp']:
            subprocess.run(['cp', '-ax', '/lib/{}'.format(name), '/tmp/tmproot/lib/'])

        # Pivot root
        subprocess.run(['mount', '--make-rprivate', '/'])
        subprocess.run(['pivot_root', '/tmp/tmproot', '/tmp/tmproot/oldroot'])
        for name in ['dev', 'proc', 'sys', 'run']:
            subprocess.run(['mount', '--move', '/oldroot/{}'.format(name), '/{}'.format(name)])

        # Unmount old rootfs
        subprocess.run(['umount', '/oldroot'])

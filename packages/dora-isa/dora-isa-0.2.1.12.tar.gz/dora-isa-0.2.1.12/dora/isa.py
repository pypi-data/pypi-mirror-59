...
class ISAContext:
    ...
    r_table = r"""(CREATE[\s]*TABLE[\s]*([a-zA-Z0-9_.]+)[\s]+AS)[\s]+((\Z|.|\s)*)"""
    ...
    def sql(self, query, **kwargs):
        ...
        try:
            match_table = re.match(self.r_table, query, re.MULTILINE | re.IGNORECASE)
            if match_table:
                ddl = match_table[2].replace(match_table[3],match_table[3].upper())
                sql = match_table[4]
                tbl = f"""'dora'='0.2.0', 'createdBy'='{DORA_USER}', 'context'='{USER_DB}, 'server'='{DORA_HOST}'"""
                query = f"""{ddl} USING PARQUET TBLPROPERTIES ({tbl}) AS ({sql})"""

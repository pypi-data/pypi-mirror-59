...
class AssistantClient:
    ...
    def init_logging(self):
        ...
        if handler == "FILE":
            print("Output redirected to the file : '{0}'".format(log_file))
            self.log_type = "json"
        ...
        #self.info('A info message')
        #self.warning('A warning message')

    def get_json_log_as_str(self, level, message):
        log = {'message' : message,
               'level' : level,
    ...
    def debug(self, message):
        if self.log_type == "json":
            logging.debug(self.get_json_log_as_str('DEBUG', message))
        else:
            logging.debug(message)

    def info(self, message):
        if self.log_type == "json":
            logging.info(self.get_json_log_as_str('INFO', message))
        else:
            logging.info(message)

    def warning(self, message):
        if self.log_type == "json":
            logging.warning(self.get_json_log_as_str('WARNING', message))
        else:
            logging.warning(message)

    def error(self, message):
        if self.log_type == "json":
            logging.error(self.get_json_log_as_str('ERROR', message))
        else:
            logging.error(message)



    def build_server_informations(self, address, role):
        """
        ...
        for intf in interfaces():
            for addr in ifaddresses(intf)[AF_INET]:   # TODO : upgrade here to handle ipv6
                ...
                if not ip.startswith("127.0"):
                    self.info("Main ip is : {0}".format(ip))
                    return ip
        return None
    ...
    def find_free_port(self):
        ...
        with closing(socket.socket(socket.AF_INET, socket.SOCK_STREAM)) as s:
            ...
            self.info("Free port found : '{0}'".format(s.getsockname()))
    ...
    def generate_selfsigned_cert(self, cert_file, key_file, hostname, public_ip, private_ip):
        ...
        try:
            ...
            if os.path.isfile(cert_file) and os.path.isfile(key_file):
                self.info("SSL : certificate and key files are already generated :)")
                return

            # 2. if not, generate the data
            self.info("SSL : no certificate and key file are present. Generating them...")
            ...
            self.info("SSL : files generated!")
        except:
            self.error("SSL : Unable to generate the certificate and key files. The error is : {0}".format(traceback.format_exc()))

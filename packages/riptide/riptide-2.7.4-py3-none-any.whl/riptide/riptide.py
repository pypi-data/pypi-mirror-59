...
def save_riptide_output(riptide_obj='NULL', path='NULL', file_type='SBML'):
	...
	if path == 'NULL':
		raise ValueError('ERROR: Did not provide an output directory')
	else:
		try:
			os.mkdir(path)
		except:
			raise OSError('ERROR: Invalid output path provided')
	...
	if riptide_obj.flux_samples != 'Not performed':
	...
	riptide_obj.minimization_coefficients.to_csv(outFile, sep='\t')
	outFile = path + '/flux_maximization_coefficients.tsv'
	riptide_obj.maximization_coefficients.to_csv(outFile, sep='\t')
	...
	with open(outFile, 'w') as parameters:
		...
		parameters.write('Fraction of genes found in model: ' + str(riptide_obj.percent_of_mapping) + '\n')
		...
		# Results
		parameters.write('Fraction of pruning reactions: ' + str(riptide_obj.additional_parameters['operation']['reactions']) + '%\n')
		parameters.write('Fraction of pruning metabolites: ' + str(riptide_obj.additional_parameters['operation']['metabolites']) + '%\n')
		if riptide_object.additional_parameters['operation']['obj_change'] != 'fails':
			parameters.write('Fraction of change to objective value: ' + str(riptide_obj.additional_parameters['operation']['obj_change']) + '%\n')

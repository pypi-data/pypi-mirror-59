...
def collect_reports(xnat_instance, experiments,
        validator='ArchivingValidator', version=['toto']):
    import json
    ...
    for e in tqdm(experiments):
        try:
            exp = xnat_instance.select.experiment(e['ID'])
...
def validation_scores(x, validator, version, id):
    from bx import xnat
    import pandas as pd

    experiments = xnat.collect_experiments(x, id, max_rows=25)

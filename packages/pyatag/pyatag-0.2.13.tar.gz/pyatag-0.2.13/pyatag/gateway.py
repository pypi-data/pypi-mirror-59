...
class AtagDataStore:
    ...
    def burner_status(self):
        ...
        if isinstance(self.sensordata.get(BOILER_STATUS), list):
            if self.sensordata[BOILER_STATUS][0] == 1:
                return (True, self.sensordata.get(FLAME))
        return (False, 0)
    ...
    async def set_hvac_mode(self, mode: str) -> bool:
        ...
        if mode == self.hvac_mode:
            return True
        try:
            if await self.async_set_atag(ch_control_mode=CH_CONTROLS[mode]):
            ...
            _LOGGER.error("Mode should be one of heat or auto, received %s", mode)
            return False

    @property
    def temperature(self):
        """Return current CV temperature"""
        return self.sensordata.get(CH_ROOM_TEMP)

    @property
    def target_temperature(self):
        """Return target CV temperature"""
        if (
            self._last_api_call[1] == CH_TEMPERATURE
            and self.sensordata.get(REPORT_TIME) < self._last_api_call[0]
        ):
    ...
    async def set_temp(self, target: float):
        ...
        try:
            ...
            _LOGGER.error("Failed to set temperature to %s", target)
            _LOGGER.debug("Message: %s", err)
            return False

    @property
    def dhw_temperature(self):
        """Return current dhw temperature"""
        return self.sensordata.get(DHW_CURRENT_TEMPERATURE)

    @property
    def dhw_min_temp(self):
        """Return dhw min temperature"""
        return self.sensordata.get(DHW_MIN)

    @property
    def dhw_max_temp(self):
        """Return dhw max temperature"""
        return self.sensordata.get(DHW_MAX)
    ...
    def dhw_target_temperature(self):
        ...
        if self.dhw_status:
            return self.sensordata.get(DHW_TEMPERATURE)
        if (
            self._last_api_call[1] == DHW_MODE_TEMPERATURE
            and self.sensordata.get(REPORT_TIME) < self._last_api_call[0]
        ):
            return self._dhw_target_temperature
        return self.sensordata.get(DHW_MODE_TEMPERATURE) % 150

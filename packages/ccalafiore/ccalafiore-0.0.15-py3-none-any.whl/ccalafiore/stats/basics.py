...
def scores_to_means(scores, axis_samples, keepdims=False):
    ...
    if scores.dtype == object:
        ...
        for indexes_object_i in indexes_object:
            ...
            means[indexes_means_tuple] = sum_of_scores / n_samples
        ...
        sum_of_scores = np.nansum(scores, axis=axis_samples, keepdims=keepdims)
        n_samples = np.sum(np.logical_not(np.isnan(scores)), axis=axis_samples, keepdims=keepdims)

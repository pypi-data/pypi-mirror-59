...
def scores_raw_to_paired_t_test(
    ...
    for i in range(n_df):
        ...
        if tails == '2':
            t_critical[tuple(indexes_df[i])] = t.ppf((1 + confidence) / 2., df[tuple(indexes_df[i])])
            p_values[tuple(indexes_df[i])] = (1.0 - t.cdf(abs(t_values[tuple(indexes_df[i])]),
                                                          df[tuple(indexes_df[i])])) * 2
        elif tails == '1l':
            t_critical[tuple(indexes_df[i])] = -t.ppf(confidence, df[tuple(indexes_df[i])])
            p_values[tuple(indexes_df[i])] = t.cdf(t_values[tuple(indexes_df[i])], df[tuple(indexes_df[i])])
        elif tails == '1r':
            t_critical[tuple(indexes_df[i])] = t.ppf(confidence, df[tuple(indexes_df[i])])
            p_values[tuple(indexes_df[i])] = 1.0 - t.cdf(t_values[tuple(indexes_df[i])], df[tuple(indexes_df[i])])
...
def scores_formatted_to_paired_t_test(
    ...
    for i in range(n_df):
        ...
        if tails == '2':
            t_critical[tuple(indexes_df[i])] = t.ppf((1 + confidence) / 2., df[tuple(indexes_df[i])])
            p_values[tuple(indexes_df[i])] = (1.0 - t.cdf(abs(t_values[tuple(indexes_df[i])]), df[tuple(indexes_df[i])])) * 2
        elif tails == '1l':
            t_critical[tuple(indexes_df[i])] = -t.ppf(confidence, df[tuple(indexes_df[i])])
            p_values[tuple(indexes_df[i])] = t.cdf(t_values[tuple(indexes_df[i])], df[tuple(indexes_df[i])])
        elif tails == '1r':

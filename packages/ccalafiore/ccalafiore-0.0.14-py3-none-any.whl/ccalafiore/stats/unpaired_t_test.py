...
def scores_raw_to_scores_formatted(scores_raw, axis=0, dtype=None):
...
def scores_to_df_for_unpaired_t_test(
    ...
    variances = cc_stats_basics.scores_to_variances(scores, axis_samples=axis_samples, keepdims=keepdims)
    if not(keepdims):
...
def scores_to_unpaired_t_test(
    ...
    means_of_scores = cc_stats_basics.scores_to_means(scores, axis_samples=axis_samples, keepdims=keepdims)
    ...
    diff_of_means = cc_stats_basics.scores_to_diff_of_scores(
        means_of_scores, axis_comparisons=axis_independent_variables)

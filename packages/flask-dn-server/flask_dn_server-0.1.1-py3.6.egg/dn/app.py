...
class DNApp(DNEnv):
    ...
    def run(self, server='gevent', host="0.0.0.0", port=5000):
        if server == "gevent":
            from gevent import monkey
            monkey.patch_all()
        if server == "simple":
            self.simple_wsgiapp(self.flaskapp, host, port)
        elif server == "gevent":
            self.gevent_wsgiapp(self.flaskapp, host, port)

    def simple_wsgiapp(self, app, host, port):
        from werkzeug.serving import run_simple
        options = {}
        options.setdefault('use_reloader', True)
        options.setdefault('use_debugger', True)
        try:
            run_simple(hostname=host, port=port, application=app, **options)
        finally:
            pass

    def gevent_wsgiapp(self, app, host, port):
        from gevent import pywsgi, local
        import threading
        if not isinstance(threading.local(), local.local):
            msg = "requires gevent.monkey.patch_all() (before import)"
            raise RuntimeError(msg)
        address = (host, port)
        server = pywsgi.WSGIServer(address, app, log=None)

...
class DataMixin(object):
    ...
    def to_pandas(self, keep_column_indexes=[], chinese_column_name=False):
        ...
        if chinese_column_name is True and 'cname' in self.meta.keys():
            df = pd.DataFrame(data=data, columns=self.cname)
            for index, type in enumerate(self.column_types):
                if type == 'Date' or type=='datetime':
                    df[self.cname[index]] = df[self.cname[index]].apply(lambda x: pd.to_datetime(x,errors='coerce'))            

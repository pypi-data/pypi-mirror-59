...
class GenericObject:
    ...
    def _generate_default(self, key):
        try:
            ...
            if isinstance(val, GenericObject):
                val._set_internal('_default_factory', self._default_factory)
        except TypeError:
            val = self._default_factory
    ...
    def __getattribute__(self, key):
        if key in object.__getattribute__(self, '_items_'):
            return object.__getattribute__(self, '_items_')[key]
    ...
    def __getattr__(self, key):
        ...
        #         and _attr_is_function(self.__class__, key[1:])
        # ):
        #     return getattr(self, key[1:])
        if key in object.__getattribute__(self, '_items_'):
            return object.__getattribute__(self, '_items_')[key]
        if key not in self._restricted_keys:
            return self._generate_default(key)
        return None

...
class UnescapedTemplateFileExtensionsVisitor(FlaskBaseVisitor):
    ...
    def visit_Call(self, call_node: ast.Call):
        ...
        unsafe_extensions_message = f"{self.name} Flask does not autoescape templates with the {str(extensions_to_go_in_message)} extension by default. Flask only autoescapes .html, .htm, .xml, and .xhtml files. Make sure your variables are escaped using `flask.Markup.escape`."

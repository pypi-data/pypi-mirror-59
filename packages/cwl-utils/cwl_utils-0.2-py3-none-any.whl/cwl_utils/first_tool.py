...
class Input(Savable):
    ...
    @classmethod
    def fromDoc(cls, doc, baseuri, loadingOptions, docRoot=None):
        # type: (Any, Text, LoadingOptions, Optional[Text]) -> Input
        ...
        errors = []
        try:
            message = load_field(_doc.get(
                'message'), strtype, baseuri, loadingOptions)
        except ValidationException as e:
            errors.append(SourceLine(_doc, 'message', str).makeError(
                "the `message` field is not valid because:\n"+str(e)))
        ...
        for k in _doc.keys():
            if k not in cls.attrs:
                if ":" in k:
                    ...
                    errors.append(SourceLine(_doc, k, str).makeError(
                        "invalid field `%s`, expected one of: `message`" % (k)))
                    break

        if errors:
            raise ValidationException("Trying 'Input'\n"+"\n".join(errors))
        loadingOptions = copy.deepcopy(loadingOptions)
        loadingOptions.original_doc = _doc
        return cls(message, extension_fields=extension_fields, loadingOptions=loadingOptions)
...
_vocab = {
    ...
    "Input": "file:///home/michael/schema_salad/1st_tool.ss.yml#Input",
...
_rvocab = {
    ...
    "file:///home/michael/schema_salad/1st_tool.ss.yml#Input": "Input",
...
InputLoader = _RecordLoader(Input)
...
union_of_InputLoader = _UnionLoader((InputLoader,))
array_of_union_of_InputLoader = _ArrayLoader(union_of_InputLoader)
union_of_InputLoader_or_array_of_union_of_InputLoader = _UnionLoader((InputLoader, array_of_union_of_InputLoader,))
...
def load_document(doc, baseuri=None, loadingOptions=None):
    ...
    if loadingOptions is None:
        loadingOptions = LoadingOptions()
    return _document_load(union_of_InputLoader_or_array_of_union_of_InputLoader, doc, baseuri, loadingOptions)
...
def load_document_by_string(string, uri, loadingOptions=None):
    ...
    return _document_load(union_of_InputLoader_or_array_of_union_of_InputLoader, result, uri, loadingOptions)

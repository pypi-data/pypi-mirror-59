...
try:
  from PIL import ImageTk, ImageOps,Image, ImageGrab
except:
...
# 新建屏幕保存方法,保存的是截取的图形
def TurtleScreen_save(self,filename,full=False):
    """ 截屏,filename主图像文件名,推荐用png,full表示是否全屏截取"""
    if full == True:
       im = ImageGrab.grab()
    else:
       c = Turtle._screen._root._canvas  # 获取画布对象
       x0 = c.winfo_rootx()              # 画布左上角x坐标+5
       y0 = c.winfo_rooty()              # 画布左上角y坐标+5
       width = c.winfo_width()
...
def mouse_pos():
    ...
    print(_root.winfo_pointerx(),_root.winfo_pointery())

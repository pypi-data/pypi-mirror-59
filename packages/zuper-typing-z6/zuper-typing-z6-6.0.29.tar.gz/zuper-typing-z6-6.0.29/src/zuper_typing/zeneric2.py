...
def make_type(
    ...
    # assert_equivalent_types(cls, cls)

    if not symbols:
        clsi = get_dataclass_info(cls)
        key = tuple(bindings.items())
    ...
    try:
        ...
        # assert_equivalent_types(res, res)
        # if res.__name__ == "Entity[MyState2]":
        #     clsi = get_dataclass_info(res)
        # if len(clsi.extra) > 0:
        #     raise ZException(cls=cls, bindings=bindings, res=res)
...
def make_type_1(cls, bindings0, symbols):
    from .assorted_recursive_type_subst import recursive_type_subst

    # T: TypeLike, f: Callable[[TypeLike], TypeLike]
    assert len(bindings0) == 1
    k, v = list(bindings0.items())[0]
    ...
    return recursive_type_subst(cls, f)
...
def make_type_(
    ...
    # if len(bindings0) == 1:
    #     v = list(bindings0.values())[0]
    #     if is_TypeVar(v):
    #         return make_type_1(cls, bindings0, symbols)
    ...
    logger.info(
        "make type", cls=cls.__name__, bindings=bindings, symbols=symbols, stacklevel=4
    )

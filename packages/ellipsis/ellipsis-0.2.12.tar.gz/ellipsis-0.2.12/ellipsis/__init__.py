...
def geometryAlter(mapId, polygonId, newProperties, newLayerName, token):
    
    keys = newProperties.keys()
    for key in keys:
        newProperties[key] = str(newProperties[key])
    
    r = s.post(url + '/geometry/alter', headers = {"Authorization":token},
                     json = {"mapId":  mapId, 'polygonId':polygonId, 'newLayerName':newLayerName, 'newProperties':newProperties})
    if int(str(r).split('[')[1].split(']')[0]) != 200:
        raise ValueError(r.text)

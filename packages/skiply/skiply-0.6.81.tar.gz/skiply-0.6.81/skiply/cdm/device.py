    ...
    try:
        if default_monitoring:
            ...
            if res_notin is not None and len(res_notin)>0:
                ...
                results = session.query(Device).filter(Device.device_status == 'IN-SERVICE', Device.entity_id.notin_(ids)).filter(Device.device_last_transmission != None, Device.device_last_transmission > deltatime_start, Device.device_last_transmission <= deltatime_end).all()
                print(results)
                ...
                try :
                    ...
                    power_results = skiply.cdm.powerSupply.get_powerSupply_with_level_under(power_threshold, power_threshold_percent)
                    print("power results : {}".format(power_results))

                    if power_results is not None and len(power_results) > 0 :
                        try:
                            device_skiply_ids = []
                            for res in power_results:
                                device_skiply_ids.append(res.device_skiply_id)

                            results_2 = session.query(Device).filter(Device.device_status == 'IN-SERVICE', Device.device_skiply_id.in_(device_skiply_ids)).all()
                            print(results_2)

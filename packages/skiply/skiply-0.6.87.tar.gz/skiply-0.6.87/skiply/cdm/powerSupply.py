...
def get_powerSupply_with_level_under(hours_start, power_threshold, power_threshold_percent):
    deltatime_start = datetime.now() - timedelta(hours=hours_start)
    
    session = db_session()
    try:
        results = session.query(func.max(PowerSupply.power_time), PowerSupply.power_voltage, PowerSupply.power_voltage_idle, PowerSupply.power_voltage_percentage, PowerSupply.device_id, PowerSupply.device_skiply_id).group_by(PowerSupply.device_skiply_id).having(and_(func.max(PowerSupply.power_time) >= deltatime_start, or_(and_(PowerSupply.power_voltage <= power_threshold, PowerSupply.power_voltage > 0), and_(PowerSupply.power_voltage_percentage <= power_threshold_percent, PowerSupply.power_voltage_percentage > 0)))).all()
    except:
        print("DB Request get_powerSupply(powerSupply_id) Failed")
        results = None
    finally:

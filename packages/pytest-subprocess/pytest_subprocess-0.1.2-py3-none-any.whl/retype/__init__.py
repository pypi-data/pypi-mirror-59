...
def maybe_replace_any_if_equal(name, expected, actual, flags):
    ...
    if not is_equal:
        expected_annotation = ast.parse(minimize_whitespace(str(expected)))
        actual_annotation = ast.parse(minimize_whitespace(str(actual)))
        if expected_annotation.replace(' ', '') != actual_annotation.replace(' ', ''):
            raise ValueError(

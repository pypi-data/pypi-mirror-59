...
def extract_edges(image, σ=1, check_σ=False, bounds=None):
    '''
    ...
    if check_σ:
        ...
        if bounds is not None:
            ax[0].set_xlim(bounds[0:2])
            ax[0].set_ylim(bounds[-1:-3:-1])

        filter_slider = widgets.Slider(ax[1], r'$\log_{10}\sigma$', -1, 1,
                                       valinit=np.log10(σ), color='gray')

        def update(val):
            edges = feature.canny(image, sigma=np.power(10, val))
            ax[0].imshow(edges, cmap='gray_r', vmin=0, vmax=1)
            fig.canvas.draw_idle()

        filter_slider.on_changed(update)
        print('Waiting for your input, please select a desired filter value '
              'and press enter when done')
        while plt.waitforbuttonpress(0) == False:
            pass
        σ = np.power(10, filter_slider.val)

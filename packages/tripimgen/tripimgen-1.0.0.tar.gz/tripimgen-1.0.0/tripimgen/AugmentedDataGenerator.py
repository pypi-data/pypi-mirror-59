...
from .options import OUTPUT_TYPE


class AugmentedDataGenerator(Sequence):

    def __init__(self, x_set, y_set, dgen_opts, batch_size=128,
                 num_classes=None, output_type: str or tuple=OUTPUT_TYPE.CATEGORICAL):
        ...
        self.datagen = ImageDataGenerator(**dgen_opts)
    ...
    def __get_categorical_batch__(self):
        ...
        a_instances = [self.datagen.random_transform(
            self.x[self.indices[i][j]]) for i, j in zip(a_cls, a_smp)]
        p_instances = [self.datagen.random_transform(
            self.x[self.indices[i][j]]) for i, j in zip(p_cls, p_smp)]
        n_instances = [self.datagen.random_transform(
            self.x[self.indices[i][j]]) for i, j in zip(n_cls, n_smp)]
        ...
        return [np.stack(a_instances), np.stack(p_instances), np.stack(n_instances)],\
            np.stack((a_classes, p_classes, n_classes), axis=1)
...
if __name__ == "__main__":
    ...
    datagen = AugmentedDataGenerator(x_train, y_train, dgen_opts)

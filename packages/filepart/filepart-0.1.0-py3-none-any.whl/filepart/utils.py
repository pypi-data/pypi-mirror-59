...
Usage
    ...
    filepart -b [-c] FILE [-o FOLDER] file [FILE ...]"""
...
\n\noptions:
    ...
    -c, --check CHECKSUM    Check if the file was built correctly via the original file's
                            checksum
...
def parse_size(size):
    ...
    if size_int < 1:
        raise exceptions.SizeError("You must give a bigger part size.")
    ...
    if size_int < 2 and size_unit == "part":
        raise exceptions.SizeError("You must split by at least 2.")

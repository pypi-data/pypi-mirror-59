...
from isc_common.common import getOrElse, blinkString
            ...
            with transaction.atomic():
                ...
                for item_ref in Item_refs.objects.get_descendants(id=demand.precent_item.item.id, where_clause='where is_bit_on(props::integer, 0) = true'):
                    ...
                    try:
                        ...
                        cnt_not += 1
                        notes.append(f'Наименование: {getOrElse("", STMP_1)}, Обозначение: {getOrElse("", STMP_2)} ID={item_ref.child.id} не входит в детализацию.')
                    ...
                    if section and section != 'Документация':
                        if operations_cnt == 0:
                            if IsBitOn(props, 5):
                                cnt_not += 1
                                notes.append(f'Наименование: {getOrElse("", STMP_1)}, Обозначение: {getOrElse("", STMP_2)} ID={item_ref.child.id} не указаны операции.')
                        else:

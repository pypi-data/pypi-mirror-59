        ...
        with transaction.atomic():
            for i in range(0, len(ids), 2):
                ...
                if visibleMode != "none":
                    ...
                    progress = Progress(qty=Uploades_documents.objects.filter(upload_id=id).count(), user=user, title='Удаление закачек')
                    try:
                        lotsman_cnt = 0
                        doc_cnt = 0
                        ...
                        for upload_document in Uploades_documents.objects.filter(upload_id=id):
                            ...
                            for lotsman_document in Lotsman_documents_hierarcy.objects.filter(document=upload_document.document):
                                ...
                                Item.objects.filter(lotsman_document=lotsman_document).delete()

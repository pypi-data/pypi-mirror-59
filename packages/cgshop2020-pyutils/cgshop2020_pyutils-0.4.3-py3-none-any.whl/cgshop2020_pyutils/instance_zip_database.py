...
class InstanceZipDatabase(InstanceBaseDatabase):
    ...
    def __iter__(self) -> Instance:
        ...
        for file_data in self._zipfile.filelist:
            if self._filename_fits_instance_convention(file_data.filename) and not self._is_hidden_folder(file_data.filename):
                instance_name = self._extract_instance_name_from_path(file_data.filename)
                try:
                    yield self._cache[instance_name]
                except KeyError:            
                    yield self._cache_and_return(self._reader.from_json_string(self._zipfile.open(file_data.filename)))
    ...
    def __getitem__(self, name: str) -> Instance:
        ...
        try:
            ...
            return self._cache_and_return(self._reader.from_json_string(self._zipfile.open(path)))

...
def init():
    ...
    if conf.DATABASE_DRIVER == "postgres":
        filename = f"{conf.CONFIG_BASEPATH}/init.pgsql"
    elif "pymssql" in conf.DATABASE_DRIVER:
        filename = f"{conf.CONFIG_BASEPATH}/init.sql"
    else:
    ...
    contents = contents.format(TABLE_NAME=conf.FRAC_SCHEDULE_TABLE_NAME)
...
def recreate(ctx):
    ...
    db_url = conf.SQLALCHEMY_DATABASE_URI
    engine = sqlalchemy.create_engine(db_url)
    engine.execute(f"drop view if exists frac_schedules_most_recent_by_api10;")

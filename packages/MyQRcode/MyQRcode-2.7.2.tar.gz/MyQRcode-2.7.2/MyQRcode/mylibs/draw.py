...
def draw_qrcode(abspath, qrmatrix, qrcolor):
    ...
    bg = Image.open(urllib.request.urlopen('http://placehold.it/200x200'))
    coord = a, b = 50, 50
    print(bg.getpixel(coord));

    for line in qrmatrix:
        for module in line:
            if module:
                draw_a_black_unit(pic, x, y, unit_len, qrcolor)
            x += unit_len
...
def draw_a_black_unit(p, x, y, ul, color):
    for i in range(ul):
        for j in range(ul):
            coord = a, b = x+i, y+j
            # pixcolor = background.getpixel(coord)

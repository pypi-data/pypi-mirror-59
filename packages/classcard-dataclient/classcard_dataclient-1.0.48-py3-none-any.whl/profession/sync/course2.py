...
from classcard_dataclient.models.course import CourseV2, CourseTableManagerV2
from classcard_dataclient.models.schedule import RestSchedule, RestTableV2, PeriodSet
...
from config import ACADEMIC_YEAR, TERM

logger = logging.getLogger(__name__)


class CourseSyncV1(BaseSync2):
    ...
    def sync(self):
        ...
        end_datetime = begin_datetime + datetime.timedelta(days=30)
        ...
        self.related_member()
        course_table = CourseTableManagerV2(name=response['semester'], number=get_md5_hash(response['semester'])[:20],
                                            rest_name=rest_table.name, semester_name=semester.name,
                                            begin_date=begin_date, end_date=end_date, walking_mode=1)

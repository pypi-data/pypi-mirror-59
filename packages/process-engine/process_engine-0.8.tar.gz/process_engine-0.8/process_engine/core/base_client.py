...
class BaseClient:
    ...
    def __init__(self, url, session=None, identity=None):
        ...
        if session is None:
            read_timeout = 0 # see https://docs.aiohttp.org/en/stable/client_reference.html 100 * 60
            self._session = aiohttp.ClientSession(read_timeout=read_timeout)
        else:
    ...
    async def __aexit__(self, exception_type, exception_value, traceback):
        await self.close() #TODO: funktioniert leider noch nicht korrekt
    ...
    async def do_post(self, path, payload, options={}):
        ...
        async with self._session.post(request_url, json=payload, headers=headers) as response:

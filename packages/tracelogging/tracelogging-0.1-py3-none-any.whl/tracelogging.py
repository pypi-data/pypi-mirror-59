...
class EventMetadata(object):
    ...
    def pack(self, *args, **kwargs) -> bytes:
        # TODO: support positional args with kwargs
        buf = b''
        for idx, a in enumerate(args):
            buf += self.fields[idx].pack(a)
...
class SelfDescribingEvent(object):
    def __init__(self, Id,  Version, Level, Channel, Opcode, Keywords, Task):
        self.metadata = None
        self.descriptor = EVENT_DESCRIPTOR(Id=Id, Version=Version,
                                           Level=Level.value, Channel=Channel,
                                           Opcode=Opcode, Keywords=Keywords, Task=Task)
...
class EventDispatcher(object):
    ...
    # TODO: somehow get the provider name/traits, ref to instance? maybe just use 'otherself'
    def __call__(self, otherself, *args, **kwargs):
        # using otherself
        l.info('passing event data')
        otherself._write(self.event, args, kwargs)
        # TODO: is calling actual body of func warranted?
...
class EventDefinition(object):
    ...
    def __call__(self, target):
        # todo: build event descriptor here according to func
        # TODO: use event descriptor, use signature of target
...
def event(Name=None, Id=0,   Version=0, Level: TraceLevel = TraceLevel.Information, Channel=0, Opcode=0, Keywords=0, Task=0):
    # id 0 is used to tell the metaclass to generate an event by itself.
    e = SelfDescribingEvent(Id, Version, Level,
                            Channel, Opcode, Keywords, Task)
...
class ProviderDefinition(ABCMeta):
    ...
    def __setup_provider_fields(name, dct):
        ...
        dct['_handle'] = None
        # TODO: should map eventname_id_version to each dispatcher
        dct['_events'] = {}
    ...
    def __collect_events(meta, dct, inherited_events):
        ...
        for event_name, event_dispatcher in dct.items():
            if isinstance(event_dispatcher, EventDispatcher):
                ...
                inherited_events[event_name] = event_dispatcher.event.metadata.name
...
class Provider(metaclass=ProviderDefinition):
    ...
    def __init__(self, throw_on_write_error=False):
        ...
        # TODO: figure out what's going on with the unkillable thread?
        EventSetInformation(

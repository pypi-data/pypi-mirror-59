...
def plotobj(inputfile, loutfactor=1):
    ...
    step = int(len(vertex)*loutfactor)
    ax.scatter(0, 0, 0, color='black')                                    # plot origin
    ax.plot([0, 1 * np.max(vertex[:, 0])], [0, 0], [0, 0], color='red')   # plot X axis
    ax.plot([0, 0], [0, 1 * np.max(vertex[:, 1])], [0, 0], color='green')   # plot Y axis
    ax.plot([0, 0], [0, 0], [0, 1 * np.max(vertex[:, 2])], color='blue')   # plot Z axis
    ax.scatter(vertex[0::step, 0], vertex[0::step, 1], vertex[0::step, 2], marker='.', color='orange')
    print('plotted vertex number: ', len(vertex[0::int(len(vertex)*loutfactor), 0]))
    plt.xlabel("x position [m]")
    plt.ylabel("y position [m]")
    plt.show()
    return
# plotobj('/Users/aescalante/spice/missions/vex/venus-express/kernels/dsk/vex.obj', loutfactor=0.0005)

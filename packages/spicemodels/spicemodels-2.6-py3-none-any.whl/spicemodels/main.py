...
def plotobj(inputfile, loutfactor=1):
    f = open(inputfile)
    lines = f.readlines()
    vertex = []
    for line in lines:
        if 'v ' in line[0:2]:
            ...
            vertex.append(v)
    print('vertex read successfully')
    vertex = np.asarray(vertex)
    fig = plt.figure(figsize=(6, 6))
    ax = fig.add_subplot(111, projection='3d')
    step = 1 * loutfactor
    ax.scatter(0, 0, 0, color='black')                                    # plot origin
    ax.plot([0, 1 * np.max(vertex)], [0, 0], [0, 0], color='red')   # plot X axis
    ax.plot([0, 0], [0, 1 * np.max(vertex)], [0, 0], color='green')   # plot Y axis
    ax.plot([0, 0], [0, 0], [0, 1 * np.max(vertex)], color='blue')   # plot Z axis
    ax.scatter(vertex[0::step, 0], vertex[0::step, 1], vertex[0::step, 2], marker='.', color='orange')
    ax.set_xlim3d(-np.max(vertex), np.max(vertex))
    ax.set_ylim3d(-np.max(vertex), np.max(vertex))
    ax.set_zlim3d(-np.max(vertex), np.max(vertex))
    print('plotted vertex number: ', len(vertex[0::step, 0]), 'of: ', len(vertex))
    plt.xlabel("x position [m]")
    plt.ylabel("y position [m]")
    plt.show()
    return
# plotobj('/Users/aescalante/spice/missions/vex/venus-express/kernels/dsk/vex.obj', loutfactor=100)
...
def obj2dsk():
    ...
    subprocess.call('mkdsk -setup setup.txt')

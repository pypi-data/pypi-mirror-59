...
class TokenizerFactory:
    ...
    def _init() -> None:
        if not TokenizerFactory.INITIALIZED:
            ...
            for _, mod, is_package in pkgutil.walk_packages([package_dir], 'pangeamt_nlp.tokenizer.'):
                # Import module
                mods = mod.split('.')
                module = mods[-1]
                ...
                for _, cls in inspect.getmembers(imported_module, inspect.isclass):
                    if (issubclass(cls, TokenizerBase) and cls != TokenizerBase):
                        ...
                        if not hasattr(cls, TokenizerFactory.NAME):
                            raise ValueError(f'Tokenizer `{cls}` has not attribute {TokenizerFactory.NAME}.')
                        ...
                        if not hasattr(cls, TokenizerFactory.LANGS):
                            raise ValueError(f'Tokenizer `{cls}` has not attribute {TokenizerFactory.LANGS}.')

                        # Avoid duplicate
                        if name in TokenizerFactory.TOKENIZERS:
                            raise ValueError(f'Tokenizer `{cls}` {TokenizerFactory.NAME} attribute already exists')
                        TokenizerFactory.TOKENIZERS[name] = cls
        TokenizerFactory.INITIALIZED = True

    def new(self, lang, tokenizer_name, *argv, **kwargs):
        if not TokenizerFactory.INITIALIZED:

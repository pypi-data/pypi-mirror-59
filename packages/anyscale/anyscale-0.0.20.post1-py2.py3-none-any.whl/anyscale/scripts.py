...
SYNCTHING_HOME = os.path.expanduser("~/.anyscale/syncthing")
SYNCTHING_CONFIG_PATH = os.path.join(SYNCTHING_HOME, "config.xml")


def get_syncthing_executable():
    current_dir = os.path.dirname(os.path.realpath(__file__))
    if sys.platform.startswith("linux"):
        return os.path.join(current_dir, "syncthing-linux")
    elif sys.platform.startswith("darwin"):
        return os.path.join(current_dir, "syncthing-macos")
    else:
...
def autosync_add_or_update_project_folder(
    project_name: str, project_folder_path: str):
    import xml.etree.ElementTree
    config = xml.etree.ElementTree.parse(SYNCTHING_CONFIG_PATH)
    root = config.getroot()
    ...
    if not project_folder_element:
        ...
        project_folder_element.set("label", "Project Folder for {}".format(project_name))
        project_folder_element.set("path", project_folder_path)
        root.append(project_folder_element)
    else:
        project_folder_element.set("path", project_folder_path)
    config.write(SYNCTHING_CONFIG_PATH)


def autosync_add_device(project_name: str, device_id: str):
    import xml.etree.ElementTree
    config = xml.etree.ElementTree.parse(SYNCTHING_CONFIG_PATH)
    root = config.getroot()
    project_folder_element = None
    ...
    project_folder_element.append(device_element)
    device_element2 = xml.etree.ElementTree.SubElement(
        root, "device", {
...
def session_setup_autosync(session_id):
    ...
    syncthing_executable = get_syncthing_executable()

    assert subprocess.check_call([
        syncthing_executable, "-generate", SYNCTHING_HOME]) == 0

    # Set syncthing folder to the project directory.
    autosync_add_or_update_project_folder(
        project_definition.config["name"],
        os.path.expanduser("~/" + project_definition.config["name"]))

    device_id = subprocess.check_output([
        syncthing_executable, "-home", SYNCTHING_HOME, "-device-id"]).decode().strip()
    ...
    syncthing = subprocess.Popen([
        syncthing_executable, "-home", SYNCTHING_HOME])
    syncthing.communicate()


@session_cli.command(
    name="autosync_add_device",
    help="Add device to autosync config.",
    hidden=True)
@click.argument("device_id", type=str, required=True)
def session_autosync_add_device(device_id):
    project_definition = load_project_or_throw()

    autosync_add_device(project_definition.config["name"], device_id)
...
def session_autosync(name):
    ...
    # Make sure syncthing is initialized.
    syncthing_executable = get_syncthing_executable()
    assert subprocess.check_call([
        syncthing_executable, "-generate", SYNCTHING_HOME]) == 0

...
def number_min(schema_name, table_name, column_name, table, column):
...
def number_max(schema_name, table_name, column_name, table, column):
    return basic_function_and_update(schema_name,
                                     table_name,
                                     column_name,
                                     'max',
...
def number_mean(schema_name, table_name, column_name, table, column):
    return basic_function_and_update(schema_name,
                                     table_name,
                                     column_name,
                                     'avg',
...
def number_median(schema_name, table_name, column_name, table, column):
    return basic_function_and_update(schema_name,
                                     table_name,
                                     column_name,
                                     'median',
...
def number_std(schema_name, table_name, column_name, table, column):
...
def number_zero(schema_name, table_name, column_name, table, column):
...
def number_quantile(schema_name, table_name, column_name, p, table, column):
    ...
    calculating_string += f"nearest_rank :=ceil({p}*num_rows); \n" + \
                          ...
                          f"quantile_value := i.{column_name}; \nexit; \nend if;\n" + \
                          "current_row := current_row +1; \n" + \
                          "end loop; \n"

    update_string = update_table(schema_name, table_name, column_name, table, column, 'quantile_value')
...
def number_range(schema_name, table_name, column_name, table, column):
    declare_string = declare_variables([{'name': 'range_value', 'data_type': 'number'}])

    select_into_string = f"select max({column_name}) - min({column_name})" + into(schema_name, table_name,
                                                                                  'range_value')

    update_string = update_table(schema_name, table_name, column_name, table, column, 'range_value')
...
def number_iqr(schema_name, table_name, column_name, table, column):
    p1 = 0.25
    p3 = 0.75
    ...
    calculating_string += f"q1_nearest_rank :=ceil({p1}*num_rows); \n" + \
                          ...
                          f"q1_value := i.{column_name}; \n" + \
                          "end if;\n" + \
                          "if current_row = q3_nearest_rank then \n" + \
                          f"q3_value := i.{column_name}; \n" + \
                          "end if ;\n" + \
                          "current_row := current_row +1; \n" + \
                          "end loop;\n" + \
                          "iqr := q3_value - q1_value;\n"

    update_string = update_table(schema_name, table_name, column_name, table, column, 'iqr')
...
def number_cov(schema_name, table_name, column_name, table, column):
    declare_string = declare_variables([{'name': 'cov', 'data_type': 'number'},
                                        {'name': 'mu', 'data_type': 'number'}])

    select_string = f"select avg({column_name}) " + into(schema_name, table_name, 'mu')
    select_string += """if mu != 0 then \n"""
    select_string += f"select sqrt(variance({column_name}))/avg({column_name}) " + into(schema_name, table_name, 'cov')
    select_string += "end if; \n"
    update_string = update_table(schema_name, table_name, column_name, table, column, 'cov')
    sql_string = declare_string + begin_and_end(select_string + update_string)

    return sql_string


def number_sum(schema_name, table_name, column_name, table, column):
    return basic_function_and_update(schema_name,
                                     table_name,
                                     column_name,
                                     'sum',
...
def number_variance(schema_name, table_name, column_name, table, column):
    return basic_function_and_update(schema_name,
                                     table_name,
                                     column_name,
                                     'variance',
...
def number_mad(schema_name, table_name, column_name, table, column):
    ...
    update_string = update_table(schema_name, table_name, column_name, table, column, 'mad')
...
def number_kurtosis(schema_name, table_name, column_name, table, column):
    ...
    second_moment_string = f"select sum(power({column_name}-mu, 2))/num_rows " + into(schema_name, table_name,
                                                                                      'second_moment')
    fourth_moment_string = f"select sum(power({column_name}-mu,4))/num_rows " + into(schema_name, table_name,
                                                                                     'fourth_moment')

    kurt_string = "if second_moment != 0 then \n"
    kurt_string += "kurt := (fourth_moment / power(second_moment,2)) -3;\nend if; \n"

    update_string = update_table(schema_name, table_name, column_name, table, column, 'kurt')
...
def number_skewness(schema_name, table_name, column_name, table, column):
    declare_string = declare_variables([{'name': 'num_rows', 'data_type': 'integer'},
                                        {'name': 'mu', 'data_type': 'number'},
    ...
    num_rows_string = standard_select(column_name, 'count') + into(schema_name, table_name, 'num_rows')
    mu_string = standard_select(column_name, 'avg') + into(schema_name, table_name, 'mu')
    third_moment_string = f"select sum(power({column_name}-mu, 3))/num_rows " + into(schema_name, table_name,
                                                                                     'third_moment')
    sigma_string = chained_funcs_select(column_name, ['sqrt', 'variance']) + into(schema_name, table_name, 'sigma')
    skewness_string = "if sigma != 0 then \n"
    skewness_string += "skewness:= third_moment/power(sigma,3); \n end if; \n "

    update_string = update_table(schema_name, table_name, column_name, table, column, 'skewness')

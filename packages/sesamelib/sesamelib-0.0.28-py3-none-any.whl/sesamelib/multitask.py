...
class Track:
    ...
    def add(self, point):
        ...
        lat = point.y
        if lat is not None and (lat > self.lat_max or lat < self.lat_min):
            # TODO raise
            raise AreaError(f"Latitude {lat} out of bounds {self.lat_max, self.lat_min}")

        lon = point.x
        if lon is not None and (lon > self.lon_max or lon < self.lon_min):
            raise AreaError(f"Longitude {lon} out of bounds {self.lon_max, self.lon_min}")

        sog = point.sog
        if sog is not None and sog > self.max_speed:

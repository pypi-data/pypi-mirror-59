...
class TopicDocument(Worksheet):
    ...
    def render_template(self, text=None, filename=None, **kwargs):
        ...
        def _display(display, date_range, *, size='medium', shape='rectangle', custom_width=1024, custom_height=768):
        ...
        def _image(filename=None, *, buffer=None, image_format=None, just_src=False):
            if not os.path.isabs(filename):
                filename = os.path.join(folder_for_relative_paths, filename)
            return self.document.add_image(filename=filename, buffer=buffer, image_format=image_format,
                                           just_src=just_src)

...
def get_dmatrix_templates(timeseries_list, firings_list):
    ...
    if max(Kmaxes) > 0:
        ...
        dmatrix = np.full((use_max, use_max, num_combos), np.nan)
        k1_dmatrix = np.full_like(dmatrix)
        k2_dmatrix = np.full_like(dmatrix)
        templates = np.full((M, CLIP_SIZE, use_max, 2 * num_combos), np.nan)

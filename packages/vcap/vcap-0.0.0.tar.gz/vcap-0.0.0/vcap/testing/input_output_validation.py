...
import cv2
import numpy as np
from vcap.tests.conftest import verify_all_threads_closed
...
def _run_inference_on_images(images: List[np.ndarray], capsule: BaseCapsule):
    ...
    with ThreadPoolExecutor(thread_name_prefix="InputOutputValidation ") as executor:
        for stream_id, image in enumerate(images):
            case: DETECTION_NODE_TYPE = []
...
def _test_capsule_input_output(capsule, packaged_capsule_path, image_paths):
    ...
    try:
        ...
        for _ in range(NUM_BATCH_CASES):
            images = [random.choice(loaded_images) for _ in range(NUM_STREAMS)]
...
def perform_capsule_tests(unpackaged_capsule_dir, image_paths):
    ...
    with mock.patch.object(BaseCapsule, '__del__') as patched_del:
        ...
        _test_capsule_input_output(capsule, packaged_capsule_path, image_paths)

...
from vcap.loading.errors import (
...
SEMVER_PATTERN = r"^([0-9]+)\.([0-9]+)$"


def load_capsule(path: Path, key=None, inference_mode=True) \
        -> BaseCapsule:
    ...
    with ZipFile(file_like, "r") as capsule_file:
        ...
        match = re.fullmatch(SEMVER_PATTERN, compatibility_version)
        if match is None:
...
def _validate_capsule(capsule: BaseCapsule):
    ...
    def check_argnames(func, correct, ignore=None):
        ...
        >>>     pass
        >>> check_argnames(my_func, ['self'], ['frame', 'detection_node'])
        True
        """
        ignore = [] if ignore is None else ignore
        all_var_names = func.__code__.co_varnames
        argnames = all_var_names[:func.__code__.co_argcount]
        filtered = [n for n in argnames if n not in ignore]
        return filtered == correct

    try:
        if re.match(r"^\w*$", capsule.name) is None:
            raise InvalidCapsuleError(
        ...
        if not all(capsule_assertions):
            raise InvalidCapsuleError(
                "The capsule has an invalid internal configuration!" +
                " Capsule Assertions: " + str(capsule_assertions))
        ...
        loader_assertions = [
            callable(loader),
            check_argnames(func=loader,
                           correct=["capsule_files", "device"])]
        if not all(loader_assertions):
            raise InvalidCapsuleError(
                "The capsule's backend_loader has an invalid configuration!"
                "\nLoader Assertions: " + str(loader_assertions))
        ...
        if capsule.backends is not None:
            ...
            if not all(backend_assertions):
                raise InvalidCapsuleError(
                    "The capsule's backend has an invalid configuration!"
                    "\nBackend Assertions: " + str(backend_assertions))
        ...
        if not all(stream_state_assertions):
            raise InvalidCapsuleError(
                ...
                f"\nStream State Assertions: {stream_state_assertions}")
        ...
        if capsule.capability.encoded:
            if "recognition_threshold" not in capsule.options.keys():
                raise InvalidCapsuleError(
                    "This capsule can encode, but doesn't have a option called "
                    "\"recognition_threshold\"!")
    except InvalidCapsuleError:
        raise
    except AttributeError as e:
        raise InvalidCapsuleError(
            "The capsule did not describe the necessary attributes.")
    except Exception as e:
        raise InvalidCapsuleError(
            "This capsule is invalid for unknown reasons. Error: ", e)

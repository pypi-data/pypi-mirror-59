...
if [ -a files/apikeys/customsearchapikey.txt ]; then
	echo "customsearch = yes" >> config.cfg
else
	echo "customsearch = no" >> config.cfg
fi
...
python3.5m git2gender.py https://github.com/davidam/orgguide-es.git --directory="/tmp/clonedir" > files/tests/git2gender1-$(date "+%Y-%m-%d-%H").txt
...
cd ../..
python3.5m mail2gender.py http://mail-archives.apache.org/mod_mbox/httpd-announce/ > files/tests/mail2gender-$(date "+%Y-%m-%d-%H").txt
...
python3.5m api2gender.py David --api="genderize" > files/tests/api2genderDavidgenderize-$(date "+%Y-%m-%d-%H").txt
...
python3.5m downloadjson.py --api=genderize --csv=files/names/min.csv
...
if [ -a files/apikeys/namsorpass.txt ]; then
    python3.5m api2gender.py Leticia --surname="Martin" --api="namsor" > files/tests/api2genderLeticianamsor-$(date "+%Y-%m-%d-%H").txt
    ...
    python3.5m downloadjson.py --api=namsor --csv=files/names/min.csv
...
if [ -a files/apikeys/genderapipass.txt ]; then
    ...
    python3.5m downloadjson.py --api=genderapi --csv=files/names/min.csv
    ...
    python3.5m api2gender.py Inés --api="genderapi" > files/tests/api2genderInésgenderapi-$(date "+%Y-%m-%d-%H").txt
    ...
    python3.5m accuracy.py --api="genderapi" --csv="files/names/min.csv" > files/tests/accuracygenderapi-$(date "+%Y-%m-%d-%H").txt

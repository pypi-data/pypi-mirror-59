...
class NVME(object):
    ...
    def upgrade_fw(self, fw_path, device_index, slot):
        ...
        charPointer = bytes(fw_path, "gbk")
        char_ret = self.__dll.upgrade_fw(int(device_index), int(slot), charPointer)
    ...
    def list_dev(self):
        ...
        for index in range(devs.number):
            dev = {"index": devs.index_list[index].index, "name": devs.name_list[index].name}
            dev_list.append(dev)
            print(devs.name_list[index].name)
            print(devs.index_list[index].index)
        return dev_list

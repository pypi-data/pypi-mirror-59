...
from utils.system import ROOT_PATH
...
class Malloc(object):
    ...
    def __init__(self, length=1, types=c_uint8):
        path = os.path.realpath(os.path.join(ROOT_PATH, "Lib", "driver", "dll", "buf.so"))
    ...
    def fill_uint32_seq(self, value=0, start=0, size=sys.maxsize, step=1):
        """Fill data with sequence uint32"""
        memory, offset, length = self.translate(start, size, c_uint32)
        self.m_dll.fill_uint32_seq(memory, offset, length, value, step)

    def fill_uint32_fix(self, value=0, start=0, size=sys.maxsize):
        """Fill data with fixed uint32"""
        memory, offset, length = self.translate(start, size, c_uint32)
        self.m_dll.fill_uint32_fix(memory, offset, length, value)

    def fill_uint32_rnd(self, seed=None, start=0, size=sys.maxsize):
        """Fill data with random uint32"""
        memory, offset, length = self.translate(start, size, c_uint32)
        if seed is not None:
            self.m_dll.fill_uint32_rnd(memory, offset, length, seed, True)
        else:
            self.m_dll.fill_uint32_rnd(memory, offset, length, 0, False)

    def fill_uint8_fix(self, value=0, start=0, size=sys.maxsize):
        """Fill data with fixed uint8"""
        memory, offset, length = self.translate(start, size, c_uint8)
        self.m_dll.fill_uint8_fix(memory, offset, length, value)

    def compare(self, array, src_start=0, src_size=sys.maxsize, dst_start=None, dst_size=None):
        dst_start = dst_start if dst_start is not None else src_start
        dst_size = dst_size if dst_size is not None else src_size

        mem_a, off_a, len_a = self.translate(src_start, src_size, c_uint8)
        mem_b, off_b, len_b = array.translate(dst_start, dst_size, c_uint8)
        length = min(len_a, len_b)
        return self.m_dll.compare(mem_a, mem_b, off_a, off_b, length)
    ...
    def look_idx_uint32(self, value):
        """Look index uint32"""
        memory, _, length = self.translate(0, self.m_size, c_uint32)
        return self.m_dll.look_idx_uint32(memory, length, value)

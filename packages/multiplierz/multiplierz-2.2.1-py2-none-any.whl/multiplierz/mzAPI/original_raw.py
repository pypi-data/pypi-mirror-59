...
class mzFile(mzAPImzFile):
    ...
    def average_scan(self, startscan, stopscan, filter = 'Full ms'):
    ...
    def MS2toMS1(self, scan_number):
        if self._ms2toms1 == None:
            self._ms2toms1 = {}
            prev_ms1 = None
            for rt, mz, scannum, level, plrty in self.scan_info():
                if level == "MS1":
                    prev_ms1 = scannum
                elif level == "MS2":
                    self._ms2toms1[scannum] = prev_ms1
        
        return self._ms2toms1[scan_number]
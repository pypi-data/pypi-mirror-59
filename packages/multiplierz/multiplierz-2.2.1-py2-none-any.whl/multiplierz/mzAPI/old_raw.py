...
class mzFile(mzAPImzFile):
            ...
            for scan in range(first.value, last.value + 1):
                ...
                if scan_mode == 'MS1':
                    ...
                    if 'Monoisotopic M/Z:' in header_fields.value:
                        ...
                        retval = self.source.GetTrailerExtraValueForScanNum(scan, 'Monoisotopic M/Z:', mz_value)
                        if retval:
    ...
    def average_scan(self, start_scan, end_scan, filter):
        ...
        return list(zip(ms.value[0],ms.value[1]))
    ...
    def scan(self, time, centroid=False, mzScanMode = True):
        ...
        retval = self.source.GetTrailerExtraValueForScanNum(the_scan, 'Charge State:', z_value)
        if retval:
        ...
        if mzScanMode:
            ...
            return mzScan(list(zip(ms.value[0],ms.value[1])), scan_time, scan_mode, mz, z)
        else:
            return list(zip(ms.value[0],ms.value[1]))
    ...
    def lscan(self,scanNum):
        ...
        return list(zip(ms.value[0],ms.value[1],ms.value[4],ms.value[5]))
    ...
    def rscan(self,scanNum):
        ...
        if retval :
            raise IOError("Could not get label data. %s" % retval)
        return list(zip(ms.value[0],ms.value[1],ms.value[4], ms.value[5], ms.value[2]))    #,ms.value[3] - Leave out baseline
    ...
    def extra_info(self,scanNum):
        ...
        return dict(list(zip( [str(x[:-1]) for x in labels.value] , list(map(_to_float, values.value)) )))
    ...
    def scanInjectionTime(self,scanNum):
        ...
        if retval :
            raise IOError("Could not get data for scans. %s" % retval)
        vals = dict(list(zip( [str(x[:-1]) for x in labels.value] , list(map(_to_float, values.value)) )))
        try:

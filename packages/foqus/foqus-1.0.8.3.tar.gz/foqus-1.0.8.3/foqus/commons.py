...
from foqus.customers import create_or_update_user_apikey, save_data_from_cms, create_update_project, send_email_to_admin
            ...
            if response.status_code == 200:
                ...
                for payment in data_retreived['recurring_application_charges']:
                    if payment.get('status') == "active":
                        ...
                        db.update_client_payement(customer_name, customer_type, payment_client[11], payment.get('name'),
                                                  1, "paypal", float(payment.get("price")) * 11)
                        db.update_transction_id(customer_name, customer_type, payment.get("id"), payment.get('name'),
                                                payment_client[11], 'paypal')
                        create_or_update_user_apikey(user=customer_name, period_in_hours=8040)
                        db.update_url_confirmation(customer_name, customer_type, 'shopify', domain, access_token, None)
                        return True
        else:
            return True
    except Exception as e:

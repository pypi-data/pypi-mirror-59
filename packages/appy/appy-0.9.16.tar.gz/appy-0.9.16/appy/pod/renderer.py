...
class Renderer:
    ...
    def __init__(self, template, context, result, pythonWithUnoPath=None,
      ...
      html=False, forceOoCall=False, forceLoad=False, loadWait=0,
      ...
      metadata=True, pdfOptions='ExportNotes=True', deleteTempFolder=True,
      protection=False, pageStart=1, tabbedCR=False):
        ...
        # before doing other things like updating the table of contents. Despite
        # this, if you still encounter partial loading problems with
        # LibreOffice, use parameter "loadWait" to specify a number of seconds
        # to wait, after having sent to him the command to open the template,
        # and before proceeding with subsequent commands, like updating the
        # table of contents or optimizing tables.
    ...
    cloneAttributes = ('pythonWithUnoPath', 'ooServer', 'ooPort', 'html',
      'forceLoad', 'loadWait', 'raiseOnError', 'imageResolver',
      'stylesTemplate', 'optimalColumnWidths', 'distributeColumns',
      'expressionsHolders', 'protection', 'tabbedCR')
    ...
    def renderText(self, s, encoding='utf-8', tags=None):
        '''Renders the unformatted string p_s, properly taking care of "carriage
           return" chars.'''
        r = s.split('\n')
        i = len(r) - 1
        if tags:
            ...
            opening = closing = ''
        while i >= 0:
            r[i] = '<p>%s%s%s</p>' % (opening, r[i], closing)
            i -= 1
        return self.renderXhtml(''.join(r), encoding=encoding)
            ...
            try:
                ...
                try:
                    Converter(resultName, result, self.ooServer, self.ooPort,
                      ...
                      self.forceLoad, self.loadWait, self.pageStart).run()

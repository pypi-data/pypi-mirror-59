...
HELP_LOAD_WAIT = 'LibreOffice may take some time to load a document in its ' \
  'entirety. With this parameter, you may specify a number of seconds: after ' \
  'having sent the command to load the document, the converter will wait ' \
  'this number of seconds before continuing its job, to ensure LibreOffice ' \
  'has completely loaded the document before sending him other commands like ' \
  'updating the table of contents or optimizing tables.'
...
class Converter:
    ...
    def __init__(self, docPath, result, server=DEFAULT_SERVER,
                 ...
                 loadWait=0, pageStart=1, verbose=False):
        ...
        # The converter will wait this number of seconds LibreOffice to load the
    ...
    def openDocument(self):
        ...
        try:
            ...
            # If requested, wait to ensure the doc is fully loaded
            wait = self.loadWait
            if wait:
                time.sleep(wait)
                suffix = ' (after having waited %d second(s))' % wait
            else:
                suffix = ''
            self.log(' done%s.' % suffix)
...
class ConverterScript:
    ...
    def run(self):
        ...
        add('-w', '--loadWait', dest='loadWait', default=0,
            metavar='LOADWAIT', type='int', help=HELP_LOAD_WAIT)
        ...
        converter = Converter(args[0], args[1], options.server, options.port,
          ...
          pdfOptions, options.ppp, stream, options.forceLoad, options.loadWait,

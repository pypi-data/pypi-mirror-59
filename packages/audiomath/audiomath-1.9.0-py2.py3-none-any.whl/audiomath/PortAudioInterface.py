...
class Player( GenericInterface.GenericPlayer ):
	...
	def __init__( self, sound, device=None, stream=None, bufferLengthMsec=None, resample=False, verbose=None, **kwargs ):
		"""
		Args:
		...
		if not alreadyInitialized:
			if bufferLengthMsec is None: 
				pass # TODO: here we might need some nanny intervention on Mac Core Audio,
				# to ensure that *some* buffering is performed---otherwise the latency is
				# *too* low and the sound is very vulnerable to CPU resource contention.
			global DEFAULT_BUFFER_LENGTH
			if bufferLengthMsec is None: bufferLengthMsec = DEFAULT_BUFFER_LENGTH
			if DEFAULT_BUFFER_LENGTH is None: DEFAULT_BUFFER_LENGTH = bufferLengthMsec
		...
		if SINGLE_OUTPUT_STREAM:
			if not isinstance( stream, Stream ):
				...
				else: stream = Stream( device=deviceIndex, bufferLengthMsec=bufferLengthMsec )
			...
			self.__stream = stream if stream else SINGLE_OUTPUT_STREAM if alreadyInitialized else Stream( mode=mode, bufferLengthMsec=bufferLengthMsec )
			...
			self.__stream = stream if isinstance( stream, Stream ) else Stream( device=deviceIndex, mode=mode, bufferLengthMsec=bufferLengthMsec )

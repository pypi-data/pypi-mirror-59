...
class IntervalAnalysisMulti(IntervalAnalysis):
    ...
    #
    @staticmethod
    def _get_slice_levels(df: pd.DataFrame) -> List:
        assert type(df.index) is pd.MultiIndex, "Use IntervalAnalysis class for single-indexed data frames"
        slice_levels = []
    ...
    @staticmethod
    def _get_timeindex_name(df: pd.DataFrame) -> str:
        assert type(df.index) is pd.MultiIndex, "Use IntervalAnalysis class for single-indexed data frames"

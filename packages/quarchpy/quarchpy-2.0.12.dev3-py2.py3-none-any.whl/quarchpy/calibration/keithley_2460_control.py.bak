    ...
    while(True): #breaks when valid user input given
        ...
        if nice: #prep data for nice list selection,
            ...
            for option in additionalOptions:
                templist.append([option]*3)
            additionalOptions = templist
            userStr = listSelection(title=title, message=message, selectionList=deviceList, tableHeaders=["Name","Serial","IP Address"], nice=nice, indexReq=True, additionalOptions=additionalOptions)[3] #Address will allways be 3 in this format

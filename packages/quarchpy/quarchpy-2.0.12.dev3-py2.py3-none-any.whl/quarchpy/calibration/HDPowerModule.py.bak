...
class HDPowerModule (PowerModule):
    ...
    def getSwitchbox(self):
        ...
        if self.switchbox is None:
            while (True):
                switchboxAddress = userSelectDevice(scanFilterStr=["QTL2294"], message="Select a calibration switchbox.", nice=True)
    ...
    class HDCalibration (Calibration):
        ...
        def report(self,action,data):
            ...
            for thisLine in data:
                ...
                if action == "calibrate":
                    report.append([reference, ppmValue, calibratedValue, actError, (errorSign + "(" + str(absError) + str(self.units) + "," + str(relError) + "%)"), passfail(result)])
                elif action == "verify":
                    report.append([reference, ppmValue, actError, (errorSign + "(" + str(absError) + str(self.units) + "," + str(relError) + "%)"), passfail(result)])

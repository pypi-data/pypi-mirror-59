...
if __name__ == "__main__":
    ...
    parser.add_argument("--function", type=str, choices=['predict', 'predict_multiple_columns', 'compare_modesl'], help="Which function in main.py use. One predict one column, ohter more at once, the last compare models on test data")
    ...
    parser.add_argument("--n_steps_in", type=int, help="How many lagged values enter the model in one step")
    parser.add_argument("--output_shape", type=str, choices=['batch', 'one_step'], help="Whether models output is one value, or values are predicted at once")
...
def predict_multiple_columns(data=[], predicted_columns=[], freqs=[], database_deploy=0, predicts=None, models_parameters=None, data_source=None,
            csv_full_path=None, plot=None, used_models=None, date_index=None, return_all=None, datalength=None, data_transform=None, debug=None, analyzeit=None,
            optimizeit=None, repeatit=None, other_columns=None, lengths=None, remove_outliers=None, standardize=None, criterion=None, compareit=None, n_steps_in=None, output_shape=None):
    ...
    # Edid config. Highest priority has command line, then function argument, then config default
    # To edit multiple variables at once, exec and string query is used
    for i in args:
        exec(setup_query)
    ...
    for j in range(len(config.freqs)):
        ...
        for i in range(len(config.predicted_columns)):
            ...
            try:
                predictions[i] = predict(data=data, predicted_column=config.predicted_columns[i], freq=config.freqs[j])
...
def compare_models(data_all, data_length=1000, **kwargs):
        ...
        for i, j in data_all.items():
            ...
            try:
                with open(file_path, "rb") as input_file:
                    config.data = pickle.load(input_file)
                data_all[i] = data
            except Exception:
    ...
    for i, j in data_all.items():
        config.plot_name = i
        try:
            result = predictit.main.predict(data=j, return_model_criterion=1, **kwargs)
...
if __name__ == "__main__" and hasattr(par_args[0], 'function'):
    if par_args[0].function == 'predict':
        predict()

    if par_args[0].function == 'predict_multiple_columns':
        predict_multiple_columns()

    if par_args[0].function == 'compare_models':
        compare_models()

# Manual test with jupyter - Usually commented out
#predict()

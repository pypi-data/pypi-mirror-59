...
from pydcd.dcd_model import GCN

from pydcd.load_data import gen_X, gen_A
...
class DCD():
	...
	def dcd_detect_community(self, node_fname, edge_fname, node_attr, peval='N'):
		...
		model = GCN(nfeature=X.shape[1], nhidden1=self.nhidden1, nhidden2=self.nhidden2, nhidden3=self.nhidden3, nclass=self.nlabels)
		...
		partition = [set() for k in range(self.nlabels)]

...
class EcoVacsAPI:
    ...
    def __init__(self, device_id, account_id, password_hash, country, continent, verify_ssl=True):
        ...
        _LOGGER.warning("Setting up EcoVacsAPI")
    ...
    def __call_main_api(self, function, *args):
        _LOGGER.warning("calling main api {} with {}".format(function, args))
        ...
        _LOGGER.warning("got {}".format(json))
        if json['code'] == '0000':
            return json['data']
        elif json['code'] == '1005':
    ...
    def __call_user_api(self, function, args):
        _LOGGER.warning("calling user api {} with {}".format(function, args))
    ...
    def __call_portal_api(self, api, function, args, verify_ssl=True, **kwargs):      
        ...
        _LOGGER.warning("calling portal api {} function {} with {}".format(api, function, params))            
...
class VacBot():
    ...
    def _handle_life_span(self, event):
        ...
        _LOGGER.warning("*** life_span " + type + " = " + str(lifespan))
    ...
    def _handle_clean_report(self, event):
        ...
        if self.fan_speed:
            _LOGGER.warning("*** clean_status = " + self.clean_status + " fan_speed = " + self.fan_speed)
        else:
            _LOGGER.warning("*** clean_status = " + self.clean_status + " fan_speed = None")
    ...
    def _handle_battery_info(self, event):
        ...
        _LOGGER.warning("*** battery_status = {} %".format(self.battery_status))
    ...
    def _handle_charge_state(self, event):
        ...
        if status != 'idle' or self.vacuum_status == 'charging':
            ...
            self.statusEvents.notify(self.vacuum_status)
        _LOGGER.warning("*** charge_status = " + self.charge_status)
...
class EcoVacsIOTMQ(ClientMQTT):
    ...
    def on_connect(self, client, userdata, flags, rc):        
        if rc != 0:
            ...
            _LOGGER.warning("EcoVacsMQTT - Connected with result code "+str(rc))
            _LOGGER.warning("EcoVacsMQTT - Subscribing to all")        
    ...
    def send_ping(self):
        _LOGGER.warning("*** MQTT sending ping ***")
    ...
    def send_command(self, action, recipient):
        ...
        _LOGGER.warning('Sending command {0}'.format(c))        
    ...
    def __call_iotdevmanager_api(self, args, verify_ssl=True):
        _LOGGER.warning("calling iotdevmanager api with {}".format(args))                
        params = {}
        params.update(args)
    ...
    def _ctl_to_dict_api_json(self, action, jsonstring):
        if jsonstring['body']['msg'] == 'ok':
            ...
            _LOGGER.warning('BEFORE Status:' + eventname)
        ...
        _LOGGER.warning("LOGGED EVENT: " + jsonstring['event'])
        return jsonstring
...
class EcoVacsXMPP(ClientXMPP):
    ...
    def session_start(self, event):
        _LOGGER.warning("----------------- starting session ----------------")
        _LOGGER.warning("event = {}".format(event))
...
class Clean(VacBotCommand):
    def __init__(self, type='auto'):
        super().__init__('clean', {'body': {'data': {'act': CLEAN_ACTION_START,'type': type}},'header': {'pri': '2', 'ts': datetime.datetime.now().timestamp(), 'tmz': 480, 'ver': '0.0.22'}})

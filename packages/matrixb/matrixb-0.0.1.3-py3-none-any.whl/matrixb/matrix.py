...
class Matrix(object):
    ...
    def __init__(self, source=None, columns=None, null_values=False, extra_null_values=None, indexed_columns=None, column_normalizer='default', columns_from_source=None, column_types=None, duplicate_column_policy='warning', recode_duplicates=True, rowcount_policy='error', autoclean=True, load_policy="lazy", data_cleaner=False):
        ...
        if source is None:
            ...
            extre = re.search(r'\.(\w+)$', source)
            if not extre:
                raise Exception("failed to identify correct source module for file named '"+source+"'. matrixb can only load files of known types with appropriate extensions - build the source by hand for custom filenames.")
            ext = extre.group(1).upper()

            # see if it is a class loaded into the matrixb.source namespace
            if ext in matrixb.source.__dict__:
                srccls = matrixb.source.__dict__[ext]
                if not issubclass(srccls, matrixb.source.SourceBase):
                    raise Exception("Extension " + ext + " does not have a known (and loaded) matrixb source")
                self.source = srccls(source)
        else:
            self._loaded = False

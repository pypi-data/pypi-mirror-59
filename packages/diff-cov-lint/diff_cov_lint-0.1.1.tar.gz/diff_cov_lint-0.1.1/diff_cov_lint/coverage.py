...
def diff_coverage_report(diff_coverage_dict: dict) -> str:
    ...
    ans += f"{'FILE':<20}\t\t\t{'COVERED'}\t{'STMTS'}\t{'PERCENT'}\n"
    total_covered = 0
    total_statements = 0
    for filename in diff_coverage_dict:
        ...
        ans += f"{filename:<20}\t\t\t{covered_num:>7}\t{statements_num:>5}\t" \
    ...
    if total_statements:
        ...
        total_percentage = 1
    ans += f"{'TOTAL DIFF COV':<20}\t\t\t{total_covered:>7}\t{total_statements:>5}\t"\

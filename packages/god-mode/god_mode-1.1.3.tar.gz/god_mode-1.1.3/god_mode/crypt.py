...
def main():
    ...
    version = 'v0.0.1 (deprecated)'

    print(Style.DIM + Fore.BLACK + Back.YELLOW + ' [MESSAGE]=> ' + Style.RESET_ALL + ' Using an outdated version of god-mode. Consider using crypt2 as substitute (in development)')
    ...
    parser.add_argument('-d','--destination', help='Path to the encrypted/decrypted file(s). (Do NOT include any file or extension. Only the location to store the file(s).)', type=str)
    parser.add_argument('-m','--mode', help='Choose whether to encrypt or decrypt the file(s).', choices=['encrypt', 'decrypt'], type=str)
    parser.add_argument('-p','--password', help='The password to encrypt and decrypt the files.', type=str)
    ...
    parser.add_argument('--version', help="Print the script version.", action='store_true')
    args = parser.parse_args()

    if args.version:
        print(Style.DIM + Fore.BLACK + Back.YELLOW + ' [GOD-MODE]=> ' + Style.RESET_ALL + '\tgod-mode ' + version)
        sys.exit(0)

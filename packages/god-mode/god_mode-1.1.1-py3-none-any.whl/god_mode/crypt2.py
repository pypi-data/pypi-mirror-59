...
def main():
    ...
    if args.destination == "." or args.destination == "./" or args.destination == ".\\":
    ...
    if not 'libs.json' in dec_file:
        is_dec_encrypted = is_binary(dec_file)
    elif 'libs.json' and is_dec_file_non_existent:
        is_dec_encrypted = False
    else:
        is_dec_encrypted = True

    print('============ {0} {1} ============='.format(is_dec_encrypted, dec_file))
    ...
    if args.argFiles:
        if args.mode == 'encrypt':
            encrypt_files(args.argFiles, args.destination, args.extension, args.password, bufferSize, args.verbose, dec_file, is_dec_encrypted, dec_file_abs_dir, args.kill, is_dec_file_non_existent)
        ...
        if args.mode == 'encrypt':
            encrypt_files(lines, args.destination, args.extension, args.password, bufferSize, args.verbose, dec_file, is_dec_encrypted, dec_file_abs_dir, args.kill, is_dec_file_non_existent)
        elif args.mode == 'decrypt':
...
def encrypt_files(files, destination, extension, password, bufferSize, verbose, dec_file, is_dec_encrypted, dec_file_abs_dir, kill, is_dec_file_non_existent):
    ...
    for item in files:
        ...
        if is_dec_encrypted or enc_triggered:
            if(verbose):
            ...
            data_temp = []
            with open(temp_dec) as json_file:
                data_temp = json.load(json_file)
            data_temp.append(data)
            
            os.remove(dec_file)

            with open(temp_dec, 'w') as outfile:

...
from rqdatac.decorators import coroutine
...
class DummyConnection:
    def execute(self, method, *args, **kwargs):
        raise connection_error("can not connect to server")
...
class Connection:
    ...
    def is_normal(self):
        # socket is normal while status is not readable and not exceptional
        readable, _, exceptional = select.select([self._socket], [], [self._socket], 0)

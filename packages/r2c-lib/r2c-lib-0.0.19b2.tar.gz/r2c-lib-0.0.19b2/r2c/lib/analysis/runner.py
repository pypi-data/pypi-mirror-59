...
class AnalysisRunner:
    ...
    # If true, Docker images are run using the same UID as the host (i.e., the
    # UID running this process). Not all analyzers will be compatible with
    # this, but most should be.
    _run_as_host_uid: bool = False
    ...
    def prepare_mount_volume(
        ...
        if self._run_as_host_uid:
            # No need to mangle permissions if we're running as the user
            # anyway.
            manager.should_set_permissions = False
        ...
        try:
            with Timeout(self._timeout):
                container = self._docker_client.containers.create(
                    ...
                    user=os.getuid() if self._run_as_host_uid else None,

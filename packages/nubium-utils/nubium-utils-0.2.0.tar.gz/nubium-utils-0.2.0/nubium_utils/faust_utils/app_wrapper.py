...
class FaustAppWrapper(ABC):
    ...
    def add_input_topic(self, topic_name, value_serializer):
        ...
        """
        faust_name = ''.join([topic_name, '_faust'])
        self.input_topics[topic_name] = self.app.topic(
            topic_name,
            key_serializer=self.key_serializer,
            value_serializer=value_serializer)
        self.faust_input_topics[topic_name] = self.app.topic(
            faust_name,
    ...
    def _init_message_parser(self):
        """
        Converts all inbound/ingress topics to a default message format (FaustMessage) with easily accessible headers.
        """
        for topic_name, topic_obj in self.input_topics.items():
            @self.app.agent(topic_obj, name=''.join([topic_name, '-agent']))
            async def parse_messages(input_messages):
                """
                Will read the input topic message, convert it to a FaustMessage object, and send it the downstream topic
                as generated by the 'self.add_input_topic' method.

                """
                async for record in input_messages.events():
                    yield await self.faust_input_topics[record.message.topic].send(
                        value=FaustMessage(**{
                                'key': record.key,
                                'value': record.value,

...
def vp_start_gui():
    ...
    global val, w, root, topw
    root = tk.Tk()
    topw = ICO3StartIcocube (root)
...
class ICO3StartIcocube:
    ...
    def launch():
        ...
        topw = ICO3StartIcocube (root)
        #ICO3Starter_support.init(root, top)
        root.protocol("WM_DELETE_WINDOW", ICO3StartIcocube.on_closing)
        root.mainloop()
    ...
    def __init__(self, top=None):
        ...
        top.geometry("300x150+300+100")
        Argv = sys.argv
        if (len(Argv) > 2):
            top.title(Argv[1])
        else:
            top.title("Icocube Starter")
        top.configure(background="#d9d9d9")

        self.BTStartIcocube = tk.Button(top)
        self.BTStartIcocube.place(relx=0.080, rely=0.155, height=33, width=250)
        self.BTStartIcocube.configure(activebackground="#ececec")
        self.BTStartIcocube.configure(activeforeground="#000000")
        self.BTStartIcocube.configure(background="#d9d9d9")
        self.BTStartIcocube.configure(command=self.CBStartIcocube)
        self.BTStartIcocube.configure(disabledforeground="#a3a3a3")
        self.BTStartIcocube.configure(foreground="#000000")
        self.BTStartIcocube.configure(highlightbackground="#d9d9d9")
        self.BTStartIcocube.configure(highlightcolor="black")
        self.BTStartIcocube.configure(pady="0")
        self.BTStartIcocube.configure(text='''Start Icocube Server''')
        self.BTStartIcocube.configure(width=250)

        self.BTStopIcocube = tk.Button(top)
        self.BTStopIcocube.place(relx=0.080, rely=0.500, height=33, width=250)
        ...
        self.BTStopIcocube.configure(command=self.CBStopIcocube)
        ...
        self.BTStopIcocube.configure(text='''Stop Icocube Server''')
        self.BTStopIcocube.configure(width=250)
        self.BTStopIcocube.config(state="disable")
        print(" User Directory : " + str(Path.home()))

    def load_file(self):
        fileName = filedialog.askopenfilename(initialdir='./Icocube', filetypes=(("Xml files", "*.xml") , ("All files", "*.*")))
        return fileName

    def CBStartIcocube(self):
        print("it's ICOCUBE STARTER")
        StarterDir = self.load_file()
        if StarterDir == "":
            return
        ...
        print(" Parameters File : "+ StarterDir)
        XmlRoot = XmlProcessing.process(StarterDir, "ICO3Node")
        if (XmlRoot == None):

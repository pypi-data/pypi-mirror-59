...
general linear inequality/equality constraints and fixed/tied variables".
...
def lsqlin(A_lsq, b_lsq, A_ineq=None, b_ineq=None, A_eq=None, b_eq=None,
           bounds=None):
    """
    Linear least-squares problem with both linear equalities and inequalities
    ...
    if A_ineq is None:
        ...
        r = E @ u - f
        assert linalg.norm(r) > tol, "Constraints are incompatible"
        x = Vs @ (-r[:-1]/r[-1] + fu)
...
class capfit:
    ...
    ``CapFit`` is a novel implementation of the trust-region nonlinear
    least-squares algorithm by Levenberg-Marquardt (LM) with the key
    addition of linear inequality/equality constraints and bound constraints.
    Moreover one can easily tie or fix some parameters without
    having the modify the fitting function.
    ...
    as it allows for linear constraints by solving a full quadratic programming
    problem at every iteration. This approach is robust and accurate, and is
    efficient in the common situations when the function evaluation is at least
    as expensive to compute as the quadratic solution.
    ...
    def __init__(self, func, p1, abs_step=None, bounds=(-np.inf, np.inf),
        ...
        while(True):
            ...
            b_ineq_p = b_ineq if A_ineq is None else b_ineq - A_ineq @ p1
            b_eq_p = b_eq if A_eq is None else b_eq - A_eq @ p1
            ...
            # Algorithm (5.1.6) of Fletcher (1987)

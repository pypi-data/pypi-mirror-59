...
class FeaturesAssembler(AbstractAssembler):
    def reassembleAdvancedFeaturesForClusterization(self, feature_type, n_past_features = 21, simple = True):
        ...
        macro_features, macro_features_names = self.preprocessMacroFeatures()
        df, strats, strat_features_names = self.preprocessReturns()

        duplicated_features = [my_feat for my_feat in advanced_features_names if my_feat in macro_features]
        advanced_features = advanced_features.drop(columns=duplicated_features)
        advanced_features_names = [col for col in list(advanced_features.columns) if col != 'Date']

        features_names = advanced_features_names + macro_features_names
    ...
    def preprocessFeatures(self):
        ...
        features = self.dbx.local_overwrite_and_load_pickle( folder='', subfolder='', returns_pkl_file_name=self.features_pkl_file_name, local_root_directory = self.local_root_directory)
    ...
    def assembleFeature(self, feature_type, n_past_features):
        ...
        df_ret = pd.merge(df_ret, macro_features, how='left', on=['Date'], right_index=True)

        df_ret = df_ret.replace([np.inf, -np.inf], np.nan)
        df_ret=df_ret.fillna(0.)

        print('number of infs in macro features')
        print(np.isinf(df_ret[macro_features_names]).sum(axis=0))
        ...
        print('number of nans in macro features')
        print(np.isnan(df_ret[macro_features_names]).sum(axis=0))

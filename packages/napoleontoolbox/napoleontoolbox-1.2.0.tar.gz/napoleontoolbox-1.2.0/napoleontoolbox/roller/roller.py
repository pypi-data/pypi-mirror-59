...
class _RollingBasis:
    ...
    def __init__(self, X, y, cbs=None, cb_funcs=None):
        ...
        sched_standard = ts_callback.create_standard_annealing_profile()
        if cb_funcs is None:
            cb_funcs = [ts_callback.Recorder,
                        ts_callback.TrainEvalCallback,
                        partial(ts_callback.AvgStatsCallback, nn.MSELoss()),
                        partial(ts_callback.ParamScheduler, 'lr', sched_standard)]

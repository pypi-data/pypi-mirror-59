...
def process_weights(w=None, df=None, s=None, n=None, low_bound=None, up_bound=None, weight_cutting_rate = 0.7, display=True):
    ...
    for t in range(n, T, s):
        ...
        to_redispatch_match =np.logical_and( assets.reshape(len(assets),-1),np.logical_not(np.logical_or(below_the_lower_bound , above_the_upper_bound)))
        among_to_redispatch_length= to_redispatch_match.sum()
        if among_to_redispatch_length>0:
            weight_vect[to_redispatch_match] += to_redispatch_quantity/among_to_redispatch_length

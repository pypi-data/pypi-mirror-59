...
class AbstractAssembler(ABC):
    def __init__(self, drop_token=None, features_pkl_file_name='features.pkl', returns_pkl_file_name='returns.pkl', local_root_directory='../data/', user='napoleon',
                 supervision_npy_file_name='_supervision.npy', low_bound=0.02, up_bound=0.4):
        super().__init__()
        self.features_pkl_file_name = features_pkl_file_name
        self.returns_pkl_file_name = returns_pkl_file_name
        self.local_root_directory = local_root_directory
        self.user = user,
        self.supervision_npy_file_name = supervision_npy_file_name
...
class UtilitySuperviser(AbstractAssembler):
    def computeUtility(self, s):
        self.dbx.download(folder = '',subfolder= '', name = self.returns_pkl_file_name)
        local_returns_pkl_path = self.local_root_directory + self.returns_pkl_file_name
        print('loading local returns pickle file reading ' + local_returns_pkl_path)
        ...
        self.dbx.download(folder = '',subfolder= '', name = self.features_pkl_file_name)
        local_features_pkl_path = self.local_root_directory + self.features_pkl_file_name
        print('advanced features pickle file reading')
        advanced_features = pd.read_pickle(local_features_pkl_path)

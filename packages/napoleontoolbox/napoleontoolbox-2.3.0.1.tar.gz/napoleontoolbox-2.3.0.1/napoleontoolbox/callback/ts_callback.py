...
class Recorder(Callback):
    ...
    def after_layer(self,**kwarg):
        ...
        if self.act_means is None:
            self.act_means = [[] for _ in self.run.layers]
            self.act_stds = [[] for _ in self.run.layers]
    ...
    def get_all_recordings(self):
        ...
        lrs_df = self.get_lrs()
        return pd.concat([layer_stats_df, losses_df, lrs_df], axis=1)

    def get_all_activation_means(self):
        return pd.DataFrame(self.act_means)

    def get_all_activation_stds(self,):
        return pd.DataFrame(self.act_stds)

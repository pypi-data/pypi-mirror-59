...
def process_weights(w=None, df=None, s=None, n=None, low_bound=None, up_bound=None, weight_cutting_rate = 0.7, display=False):
    ...
    for t in range(n, T, s):
        ...
        to_redispatch = above_vect.sum()-below_vect.sum()

        among_to_redispatch_length= np.logical_and(assets ,np.logical_not(np.logical_or(below_the_lower_bound , above_the_upper_bound))).sum()

...
def process_weights(w=None, df=None, s=None, n=None, low_bound=None, up_bound=None, weight_cutting_rate = 0.5):
    ...
    for t in range(n, T, s):
        ...
        if w[test_slice.start].max()> up_bound or w[test_slice.start].min() < low_bound:
            raise Exception('Invalid incoming weights')
        ...
        if newValidWeights[0].max()> up_bound or newValidWeights[0].min() < low_bound:
            raise Exception('Invalid outgoing weights')

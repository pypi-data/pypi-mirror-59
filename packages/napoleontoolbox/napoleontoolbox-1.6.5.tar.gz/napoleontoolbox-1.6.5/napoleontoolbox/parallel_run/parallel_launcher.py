...
class ParalleLauncher():
    ...
    def __init__(self,drop_token='', local_root_directory='../data',user='napoleon',  db_path_suffix = '_run.sqlite', meta_features_types=[features_type.FeaturesType.STANDARD_ADVANCED], meta_layers=[[2048,1024,512,256,128,64,32]], convolutions=[0], activations=['relu'], meta_n=[126], meta_seeds=[0], epochss=[1], ss=[21], meta_n_past_features=[21], utilities=['f_drawdown']):
        ...
        db_path = local_root_directory + '/' + user + db_path_suffix
        print('local database saving path '+ db_path)
    ...
    def saveResults(self, run, values, weights_df, recordings_df):
        self.addRun(run)
    ...
    def upload_results_to_dropbox(self):
        file_name = self.user + self.db_path_suffix
        print('uploading to dropbox the results '+self.db_path)
        self.dbx.uploadRunResuts(file_name=file_name,full_path = self.db_path)

    ...
    for t in range(n, T, s):
        ...
        if weight_vect.sum() == 0.:
            if previousValidWeights is None:
                try:
                    weight_mat.iloc[test_slice] = np.ones([test_slice.stop - test_slice.start, N]) / N
                except ValueError:
                    print('###################previous weights')
                    print(previousValidWeights)
                    print(previousValidWeights.shape)
                    print(test_slice)
            else :
                try:
                    weight_mat.iloc[test_slice] = previousValidWeights
                except ValueError:
                    print('###################previous weights')
                    print(previousValidWeights)
                    print(previousValidWeights.shape)
            ...
            weight_mat.iloc[test_slice] = np.ones([test_slice.stop - test_slice.start, 1]) @ weight_vect.T / weight_vect.sum()
            previousValidWeights =  np.ones([test_slice.stop - test_slice.start, 1]) @ weight_vect.T / weight_vect.sum()
        else:
            weight_mat.iloc[test_slice] = np.ones([test_slice.stop - test_slice.start, 1]) @ weight_vect.T
            previousValidWeights =  np.ones([test_slice.stop - test_slice.start, 1]) @ weight_vect.T

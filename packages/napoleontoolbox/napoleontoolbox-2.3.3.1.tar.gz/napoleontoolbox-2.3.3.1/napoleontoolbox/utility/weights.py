...
def process_weights(w=None, df=None, s=None, n=None, low_bound=None, up_bound=None, weight_cutting_rate = 0.7, display=True):
    ...
    for t in range(n, T, s):
        ...
        if to_redispatch_quantity>0:
            match =np.logical_and(assets.reshape(len(assets),-1),np.logical_not(above_the_upper_bound))
            weight_vect[match] += to_redispatch_quantity/match.sum()
        else:
            match = np.logical_and(assets.reshape(len(assets),-1),np.logical_not(below_the_lower_bound))
            weight_vect[match] += to_redispatch_quantity/match.sum()

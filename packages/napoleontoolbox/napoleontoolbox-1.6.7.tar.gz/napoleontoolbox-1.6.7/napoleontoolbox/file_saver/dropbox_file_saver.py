...
class NaPoleonDropboxConnector(object):
    ...
    def local_sqlite_overwrite(self,folder='', subfolder='', sqlite_file_name='',local_root_directory=''):
        ...
        print('overwriting local file ' + local_sqlite_path + ' with dropbox file ', sqlite_file_name)
    ...
    def local_supervision_npy_overwrite_and_load_array(self, rebal=None, local_root_directory='', user='',supervision_npy_file_suffix=''):
    ...
    def local_features_npy_save_and_upload(self, features=None, features_names=None, feature_type=None, n_past_features = None, local_root_directory='', user='', features_saving_suffix='', features_names_saving_suffix=''):
        features_file_name = user + '_' + feature_type.name + '_' + str(n_past_features) + features_saving_suffix
        features_full_path = local_root_directory  + features_file_name
        print('saving local file '+features_full_path)
        np.save(features_full_path, features)
        self.upload(fullname=features_full_path, folder='', subfolder='', name = features_file_name, overwrite=False)
        ...
        self.upload(fullname=features_names_full_path, folder='', subfolder='', name = features_names_file_names, overwrite=False)
    ...
    def local_supervision_npy_save_and_upload(self, data=None, rebal = None, local_root_directory='', user='', supervision_npy_file_suffix=''):
        supervision_file_name = user + '_' + str(rebal) + supervision_npy_file_suffix
        local_supervision_npy_path = local_root_directory + supervision_file_name
        print('saving local file '+local_supervision_npy_path)
    ...
    def uploadRunResuts(self, filename='', fullpath=''):
        self.upload(fullname=fullpath, folder='', subfolder='', name = filename, overwrite=False)

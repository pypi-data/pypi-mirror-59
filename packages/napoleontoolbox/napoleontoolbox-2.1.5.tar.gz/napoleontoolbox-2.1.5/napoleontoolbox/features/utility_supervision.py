...
class AbstractAssembler(ABC):
    def __init__(self, drop_token=None, features_pkl_file_name='features.pkl', returns_pkl_file_name='returns.pkl', local_root_directory='../data/', user='napoleon',
                 supervision_npy_file_suffix='_supervision.npy', low_bound=0.02, up_bound=0.4,macro_supervision_npy_file_suffix='_macro_supervision.npy',):
        super().__init__()
...
class UtilitySuperviser(AbstractAssembler):
    ...
    def computeMacroForecastingUtility(self, s, cutting_rate_threshold=0.7):
        ...
        def process(series):
            ...
            return series.value_counts(dropna=False).max() < cutting_rate_threshold * s
        ...
        self.dbx.local_supervision_npy_save_and_upload(data = result, rebal = s , local_root_directory= self.local_root_directory, user = self.user, supervision_npy_file_suffix= self.macro_supervision_npy_file_suffix)
        print('files saved and updated to dropbox')

    def computeUtility(self, s, cutting_rate_threshold=0.7):
        df = self.dbx.local_overwrite_and_load_pickle( folder='', subfolder='', returns_pkl_file_name=self.returns_pkl_file_name, local_root_directory = self.local_root_directory)
        df['Date'] = pd.to_datetime(df['Date'])
        df = df.set_index('Date')
        strats = [col for col in list(df.columns) if col != 'Date']
        ...
        self.dbx.local_supervision_npy_save_and_upload(data = result, rebal = s , local_root_directory= self.local_root_directory, user = self.user, supervision_npy_file_suffix= self.supervision_npy_file_suffix)
        print('files saved and updated to dropbox')

...
def process_weights(w=None, df=None, s=None, n=None, low_bound=None, up_bound=None, weight_cutting_rate = 0.7, display=True):
    ...
    for t in range(n, T, s):
        ...
        redispatched_quantity = above_vect.sum()-below_vect.sum()
        to_redispatch_quantity = 1-weight_vect.sum()

        to_redispatch_match =np.logical_and( assets.reshape(len(assets),-1),np.logical_not(np.logical_or(below_the_lower_bound , )))
        among_to_redispatch_length= to_redispatch_match.sum()
        if among_to_redispatch_length>0:
            if to_redispatch_quantity>0:
                weight_vect[np.logical_and(assets,np.logical_not(above_the_upper_bound))] += to_redispatch_quantity/among_to_redispatch_length
            else:
                weight_vect[np.logical_and(assets,np.logical_not(below_the_lower_bound))] += to_redispatch_quantity/among_to_redispatch_length

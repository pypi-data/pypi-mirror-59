The rho function is defined so that rho(G, m, h) is the smallest size of hA, where |A| = m. In other words, rho(G, m, h) is the smallest the h-fold sumset of a size m subset of G can be.

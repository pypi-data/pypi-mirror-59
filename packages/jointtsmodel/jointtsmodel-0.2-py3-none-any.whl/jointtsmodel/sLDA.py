...
class sLDA(BaseEstimator):
    ...
    def fit(self, X, vocabulary, rerun=False, max_iter=None):
        ...
        normalized_n_vts = self.n_vts.copy()
    ...
    def phi(self):
        ...
        """
        normalized_n_dts = self.n_dts.copy()
    ...
    def getTopKWords(self, vocabulary, num_words=5):
        ...
        pseudocounts = np.copy(self.n_vts + self.topic_sentiment_word_prior_)
        ...
        for t in range(self.n_topic_components):
            for s in range(self.n_sentiment_components):
                ...
                worddict[(t, s)] = [vocabulary[i] for i in topWordIndices]

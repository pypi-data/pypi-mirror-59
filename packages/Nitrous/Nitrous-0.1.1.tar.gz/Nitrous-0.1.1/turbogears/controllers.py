...
def _process_output(output, template, format, content_type, fragment=False,
    ...
    if 'json' not in view.engines:
        # FIXME: depends on our stuff :(
        # from blcore.utils.json import dumps
        ...
        class JSONSupport(object):
            def render(self, info, format=None, fragment=False, template=None):
                ...
                return dumps(info).encode('utf-8')

        view.engines['json'] = JSONSupport()
...
def _add_rule(_expose, found_default, as_format, accept_format, template,
    ...
    #
    rules.append((_expose, rule, rulefunc))
...
def expose(template=None, allow_json=None, format=None, content_type=None,
    ...
    def expose_view(fn, *args, **kw):
        def do_invoke(*args, **kw):
            if kw.get('tg_format') == 'json':
                return _execute_func(fn, 'json', 'json', 'application/json',
                    False, {}, args, kw)

            else:
                return _execute_func(fn, template, format, content_type,
                      fragment, options, args, kw)

        @cherrypy.expose
        def invoke_view(*args, **kw):
            accept = request.headers.get('Accept', "").lower()
            accept = tg_util.simplify_http_accept_header(accept)

            if hasattr(request, 'in_transaction'):
                return do_invoke(*args, **kw)
            else:
                request.in_transaction = True
                return database.run_with_transaction(do_invoke, *args, **kw)
    ...
    def entangle(func):
        ...
        if not getattr(func, 'exposed', False):
            def expose(func, *args, **kw):
                ...
                try:
                    if kw.get('tg_format') == 'json':
                        return _execute_func(func, 'json', 'json', 'application/json',
                            False, {}, args, kw)

                    else:
                        return _execute_func(func, template, format, content_type,
                              fragment, options, args, kw)
...
def _execute_func(func, template, format, content_type, fragment, options,
    ...
    if str(getattr(response, 'status', '')).startswith('204'):
        ...
        assert isinstance(output,
            (basestring, dict, list, types.GeneratorType, type(None))) or (

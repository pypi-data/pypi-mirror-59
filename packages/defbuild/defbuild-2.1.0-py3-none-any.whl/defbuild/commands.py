...
def build(project):
    ...
    if project.platform == "armv7-android":
        ...
        if not project.identity:
            ...
            cert = project.config.get(project.name, "certificate", None)
            pk = project.config.get(project.name, "private-key", None)
            if cert and pk:
                command.extend(["--certificate", cert, "--private-key", pk])

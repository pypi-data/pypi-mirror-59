...
def get_rule_number_from_language_code(language_code):
    """
    ...
    Args:
        - language_code (str): the language code for which to get the rule number. It should be in ISO
          ...
          in 639-1 (e.g. "frr"). Country codes should generally not be used.
...
def get_form_from_rule(rule_number, count):
    """
...
def get_plural_form(form_number, forms):
    """
...
class Pluralizable:
    ...
    def __init__(self, forms=None):
        ...
        """
        if forms is None:
            raise ValueError("`form` argument is required for Pluralizable class.")
        self.forms = forms
    ...
    def pluralize_by_language(self, language_code, count):
        """

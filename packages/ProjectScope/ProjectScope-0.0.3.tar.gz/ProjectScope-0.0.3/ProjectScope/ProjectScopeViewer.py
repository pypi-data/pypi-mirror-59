...
class ProjectNodes():
    def __init__(self, db_file):
        ...
        c = conn.cursor()
        c.execute("""Create TABLE IF NOT EXISTS nodes (id integer PRIMARY KEY, name TEXT, path TEXT, tags_json TEXT, params_3d_json TEXT)""")
        conn.commit()
        conn.close()
...
class ProjectLinks():
    def __init__(self, db_file):
        ...
        conn = sqlite3.connect(self.db_file)
        c = conn.cursor()
        c.execute("""Create TABLE IF NOT EXISTS links (id INTEGER PRIMARY KEY, node_id1 INTEGER, node_id2 INTEGER, name TEXT, tags_json TEXT, params_3d_json TEXT)""")
        conn.commit()
        conn.close()

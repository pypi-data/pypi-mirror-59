...
class Panda3dApp(ShowBase):
    ...
    def __init__(self, width, height, handle, pipe):
        ...
        project_scope_path = None
        while project_scope_path is None:
            # Check the current directory
            filepath = os.path.join(project_folder, filename)
            if(os.path.isfile(filepath)):
                # Found the project filepath
                project_scope_path = filepath
            else:
                # Didn't find the file in this folder, so check one level up
                parent_folder = os.path.abspath(os.path.join(project_folder, ".."))
                if(parent_folder == project_folder):
                    # Unable to get a parent folder from here, and we didn't find the
                    # .projectscope file, so just bail out!
                    break
                else:
                    project_folder = parent_folder

        if(project_scope_path is None):
            print("Did not find '.projectscope' file")
            return

        global project_path
        project_path = project_scope_path
        self.db_file = project_scope_path
        msg_dict = {}
        msg_dict["msg"] = "project_path_set"
        msg_dict["path"] = project_scope_path
    ...
    def controlCamera(self, task):
        ...
        movement_sum = sum([self.keys['a'], self.keys['d'], self.keys['s'],
                            self.keys['q'], self.keys['w'], self.keys['e']])
        if(self.shift_held):
        ...
        try:
            while self.pipe.poll():
                ...
                if "resize" == request['msg']:
                    ...
                    self.move_node(request['id'], new_x_pos=request['x'],
                                   new_y_pos=request['y'], new_z_pos=request['z'])
                elif "node_size_change" == request['msg']:
                    self.size_node(request['id'], new_x_size=request['x'],
                                   new_y_size=request['y'], new_z_size=request['z'])
    ...
    def create_cylinder(self, p1, p2, radius, colour, transparency):
        ...
        quat_coef = LVecBase4f(cos(rotation_angle/2), rotation_axis[0]*sin(
            rotation_angle/2), rotation_axis[1]*sin(rotation_angle/2), rotation_axis[2]*sin(rotation_angle/2))
    ...
    def lmb_shift_press(self):
        ...
        if(0 == num_selected_nodes):
            ...
            params_3d = {}
            params_3d['center'] = [x for x in (LMatrix4f().translateMat(
                0.0, 15.0, 0.0) * base.camera.getMat()).getRow3(3)]
...
class ProjectViewer(wx.Frame):
    ...
    def populate_node_info_panel(self, params):
        ...
        if(None == params['params_3d']['colour']):
            ...
            self.node_info_panel_dict['colour'].SetValue(
                ",".join([str(round(part, 3)) for part in params['params_3d']['colour']]))
    ...
    def populate_link_info_panel(self, params):
        ...
        if(None == params['params_3d']['colour']):
            ...
            self.link_info_panel_dict['colour'].SetValue(
                ",".join([str(round(part, 3)) for part in params['params_3d']['colour']]))

...
task_monitor_config =
{
  ...
  "stdout_stream.backup_count": "5"
    ...
    def __init__(self, config_file_path, monitor_config_filename="monitor.json"):
        ...
        self.monitor_config_filename = monitor_config_filename
    ...
    def __init_global_config(self):
        if Path(self.config_file_path).exists():
            file_path_final = self.config_file_path
        else:
            relative_path = "%s/%s" % (self.__work_path, self.config_file_path)
    ...
    def __get_monitor_config(self):
        file_path = "%s/%s" % (self.__work_path, self.monitor_config_filename)
        config_info = None
    ...
    def process_cmd_info(self, cmd_type):
        cmd = self.__init_env()
        cmd['type'] = cmd_type
    ...
    def monitor_start(self):
        self.process_cmd_info("start")

    def monitor_restart(self):
        self.process_cmd_info("restart")

    def monitor_stop(self):
        self.process_cmd_info("stop")

    def monitor_install(self):
        self.process_cmd_info("install")

    def monitor_uninstall(self):
        self.process_cmd_info("uninstall")

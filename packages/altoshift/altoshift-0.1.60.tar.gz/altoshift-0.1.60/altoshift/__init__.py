...
class connect():
    ...
    def __init__(self, **kwargs):
        ...
        if not(args.get('DEV',False)):
            self.conf['host'] = args.get('host','https://api.altoshift.com')
            self.conf['xEngineId'] = args.get('engineId','')
            self.conf['xUserId'] = args.get('userId','')
            self.conf['xToken'] = args.get('token','')
            self.conf['sToken'] = args.get('sToken','')

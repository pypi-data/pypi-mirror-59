...
def back(graph, chosen, wr):
    return graph.get_out_degrees(chosen, wr)


def str(graph, chosen, wr):
    return graph.get_in_degrees(chosen, wr)


class ShoNodeEx(NodeExInfo):
    @staticmethod
    def get_ex(gr, root, frm, c, stab=30, entity=BlocksEstimate,
               full=None, frm_stub=True, view=5, part=True, criterion=pgr, w=False, way=str):
        if full is None:
            full = gr.size()

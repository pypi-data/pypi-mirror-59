    ...
    if  path.isfile(direct + filename):
        ...
        for i in range(1, dict_.nrows):
            try:
                ...
                atr = autogovernance.Attributes(*[x for x in row[11:]])
                bt = autogovernance.BusinessTerm(*[x for x in row[8:11]])
                elem = autogovernance.Element(*[x for x in row[0:8]], bt, atr)
...
def read_qr(filename):
    ...
    if  path.isfile(direct + filename):
        ...
        for i in range(1, dict_.nrows):
                ...
                row = dict_.row_values(i)
                qr_ = autogovernance.QR_object(*[x for x in row[0:14]])

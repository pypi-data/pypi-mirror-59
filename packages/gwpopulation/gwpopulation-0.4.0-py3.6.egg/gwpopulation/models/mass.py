...
def one_point_five_generation_mass_ratio(dataset, spectal_index, mmin):
    ...
    prob = (powerlaw(dataset['mass_ratio'], spectal_index, high=split, low=mmin / dataset['mass_1']) * (dataset["mass_ratio"] <= split) +
            powerlaw(1 - dataset['mass_ratio'], spectal_index, high=split, low=mmin / dataset['mass_1']) * (dataset["mass_ratio"] >= split)) / 2
    return prob

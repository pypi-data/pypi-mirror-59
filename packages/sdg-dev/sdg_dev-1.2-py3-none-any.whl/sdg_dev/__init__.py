...
import logging
from sdg_io import SdgIO, get_comports, get_log
from .dev_common import DevException, DevBadCode, BROADCAST
from .dev_master import DevMaster
...
class _ForSeachPort:
    ...
    def search_port(self, portlist=None) -> str:
        ...
        def _do_check_id():
            try:
                if self.check_id():
                    self.log.info(f"device {self.__class__.__name__} found {self._port}")
                    return True
            except DevException:
                self.log.info(f"no answer from '{self._port}'")
        ...
        for p in portlist:
            ...
            self.log.info(f"try connect to '{portname}'")
            try:
                io = SdgIO(self._port, self._portcfg, dump=self._dump)
            except IOError:
                self.log.info(f"port not avalible '{portname}'")
            else:
                self.set_io(io)
                if _do_check_id():
                    return self._port
                self.set_io(None)
                io.close()
        self.log.error(f"device '{self.__class__.__name__}' not found")
        return ''

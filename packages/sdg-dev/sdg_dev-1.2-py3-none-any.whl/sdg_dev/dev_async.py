...
from .dev_common import DevException, DevBadCode
...
class DevAsync(DevMaster):
    ...
    def _incmd_parser(self, rx):
        ...
        if rx[0] in self._incmd_table:
            ...
            self.log.debug(f"<cmd{name} {dump_msg(msg)}")
            ...
            if err is not None:
                ...
                self.log.debug(f"<cmd{name}!exe{err} {dump_msg(msg)}")

            if broadcast:
                return None
            else:
                self.log.debug(f">ack{name} {dump_msg(ack)}")
                return ack

        # нет такой команды в таблице -> команда недопустима
        self.log.warning(f"<cmd{name}?unknown {dump_msg(msg)}")
...
class _CmdAsync(CmdMaster):
    ...
    def __init__(self, addr, msg, ackfrmt, log, timeout, remix):
        super().__init__(addr, msg, ackfrmt, log, timeout, remix)

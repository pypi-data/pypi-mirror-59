...
def _getElement(tag,attr='class',value=None,html=None,start=None,end=None,before=None):
  ...
  while True:
    ...
    if(singleTag):
      strP = u'<[\s]*'+_dealRegChar(tag)+'[^>]+?'+_dealRegChar(attr)+'[=\'"\s]+([\w\-\.]+[\s\w\-\.]*[ ]|\s*|)'+value+'[\s\'"][\s\S]*?>'
    else:
      strP = u'<[\s]*'+_dealRegChar(tag)+'[^>]+?'+_dealRegChar(attr)+'[=\'"\s]+([\w\-\.]+[\s\w\-\.]*[ ]|\s*|)'+value+'[\s\'"][\s\S]*?>[\s\S]*?</'+_dealRegChar(tag)+'>'

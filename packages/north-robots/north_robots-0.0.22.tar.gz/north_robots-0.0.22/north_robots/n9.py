...
ProbeOffset = Union[Vector3, Tuple[float, float, float], List[float]]
ProbeRotation = Union[Vector3, Tuple[float, float, float], List[float]]
...
class N9Robot(Robot):
    ...
    def __init__(self, controller: C9Controller=C9Controller.default_controller, column_axis_number: int=3,
                 shoulder_axis_number: int=2, elbow_axis_number: int=1, gripper_axis_number: int=0, probe_offset: ProbeOffset=(0, 41.5, 0),
        ...
        :param probe_offset: the horizontal distance from the center of the gripper to the center of the probe in mm, defaults to 41.5mm
    ...
    def move(self, x: Optional[float]=None, y: Optional[float]=None, z: Optional[float]=None, gripper: Optional[float]=None,
             order: MovementOrder=MOVE_XYZ, velocity: Optional[float]=None, acceleration: Optional[float]=None, relative=False, wait: bool=True,
             probe: bool=False, probe_offset: ProbeOffset=Vector3(0, 0, 0), probe_rotation: ProbeRotation=Vector3(0, 0, 0)):
        """
        ...
        :param probe: use the default probe offset when moving
        :param probe_offset: [Optional] a vector or tuple offset from the gripper to use when moving
        :param probe_rotation: [Optional] a vector or tuple rotation to apply to the probe
        """
        ...
        if probe and probe_offset == Vector3(0, 0, 0):
            probe_offset = self.probe_offset

        if isinstance(probe_offset, tuple) or isinstance(probe_offset, list):
            probe_offset = Vector3(*probe_offset)

        if isinstance(probe_rotation, tuple) or isinstance(probe_rotation, list):
            probe_rotation = Vector3(*probe_rotation)

        offset = Quaternion.fromEulerAngles(probe_rotation) * probe_offset
        ...
        kwargs = dict(velocity=velocity, acceleration=acceleration, relative=relative)
        ...
        if x is None and y is None and z is None and gripper is not None:
            ...
            for item in order:
                ...
                # while supporting a vector probe offset
    ...
    def move_to_location(self, location: Union[Location, Vector3, Component, list, dict], gripper: Optional[float]=None,
        ...
        :param gripper: [Optional] final gripper rotation in degrees
        :param velocity: [Optional] velocity to use during the movement in counts/s, defaults to self.velocity_counts
        :param acceleration: [Optional] acceleration to use during the movement in counts/s^2, defaults to self.acceleration_counts
        :param order: [Optional] movement order constant (one of N9Robot.MOVE_XYZ, N9Robot.MOVE_Z_XY, N9Robot.MOVE_XY_Z)
        :param probe: use the default probe offset when moving
        :param probe_offset: [Optional] a vector or tuple offset from the gripper to use when moving
        :param probe_rotation: [Optional] a vector or tuple rotation to apply to the probe
        ...
        self.move(x=position.x(), y=position.y(), z=position.z(), gripper=gripper, order=order,
                  velocity=velocity, acceleration=acceleration, probe=probe, probe_offset=probe_offset, probe_rotation=probe_rotation)
    ...
    def move_to_locations(self, locations: List[Union[Location, Vector3, Component, list, dict]], gripper: Optional[float]=None, order: MovementOrder=MOVE_XYZ,
        ...
        :param acceleration: [Optional] acceleration to use during the movement in counts/s^2, defaults to self.acceleration_counts
        :param order: [Optional] movement order constant (one of N9Robot.MOVE_XYZ, N9Robot.MOVE_Z_XY, N9Robot.MOVE_XY_Z)
        :param probe: use the probe as the target instead of the gripper if True, defaults to False
        """

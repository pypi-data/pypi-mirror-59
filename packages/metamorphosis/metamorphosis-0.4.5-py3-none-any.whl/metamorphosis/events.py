...
def load_objecttype(value: dict) -> g.ObjectType:
    ...
    if isinstance(oclass._meta, g.types.union.UnionOptions):
        ...
        for field in oclass._meta.fields:
            ...
            print(oclass._meta.fields[field].type)
            if oval is not None:
                if hasattr(f.type, 'parse_value'):
                    vargs[field] = f.type.parse_value(oval)
                elif field == 'id':
                    vargs[field] = UUID(oval)
                elif isinstance(oclass._meta.fields[field].type, g.types.structures.List):
                    vargs[field] = [dump_objecttype(o) if isinstance(o, g.ObjectType) else o for o in oval]

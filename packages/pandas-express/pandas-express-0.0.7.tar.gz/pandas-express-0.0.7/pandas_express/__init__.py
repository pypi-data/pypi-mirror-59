...
@pf.register_series_method
def check_spelling(series, verbose=False):
    misspellings = {}
    for _, row in tqdm(series.iteritems(), total=series.shape[0]):
        spell = SpellChecker()
        words = row.split(" ")
        row_misspellings = spell.unknown(words)
        row_dict = {word: spell.correction(word) for word in row_misspellings}
        misspellings.update(row_dict)

    return misspellings


@pf.register_series_method
def fix_spelling(series, spellings=None):
    if not spellings:
        spellings = check_spelling(series)

    return series.replace(spellings)

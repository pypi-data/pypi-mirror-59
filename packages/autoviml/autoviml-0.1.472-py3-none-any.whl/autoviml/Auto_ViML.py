        ...
        if modeltype == 'Regression':
            if Boosting_Flag:
                if model_name == 'CatBoost':
                    ...
                    print('Using %s Model, Estimated Training time = %0.1f mins' %(model_name,data_dim/(20000.*CPU_count)))
            elif Boosting_Flag is None:
...
def find_top_features_xgb(train,preds,numvars,target,modeltype,corr_limit,verbose=0):
    ...
    if max_feats > 1:
        print('    Performing Linear feature selection from %d categorical vars...' %max_feats)
        if modeltype == 'Regression':
    ...
    important_features = []
    if modeltype == 'Regression':
        model_xgb = XGBRegressor(objective='reg:squarederror', n_estimators=100,subsample=subsample,

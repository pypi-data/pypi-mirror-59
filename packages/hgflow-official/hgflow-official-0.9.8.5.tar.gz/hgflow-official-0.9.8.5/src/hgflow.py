...
# The py3-Mercurial has created its world in terms of string type: It uses `bytes' everywhere it can. But the Python3 world
# uses `str' ("utf-8"). We have to convert `str' objects to `bytes' before calling the Mercurial APIs, and vice versa for the
# returned objects. It is very tedious and error-prone if we do the convertions for each Mercurial API call. So we create
# the `HgObj' class to do it automatically.
def output_filter( a ) :
    """
    Filters `a' that is supposed to be an output from a Mercurial API. Some objects may be converted per the following rule:
    - All `bytes' objects in `a' will be converted to `str' if it's convertable.
    - If a `bytes' object is not convertable to `str', it will be kept as is.
    - If `a' has or is a callable or a non-builtin type, it's converted to a `HgObj' object.
    - For other non-container types, `a' itself will be returned.
    """
    ...
    if (isinstance( a, list  )) : return list ( output_filter( e ) for e in a )
    if (isinstance( a, tuple )) : return tuple( output_filter( e ) for e in a )
    if (isinstance( a, set   )) : return set  ( output_filter( e ) for e in a )
    if (isinstance( a, dict  )) : return dict ( (output_filter( k ), output_filter( v )) for k, v in a.items() )
    if (callable( a ) or ("builtins" not in a.__class__.__module__)) : return HgObj( a )
...
def input_filter( a ) :
    """
    Filters `a' that is supposed to be an input for a Mercurial API. Some objects may be converted per the following rule:
    - All `str' objects in `a' will be converted to `bytes'.
    - If `a' is a `HgObj' object, it will be converted to the original Mercurial's object.
    """
    if (a is None             ) : return None
    if (isinstance( a, str   )) : return a.encode( "utf-8" )
    if (isinstance( a, list  )) : return list ( input_filter( e ) for e in a )
    if (isinstance( a, tuple )) : return tuple( input_filter( e ) for e in a )
    if (isinstance( a, set   )) : return set  ( input_filter( e ) for e in a )
    if (isinstance( a, dict  )) : return dict ( (k, input_filter( v )) for k, v in a.items() )
    if (isinstance( a, HgObj )) : return a.hg_obj
    return a
...
class HgObj:
    """
    Class to wrap Mercurial objects, which uses `bytes' instead of `str'.
    """
    def __init__( self, hg_obj ) :
        self._hg_obj = hg_obj


    def __getattr__( self, attr ) :
        return output_filter( getattr( self._hg_obj, attr ) )


    def __getitem__( self, key ) :
        return output_filter( self._hg_obj[input_filter( key )] )


    def __call__( self, *arg, **kwarg ) :
        return output_filter( self._hg_obj( *input_filter( arg ), **input_filter( kwarg ) ) )


    @property
    def hg_obj( self ) :
        """
        Return the original Mercurial object.
        """
        return self._hg_obj



extensions = HgObj( extensions )
mercurial  = HgObj( mercurial  )
cmdutil    = HgObj( cmdutil    )
config     = HgObj( config     )
short      = HgObj( short      )
util       = HgObj( util       )
...
def _version_numbers( version_string ) :
    """
    Convert a version string to a 5-tuple of integer numbers. The numbers in the version string should be separated by a
    single dot. If the version has less than 5 numbers, the missing numbers are zeros.
    Example:
      assert (1, 2, 4, 0, 0) == _version_numbers( "1.2.4" )
    """
    return tuple( [int( e ) for e in version_string.split( '.' )] + [0, 0, 0, 0, 0] )[:5]


def _is_equal_or_later( ref_version ) :
    """
    Return `True' if the current version of Mercurial is equal to or later than the given version `ref_version', or `False' if
    not.
    """
    cur_version = util.version()
    for cur, ref in zip( _version_numbers( cur_version ), _version_numbers( ref_version ) ) :
        if (cur > ref) :
            return True
        if (cur < ref) :
            return False
...
def _choose_command() :
    return mercurial.registrar.command


def _choose_datestr() :
    return mercurial.utils.dateutil.datestr
...
def _get_context( repo, rev ) :
    ...
    try :
        return repo[rev]
    except (error.ProgrammingError, error.RepoLookupError, TypeError) :
        return repo[repo.branchtip( rev )]


def _get_branchmap_items( repo ) :
    return repo.branchmap().iteritems()
...
colortable = {b"flow.error"      : b"red bold",
              b"flow.warn"       : b"magenta bold",
              b"flow.note"       : b"cyan",
              b"flow.help.topic" : b"yellow",
              b"flow.help.code"  : b"green bold",
              }
    ...
    def __call__( self, ui, repo, *arg, **kwarg ) :
        ...
        try :
            if ("strip" == cmd) :
                ...
                cmd   = "stripcmd"
            ret = None or getattr( HgObj( where ), cmd )( ui, repo, *arg, **kwarg )
...
class Stream( object ) :
    ...
    def __eq__( self, rhs ) : return  0 == Stream.cmp( self, rhs )
    def __ne__( self, rhs ) : return  0 != Stream.cmp( self, rhs )
    def __lt__( self, rhs ) : return -1 == Stream.cmp( self, rhs )
    def __le__( self, rhs ) : return  1 >  Stream.cmp( self, rhs )
    def __gt__( self, rhs ) : return  1 == Stream.cmp( self, rhs )
    def __ge__( self, rhs ) : return -1 <  Stream.cmp( self, rhs )
...
class Branch( object ) :
    def __init__( self, ui, repo, rev = None ) :
        ...
        self._fullname = self.ctx.branch()
    ...
    def __eq__( self, rhs ) : return  0 == Branch.cmp( self, rhs )
    def __ne__( self, rhs ) : return  0 != Branch.cmp( self, rhs )
    def __lt__( self, rhs ) : return -1 == Branch.cmp( self, rhs )
    def __le__( self, rhs ) : return  1 >  Branch.cmp( self, rhs )
    def __gt__( self, rhs ) : return  1 == Branch.cmp( self, rhs )
    def __ge__( self, rhs ) : return -1 <  Branch.cmp( self, rhs )
...
class Flow( object ) :
    ...
    def latest_master_tags( self ) :
        ...
        while (master_context) :
            ...
            # If the current `master_context' is the first commit, its `parent' will be itself (not `None'!).
            parents = master_context.parents()
            if (1 == len( parents )) :
                if (parents[0].hg_obj == master_context.hg_obj) :
                    break
...
@command(b'flow',
         [(b"",  b"history",   False, _("Print history of hg commands used in this workflow."),                          ),
          (b"",  b"dry-run",   None,  _("Do not perform actions, just print history."),                                  ),
          (b"",  b"dirty",     False, _("Start a new branch from a dirty workspace, and move all"
                                        " uncommitted changes to the new branch. [start]"),                              ),
          (b"c", b"closed",    False, _("Show normal and closed branches in stream. [list, log]"),                       ),
          (b"c", b"commit",    False, _("Commit changes before closing the branch. [finish]"),                           ),
          (b"d", b"default",   False, _("Initialize flow with default configuration. [init]"),                           ),
          (b"d", b"date",      b'',   _("Record the specified date as commit date. [start, finish, promote]"), _('DATE'),),
          (b"d", b"date",      b'',   _("Show revisions matching date spec. [log]"),                           _('DATE'),),
          (b"d", b"dest",      b'',   _("Destination changeset of rebasing. [rebase]"),                        _('REV' ),),
          (b"e", b"erase",     False, _("Erase branch after it is merged or aborted successfully. [finish, abort]"),     ),
          (b"F", b"file",      [],    _("File to show history of. [log]"),                                     _('FILE'),),
          (b"f", b"force",     False, _("Force reinitializing flow. [init]"),                                            ),
          (b"g", b"git",       False, _("Use git extended diff format to show patch. [log]"),                            ),
          (b"k", b"keyword",   b'',   _("Do case-insensitive search for a given text. [log]"),                 _('TEXT'),),
          (b"l", b"limit",     b'',   _("Limit number of changesets displayed. [log]"),                                  ),
          (b"m", b"message",   b'',   _("Record TEXT as commit message. [start, finish, promote, abort]"),     _('TEXT'),),
          (b"p", b"patch",     False, _("Show patch. [log]"),                                                            ),
          (b"p", b"stamp",     b'',   _("Append TEXT to all commit messages. [init, upgrade, start, finish,"
                                        " promote, abort]"),                                                   _('TEXT'),),
          (b"r", b"rev",       b'',   _("Revision to start a new branch from. [start]"),                       _('REV'), ),
          (b"r", b"rev",       b'',   _("Revision to promote to other branches. [promote]"),                   _('REV'), ),
          (b"s", b"onstream",  False, _("Act on stream. [finish, rebase, log, abort]"),                                  ),
          (b"t", b"tag",       b'',   _("Tag the merging changeset with NAME. [promote]"),                     _('NAME'),),
          (b"t", b"tag",       b'',   _("Tag the <master> trunk with NAME after merging. [finish]"),           _('NAME'),),
          (b"t", b"to",        b'',   _("Rename the branch to NAME. [rename]"),                                _('NAME'),),
          (b"u", b"user",      b'',   _("Use specified user as committer. [init, upgrade, start, finish,"
                                        " promote]"),                                                          _('USER'),),
          (b"u", b"user",      b'',   _("Show revisions committed by specified user. [log]"),                  _('USER'),),
         ],
         b"hg flow {<stream> [<action> [<arg>]] | <command>} [<option>...]"
        )
...
"""
    ui    = HgObj        ( ui    )
    repo  = HgObj        ( repo  )
    cmd   = output_filter( cmd   )
    arg   = output_filter( arg   )
    kwarg = output_filter( kwarg )
    ...
    def print_help( self, topic = None, *arg, **opts ) :
        ...
        if (topic is None) :
            ...
            try :
                ...
                if ((b"@" + topic) in self.TOPIC or topic == "all") :
...
def _getopt( ui, key, arg, opt ) :
    ...
    bad_opt = [e for e in     opt if (e not in (["history", "dry_run"] + list( ret.keys() )) and opt[e])]

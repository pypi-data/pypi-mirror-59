    ...
    for list_name in registry.lazy_types:
        ...
        for key, node in type_list.items():
            ...
            if type_definition.get("lazy"):
                ...
                exclude_fields = [field.name for field in cls._meta.get_fields(include_parents=False)]

                # Add any custom fields to node if they are defined.
                node_fields = []
                model_fields = [field.name for field in cls._meta.get_fields(include_parents=False)]
                methods = {}
                if hasattr(cls, "graphql_fields"):
                    for field in cls.graphql_fields:
                        ...
                        if field.field_name in model_fields:
                            node_fields.append(field.field_name)

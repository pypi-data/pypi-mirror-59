...
class Output:
    ...
    def duration(self, duration: float) -> None:
        if not duration or duration < 0.:
            duration = 0.
        self.__set_parameter("maxDurationCapture", duration)

    @property
    @decorate.translate_requests
    @trace
    def size(self) -> typing.Optional[float]:
        """The maximum file size of the capture (MB),
        ``None`` for unlimited."""
        self.__update()
    ...
    def size(self, size: float) -> None:
        if not size or size < 0:
            size = 0
        self.__set_parameter("maxBytesCapture", size)
    ...
    def file_duration(self, file_duration: float) -> None:
        if not file_duration or file_duration < 0.:
            file_duration = 0.
        self.__set_parameter("maxDurationPerFile", file_duration)
    ...
    def file_size(self, file_size: float) -> None:
        if not file_size or file_size < 0:
            file_size = 0
        self.__set_parameter("maxBytesPerFile", file_size)

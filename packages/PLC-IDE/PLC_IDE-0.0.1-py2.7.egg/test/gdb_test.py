...
class TestPyGdbMi(unittest.TestCase):
    def test_parser(self):
        ...
        assert_match(
            parse_response(
                '=breakpoint-modified,bkpt={number="1",empty_arr=[],type="breakpoint",disp="keep",enabled="y",addr="0x000000000040059c",func="main",file="hello.cpp",fullname="hello.cpp",line="9",thread-groups=["i1"],times="1",original-location="hello.cpp:7"}'
            ),
            {
    ...
    def _get_c_program(self, makefile_target_name, binary_name):
        ...
        SAMPLE_C_CODE_DIR = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "hello.cpp"
        )

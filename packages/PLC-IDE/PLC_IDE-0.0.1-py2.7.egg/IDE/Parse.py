...
def Build(data,filepath):
    ...
    allvar ,dcVar = AddrGvlVariable(gvlVar,2)
    BuildScadaVariable(dcVar)
    ...
    for i in data['gvl'].keys():
        ...
        for m in ms:
            ...
            if _lenth == '' or int(_lenth) == 0:
                ...
                if _type in ALLCLASS.keys():
                    dc['child'] = ALLCLASS[_type]['member']
                elif _type in dcPRG.keys():
                    dc['child'] = dcPRG[_type]
                lt.append(dc)
            else:
...
def AddrGvlVariable(data,mode):
    ...
    def print_member(DATA, fix,start,node):
        ...
        for d in DATA:
            ...
            if d.has_key('child'):
                start = print_member_child(d['child'], fix + _name + '.', tb,tb1,start,node[len(node)-1]['child'])
    ...
    def print_member_child(DATA, fix, grid,grid2,start,node):
        for d in DATA:
            ...
            if d.has_key('child'):
                start = print_member_child(d['child'], fix + _name + '.', grid,grid2,start,node[len(node)-1]['child'])
...
def BuildScadaVariable(data):
    ...
    objJSON = CJSON()
    objJSON.writefile(_dcDrv,'Device.config')


    _dcVar = build_variable_table(data)
    objJSON.writefile(_dcVar, 'Variable.config')
    print 11
    pass

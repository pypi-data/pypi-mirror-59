...
from PyQt5.QtWidgets import QTreeWidget, QTreeWidgetItem, QPushButton, QApplication, QCheckBox
...
from ..gui.roi_list import RoiListView
...
class MainGUI(QMainWindow):
    ...
    def __init__(self, database, data, modules, splash = None):
        ...
        sql = """
            ...
            COALESCE(
                (SELECT 'channel-' || CAST(channel AS TEXT) from %s s where s.experiment_id=e.experiment_id),
                (SELECT title from %s s where s.experiment_id=e.experiment_id),
                (SELECT title from %s s where s.experiment_id=e.experiment_id),
                (SELECT title from %s s where s.experiment_id=e.experiment_id),
                'none') AS info,
            hardware FROM %s e""" % (database.table("strathkelvin"),
                                     database.table("electrophysiology"),
                                     database.table("mechanics"),
                                     database.table("confocal_catransient"),
        ...
        for name, value in database.connection_parameters.items():
            dbs += '%s: %s\n' % (name, value)
        open_layout.addWidget(QLabel(dbs[:-1]), stretch=1)
        open_file = QPushButton(' Open new experiment ')
        open_db_connection = QPushButton(' Change database ')
        open_buttons_layout.addWidget(open_file)
        open_buttons_layout.addStretch(1)
        open_buttons_layout.addWidget(open_db_connection)
        ...
        open_db_connection.clicked.connect(self.onOpenDbConnection)

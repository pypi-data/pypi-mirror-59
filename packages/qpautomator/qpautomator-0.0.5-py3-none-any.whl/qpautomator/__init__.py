...
# itype =begin: 用于切换到自带的输入法
# itype =text : 用于输入文本
# itype =clear: 清空文本
# itype =enter: 输入回车
class imeHelper(object):

    pool = urllib3.PoolManager()
    def __init__(self,ip="127.0.0.1",port=9600,timeout=30):
        self.url ="http://%s:%d/ime" % (ip,  port)
        self.timeout =timeout
        
    def __call__(self,itype,icontext=None):
        data = {"type": itype, "context": icontext}
...
class AutomatorServer(object):
    ...
    def __init__(self,rpc_server_host=None,rpc_port=None):
        self.uiautomator_process = None
        self.device_ip   = rpc_server_host
        self.device_port = rpc_port
    ...
    # RPC server 由uiautomator-helper来启动
    def start(self, timeout=5):
        pass
...
class AutomatorDevice(object):
    ...
    def __init__(self,rpc_server_host="127.0.0.1",rpc_port=9008,ime_port=9600):
        self.server     = AutomatorServer( rpc_server_host=rpc_server_host, rpc_port=rpc_port)
        self.shell      = shellHelper(ip=rpc_server_host,port=ime_port)
        self.imehost    = rpc_server_host
        self.imeport    = ime_port
...
class AutomatorDeviceUiObject(object):
    ...
    def __init__(self, device, selector):
        self.inputIme = imeHelper(ip=device.imehost,port=device.imeport)    
        self.device   = device
        self.jsonrpc  = device.server.jsonrpc
        self.selector = selector

        self.inputIme(itype="begin")
    ...
    def clear_text(self):
        self.click()
        self.inputIme(itype="clear")

    @property
    def write_text(self,text):
        self.click()
        self.inputIme(itype="text",icontext=text)


    @property
    def commit_text(self,text):
        self.write_text(text=text)
        self.inputIme(itype="enter")

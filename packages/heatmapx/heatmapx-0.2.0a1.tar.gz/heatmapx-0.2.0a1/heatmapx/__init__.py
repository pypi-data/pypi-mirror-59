...
# def draw_simply(G, prog='neato', layout=None, node_color='g', node_size=75, edge_color='gray', width=0.1, cmap=plt.get_cmap('viridis'), **kwargs):
...
#     nx.draw(G, pos=layout, cmap=cmap, with_labels=False,
#             node_color=node_color, node_size=node_size,
#             edge_color=edge_color, width=width, **kwargs)
#     return fig, layout
#
#
# def create_heatmap(G, layout, cmap):
#     try:
#         layout = nx.nx_agraph.graphviz_layout(G, prog=prog)
#     except ImportError:
#         layout = nx.layout.random_layout(G)
#     fig, _ = draw_simply(G, layout=layout,
#                          node_color=[cmap(G[node].get(key, 0)) for node in G.nodes()],
#                          edge_color=[cmap(edge[-1]) for edge in G.edges(data=key)], **kwargs)
#     return fig, layout

...
class DataProvider(object):
    ...
    def get_all_data(self,is_shuffle=False,topk=100):
        if is_shuffle==False:
            data=self.data[self.current_scenario][:topk]
            if isinstance(data[0],str):
                data = [image2array(d) for d in data]
            return data
        else:
            idxes=np.random.shuffle(np.arange(len(self)))
            data = self.data[self.current_scenario][idxes][:topk]
            if isinstance(data[0], str):
                data = [image2array(d) for d in data]
            return data
    ...
    def label_transform(self, label_data):
        label_data=label_backend_adaptive(label_data,list(self.class_names.values())[0])

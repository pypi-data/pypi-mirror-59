...
__all__ = ['read_image','read_mask','save_image','save_mask','image2array','array2image','mask2array','array2mask','list_pictures','normalize','unnormalize','random_crop','resize','add_noise','gray_scale','to_rgb','auto_level','random_invert_color',
           'image_backend_adaptive','reverse_image_backend_adaptive','random_adjust_hue', 'random_channel_shift', 'random_cutout','random_center_crop','random_adjust_gamma','random_contast','clahe','erosion_then_dilation','adaptive_binarization']
...
def random_contast(scale=(0.5, 1.5)):
    beta=0
...
def adaptive_binarization( block_size= 15,sigma=1):
    def img_op(image: np.ndarray):
        image=image
        blur = gaussian(image, sigma=sigma)
        local_thresh=threshold_otsu(blur)
        #local_thresh = threshold_local(image, block_size, method=method,offset=offset,mode=mode)

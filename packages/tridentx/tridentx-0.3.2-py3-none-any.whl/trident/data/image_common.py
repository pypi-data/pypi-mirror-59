...
__all__ = ['read_image','read_mask','save_image','save_mask','image2array','array2image','mask2array','array2mask','list_pictures','normalize','unnormalize','random_crop','resize','add_noise',
           'image_backend_adaptive', 'random_channel_shift', 'random_cutout','random_center_crop','random_adjust_gamma','random_contast']
...
def resize(size,keep_aspect=True):
    def img_op(image:np.ndarray):
        if keep_aspect:
            ...
            output=np.zeros((size[1],size[0],3))
            scale=min(scalex,scaley)
            image=transform.rescale(image, scale,clip=False,anti_aliasing=True,multichannel=True)

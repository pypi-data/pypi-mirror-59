...
__all__ = ['Adam','RAdam','Ranger','get_optimizer']
...
class Optimizer(object):
    ...
    def get_g(self):
        ...
        # for param in self.param_groups[0]:
        #     p_shape = param.shape
        #     weight_dict[p_shape] = param
        #     grad_dict[p_shape] = np.zeros_like(param)
        #
        # grads = loss.grad(at={loss.arguments[0]: training_context['current_input']}, wrt=list(weight_dict.values()),
        #                   as_numpy=True)
        # self.update(grads, training_context['current_input'].shape[0], is_sweep_end=False)
...
class Adam(C.Function,OptimizerMixin):
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8, weight_decay=0):
        self.optimizer=C.learners.adam(params,lr =lr,momentum=betas[0],variance_momentum=betas[1])

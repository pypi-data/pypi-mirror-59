...
class Conv2d_Block(Layer):
    def __init__(self, kernel_size=(3,3), num_filters=32, strides=1, auto_pad=True,padding_mode=PaddingMode.replicate, activation='relu',
                 normalization='batch',  use_bias=False,dilation=1, groups=1,add_noise=False,noise_intensity=0.005,dropout_rate=0,name=None,**kwargs):
    ...
    def build(self, input_shape):
        if self._built == False or self.conv is None:
            self.conv =Conv2d(kernel_size=self.kernel_size, num_filters=self.num_filters, strides=self.strides,auto_pad=self.auto_pad,padding_mode=self.padding_mode,  activation=None, use_bias=self.use_bias,dilation=self.dilation, groups=self.groups,name=self._name).to(_device)

    ...
    def build_yolov3_net(self, inputs, network_type, is_training):
        ...
        def build_boxes(inputs):
            ...
            if self.lite:
                ...
                classes = tf.boolean_mask(classes, selected_mask, axis=None)
                ...
                classes = tf.reshape(tensor=classes, shape=(-1, total_num, 1))
                ...
                '''
                self.results = {} 
                self.results['detection_boxes'] = tf.concat([top_left_x, top_left_y, bottom_right_x, bottom_right_y], axis = -1)
                self.results['detection_classes'] = classes
                self.results['detection_scores'] = confidence
                return self.results

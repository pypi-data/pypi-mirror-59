...
import os, sys, json, shutil, subprocess, re, errno, concurrent.futures
...
def tabulate(collection, args, cell_transforms=None):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        if getattr(args, "json", None):
            def render_json_row(row):
                return {f: get_cell(row, f, cell_transforms.get(f)) for f in args.columns}
            table = executor.map(render_json_row, collection)
            return json.dumps(table, indent=2, default=lambda x: str(x))
        else:
            def render_table_row(row):
                return [get_cell(row, f, cell_transforms.get(f)) for f in args.columns]
            table = executor.map(render_table_row, collection)
            ...
            format_args = {"auto_col_width": True} if args.max_col_width == 0 else {"max_col_width": args.max_col_width}

...
def test_run_script_inline_timeout(testdir):
    testdir.makepyfile("""
        def test_run_script_inline_timeout(bash):
            from pytest_shell.connection import TimeOutError
            import pytest
            with pytest.raises(TimeOutError):
                bash.run_script_inline(['sleep 0.5 && echo "blah"'], timeout=0.3)
    """)
    assert testdir.runpytest().ret == 0

def test_run_script_inline_timeout2(testdir):
    testdir.makepyfile("""
        def test_run_script_inline_timeout2(bash):
            import pytest
            bash.run_script_inline(['sleep 11.0 && echo "blah"'], timeout=12.0)
    """)
    assert testdir.runpytest().ret == 0

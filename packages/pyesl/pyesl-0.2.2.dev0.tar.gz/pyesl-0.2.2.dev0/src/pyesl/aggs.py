...
class Groupby(AggsBase):
    ...
    def __init__(self, field: str = 'key', name: str = None, size: Union[int, None] = 1024, **params):
        self._field = field
...
class Aggregations(AggsBase):
    ...
    **Aggregation combination of calculation(with filter), group by**
    ...
    :param groupby: Groupby structure list, group by tag's order which [ts, tag1, tag2] and so on, order by TopKType specified order
    ...
    :param topk_type: if topk sort in es or not, if True agg by ts first else terms, it means True order: [ts, tag1, tag2], else [tag1, tag2, ts]
    :type topk_type: TopKType
    :param page_after: pagination after which last tags key tags value
    :type page_after: Union[List[Dict], None]
    :param page_size: return size 返回数据大小,默认None,不分页，分页最大获取1000000
    :type page_size: Union[int, None]
    ...
    def __init__(
            ...
            page_after: Union[List[Dict], None] = None, page_size: Union[int, None] = None
    ):
        ...
        self._page_size = None if page_size is None or page_size <= 0 else page_size
        self._pagination = True if self._page_after or self._page_size else False
        _body = {}
        _cu = _body
        _result_path = []
        ...
        if self._topk is not None and self._topk > 0:
            if self._topk_type == TopKType.TOPK_GROUPBY.value:
                # 普通groupby聚合计算的TOPK，时间groupby放在最前面
                ...
                # 滑动窗口计算的TopK，活动窗口需要用到每个窗口上的数据，因此需要先用topk做groupby
            ...
            if self._pagination:
                ...
                _cu['aggs'] = {
                    'pagination_aggs': {
                        'composite': {
                            'sources': [
                                _g.body for _g in before_step_groupbys
                            ]
                        }
                    }
                }
        ...
        if self._step:
            if self._pagination:
                ...
                _aggs['pagination_aggs']['composite']['sources'].append(self._step.body)
                _cu['aggs'] = _aggs
        ...
        if after_step_groupby:
            if self._pagination:
                ...
                for _g in before_step_groupbys:
                    _aggs['pagination_aggs']['composite']['sources'].append(_g.body)
                _cu['aggs'] = _aggs
            else:
        ...
        if self._pagination:
            if self._page_size:
                _cu['aggs']['pagination_aggs']['composite']['size'] = self._page_size
            if self._page_after:
                _cu['aggs']['pagination_aggs']['composite']['after'] = self._page_after
            _cu['aggs']['pagination_aggs']['aggs'] = calculations.body['aggs']
            _result_path.append('pagination_aggs')
        else:

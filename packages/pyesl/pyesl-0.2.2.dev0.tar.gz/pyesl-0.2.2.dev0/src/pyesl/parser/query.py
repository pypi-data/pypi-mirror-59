...
class QueryParser(object):
    ...
    def aggs_by_within(
            ...
            topk: Union[int, None] = None, order: str = 'desc',
            page_after: Union[Dict[str, str], None] = None, page_size: Union[int, None] = None
    ) -> Aggregations:
        """
        **聚合语句块(合集)加上groupby和within语义**
        ...
        :type order: str
        :param page_after: 分页过滤条件
        :type page_after: Union[Dict[str, str], None]
        :param page_size: 每一分页的大小
        ...
        if groupby:
            ...
            for idx, _g in enumerate(groupby):
                if page_after or page_size:
                    _groupbys.append(Groupby(field='tag.{}'.format(_g), size=None))
                else:
        ...
        if step_s:
            ...
            _step = None
        return Aggregations(
            calculations=calculations, groupby=_groupbys, step=_step, topk=topk, topk_type=topk_type,

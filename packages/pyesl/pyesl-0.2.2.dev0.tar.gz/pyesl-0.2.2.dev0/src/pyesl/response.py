...
class Series(object):
    ...
    **Series structure of ts datapoint**
    ...
    @property
    def after_key(self) -> dict:
        return self._after_key

    @after_key.setter
    def after_key(self, value: dict):
        self._after_key = value
    ...
    def add_copy_from_tags(self, **tags: str) -> Series:
        new_s = Series(
            self.new_series_name_from_tags(**tags), deepcopy(self._legends), topk_filter=self._topk_filter)
    ...
    def new_series_name_from_tags(self, **tags: Dict[str, str]) -> str:
        return '{}#{}'.format(self._name, '#'.join('{}={}'.format(tag, tags[tag]) for tag in sorted(tags.keys())))
...
class TsdbResponse(object):
    def __init__(self, series: List[Series], response: dict):
        ...
        if self._series:
            self._page_after_key = self._series[0].after_key
    ...
    @property
    def page_after_key(self) -> dict:
        return self._page_after_key

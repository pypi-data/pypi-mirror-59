...
class ListDataset():
    def __init__(self,
        ...
        self.images, self.labels, self.aspect_ratios = [], [], []
        ...
        for line in lines:
            ...
            self.aspect_ratios.append(int(splited[1])/int(splited[2]))
            if self.multiclass:
                self.labels.append([int(i) for i in splited[3:]])
            else:
                self.labels.append(torch.LongTensor([int(splited[3])]))
        ...
        if self.opt.resize:
            self.resize = MyResizer(min_side=self.opt.min_size, max_side=self.opt.max_size)

    def __getitem__(self, idx):
        img_loc = self.images[idx]
        label = self.labels[idx]

        #if the given index has corrupted image,returns another indexs
        try:
            img, scale = self.get_image(img_loc)
        except Exception as e:
            with open("corrupted_imgs.txt", 'a') as write_file:
    ...
    def get_image(self, img_loc):
        ...
        if self.train:
            ...
            img["image"] = img["image"].astype(np.float32)/255.0
        else:
            img = {"image": img.astype(np.float32)/255.0}
        if self.opt.normalize:

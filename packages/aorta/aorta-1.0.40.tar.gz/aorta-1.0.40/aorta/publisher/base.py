...
from aorta.messaging.transaction import LocalTransaction
...
class BasePublisher:
    ...
    def publish(self, message, on_settled=None, to_backend=None):
        ...
        Args:
            ...
            on_settled: a callable that is invoked, with the message
                ...
                backend.
            to_backend: a callable that puts the message on a queue
                for transmission.

        Returns:
            None
        """
        to_backend = to_backend or self.to_backend
        self.prep_outbound_message(message)

        # Place the message on the outbound message queue and have the
        # backend schedule it for transmission to the remote AMQP peer.
        if on_settled is not None:
            on_settled = functools.partial(self.on_responsibility_transferred,
                on_settled)
        to_backend(message, on_settled=on_settled)

    def prep_outbound_message(self, message):
        """Prepares a message for transmission to the AMQP peer."""
    ...
    def to_backend(self, message, on_settled=None):
        """Put a message on the transmission backend for transfer to
        the AMQP peer.
        """
    ...
    def begin(self):
        """Begins a local transaction. Messages are not transmitted to the
        AMQP peer until the :meth:`commit()` method is invoked.
        """
        return LocalTransaction(self, self.backend)

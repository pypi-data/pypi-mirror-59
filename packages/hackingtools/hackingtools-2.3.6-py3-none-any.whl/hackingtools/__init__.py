...
def worker(workerName, functionCall, args=(), timesleep=1):
    Utils.startWorker(workerName, functionCall, args, int(timesleep))
...
if Utils.amIdjango(__name__):
    worker('refresh-pool-servers', 'ht.Pool.__checkPoolNodes__', timesleep=180)
    worker('clear-htpass-files', 'ht.Config.__cleanHtPassFiles__', timesleep=100)
    worker('clear-output-modules', 'ht.__cleanOutputModules__', timesleep=200)
# try:
#     for t in threads:
#         t.join()
# except:

...
def getModulesFunctionsCalls():
    ...
    ht_loaded_{module_name} = ht.getModule("{module_name}")
    {header_params}
    # Call the module\'s function for getting a result
    mod_result = ht_loaded_{module_name}.{module_function}({module_function_params})
                    ...
                    for param in ht.modules_loaded[module][func]['original_params']:
                        ...
                        if 'params' == param:
                            ...
                            for reqpar in required_params:
                                header_params[module][func] += '\n{p} = input(\'Set the param {p}: \')\nparams += {p}'.format(p=reqpar)

                        if 'defaults' == param:
                            default_params = ht.modules_loaded[module][func]['original_params'][param]
                            for defpar in default_params:
                                header_params[module][func] += '\n{p} = input(\'Set the param {p} (Default value: {d}): \')\nif not {p}:\n\t{p} = \'{d}\''.format(p=defpar, d=default_params[defpar])
            except:
                header_params = ''
    for module in ht.modules_loaded:
        module_funcs = {}
        for func in ht.modules_loaded[module]:
            try:
                for param in ht.modules_loaded[module][func]:
                    module_funcs[func] = header.format(header_params=header_params[module][func], module_name=module.split('.')[-1], module_function=func, module_function_params=', '.join(ht.modules_loaded[module][func][param]))
            except:

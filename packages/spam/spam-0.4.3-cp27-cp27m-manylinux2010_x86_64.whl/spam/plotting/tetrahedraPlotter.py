...
def plotDelaunay3D(delaunay,com1,center_fraction,tet_strains,strain_range,axstr=1):
    """ 
    Plots Delaunay triangulation with strains.

    Parameters
    ----------
        delaunay: Delaunay triangulation generated by mesh_sandbox.voro_to_delaunay_3d
        com1: Particles' x,y,z coordinates at original step in Np x 3 array. (Just used for plotting)
        center_fraction: Fraction of center of specimen to plot.
        tet_strains: List of strain tensors in each tetrahedron, from mesh_sandbox.voro_delaunay_to_strain_3d.
    ...
    Example
    -------
        mesh_sandbox.voro_plot_delaunay_3d(delaunay,com1,0.02,tet_sym,0.02,'Z')
    ...
    zmax = com1[:,2].max()
    xspan = xmax - xmin
    idx = np.transpose(np.where((com1[:,1]>(xmin+(0.5-center_fraction/2)*xspan)) & 
                   (com1[:,1]<(xmin+(0.5+center_fraction/2)*xspan))))
    
    # Now find all tetrahedra containing these centroids
    print("Finding tetrahedra to plot...\n")
    indelaunay = np.isin(delaunay,idx)
    idxtetbool,_ = np.where(indelaunay)
    ...
    for ii in (range(len(idxtet))):
        ...
        for jj in range(4):
            face_plot_ids.append(np.sort([delaunay[idxtet[ii]][combs[jj][0]],delaunay[idxtet[ii]][combs[jj][1]],delaunay[idxtet[ii]][combs[jj][2]]]))
    ...
    for ii in (range((face_plot_ids_unique.shape[0]))):
        tet_coms = com1[face_plot_ids_unique[ii],:]
        x.append(tet_coms[:,0])
        y.append(tet_coms[:,1])
        z.append(tet_coms[:,2])

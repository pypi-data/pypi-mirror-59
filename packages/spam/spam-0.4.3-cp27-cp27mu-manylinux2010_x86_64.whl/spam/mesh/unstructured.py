...
def triangulate(coordinates, alpha=None, weights=None):
    """
    ...
    Note
    ----
        Function contributed by Robert Caulk (Labortoire 3SR, Grenoble)
    ...
    nTet = mtk.countTetrahedraCGAL(coordinates, weights, alpha)
    connMat = numpy.zeros([nTet, 4], dtype='<u4')

    # setup the return types and argument types
    mtk.triangulateCGAL(coordinates, weights, connMat, alpha)
...
def bagiStrain(coordinates, connectivity, displacements, largeStrains=True, onlyStrain=True):
    ...
    for tet in pbar(range(connectivity.shape[0])):
        ...
        if max(tet_ids) >= coordinates.shape[0]:
            print("spam.mesh.unstructured.bagiStrain(): this tet has node > coordinates.shape[0], skipping")
            pass
        else:
            ...
            if numpy.isfinite(tetCoords).sum() + numpy.isfinite(tetDisp).sum() != 3*4*2:
                ...
                # Compute strains
                F[tet] = numpy.zeros((3,3))*numpy.nan
                volStrain[tet] = numpy.nan
                devStrain[tet] = numpy.nan
                if largeStrains:
                    U[tet] = numpy.zeros((3,3))*numpy.nan
                    R[tet] = numpy.zeros((3))*numpy.nan
                else:
                    e[tet] = numpy.zeros((3,3))*numpy.nan
            else:
...
def projectTetFieldToGrains(com2, connectivity, tetField):
    """
...
def projectBagiStrainToGrid(com2, connectivity, tet_strain, nx=8, ny=8, nz=10):
    """
...
def tetVolumes(coordinates, connectivity):
    ...
    for tetNumber in range(connectivity.shape[0]):
        ...
        if max(fourNodes) >= coordinates.shape[0]:
            print("spam.mesh.unstructured.tetVolumes(): this tet has node > coordinates.shape[0], skipping.")
            pass
        else:
            if numpy.isfinite(coordinates[fourNodes]).sum() != 3*4:

...
# TODO: generator objects don't appear as user-friendly when reviewing test failures
...
def get_return_value(x):
    ...
    value = x['return_value']
    if isinstance(x['return_value'], GeneratorType):
        return list(value)
    return value
...
class Recorder:
    ...
    def is_module_allowed(self, module):
        ...
        if 'simple_test_generator' in module_name:
            logger.debug('Excluding the recorder itself')
            return
        # if 'pytest' in module_name:
        #     logger.debug('Excluding pytest and its plugins')
        #     return
        if 'pydev' in module_name or 'py.builtin' in module_name:
            logger.debug('Excluding debugger modules')
            return
        if exclude_site_package and is_site_package(module):
            logger.debug(f'excluding site package {module}')
            return
        if exclude_importers(module):
            logger.debug(f'excluding importer {module}')
            return
        if exclude_system_package and is_system_package(module):
            logger.debug(f'excluding system module {module}')
            return
        return True

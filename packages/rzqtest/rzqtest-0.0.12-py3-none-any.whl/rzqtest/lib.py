    ...
    def __init__(self, filename):
        ...
        try:
            with open(filename, 'r', encoding='utf-8') as hander_urls:
                ...
                content = ''
                for url in hander_urls.readlines():
...
class Test(ABC):
    ...
    def test(self, Urls):
        ...
        summary = self._type+'完成，总数：{0}条，问题：{1}条，跳过：{2}'.format(len(Urls.get_urls()),i,jump)
...
class TestUrl(Test):
    ...
    def _run(self, url):
        ...
        try:
        	...
        	if num > 400:
        		return template.format(url, '失效:'+str(num))
        except Exception as e:
            return template.format(url, '失效:'+ str(e))
        return ''
